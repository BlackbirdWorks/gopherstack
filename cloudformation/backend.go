...
type StorageBackend interface {
	...
	CreateChangeSet(ctx context.Context, stackName, changeSetName, templateBody, description string, params []Parameter) (*ChangeSet, error)
...
func (b *InMemoryBackend) CreateStack(ctx context.Context, name, templateBody string, params []Parameter, tags []Tag) (*Stack, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	b.addEvent(arn, name, name, arn, "AWS::CloudFormation::Stack", "CREATE_IN_PROGRESS", "User Initiated")
	...
	if templateBody != "" {
		...
		if err != nil {
			...
			b.addEvent(arn, name, name, arn, "AWS::CloudFormation::Stack", "CREATE_FAILED", err.Error())
		...
		for logicalID, res := range tmpl.Resources {
			b.addEvent(arn, name, logicalID, "", res.Type, "CREATE_IN_PROGRESS", "")
			...
			if cerr != nil {
				...
				b.addEvent(arn, name, logicalID, "", res.Type, "CREATE_FAILED", cerr.Error())
				b.addEvent(arn, name, name, arn, "AWS::CloudFormation::Stack", "ROLLBACK_IN_PROGRESS", cerr.Error())
				b.addEvent(arn, name, name, arn, "AWS::CloudFormation::Stack", "ROLLBACK_COMPLETE", "")
			...
			b.addEvent(arn, name, logicalID, physicalID, res.Type, "CREATE_COMPLETE", "")
		}
	...
	b.addEvent(arn, name, name, arn, "AWS::CloudFormation::Stack", "CREATE_COMPLETE", "")

	return stack, nil
}

// UpdateStack updates an existing stack.
func (b *InMemoryBackend) UpdateStack(ctx context.Context, nameOrID, templateBody string, params []Parameter) (*Stack, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	b.addEvent(stack.StackID, stack.StackName, stack.StackName, stack.StackID, "AWS::CloudFormation::Stack", "UPDATE_IN_PROGRESS", "User Initiated")
	...
	if stack.TemplateBody != "" {
		...
		for logicalID, res := range tmpl.Resources {
			if existing, exists := b.resources[stack.StackID][logicalID]; exists {
				existing.Status = "UPDATE_COMPLETE"
				b.addEvent(stack.StackID, stack.StackName, logicalID, existing.PhysicalID, res.Type, "UPDATE_COMPLETE", "")
			} else {
				b.addEvent(stack.StackID, stack.StackName, logicalID, "", res.Type, "CREATE_IN_PROGRESS", "")
				physicalID, cerr := b.creator.Create(ctx, logicalID, res.Type, res.Properties, resolvedParams, physicalIDs)
				if cerr != nil {
					stack.StackStatus = "UPDATE_FAILED"
					stack.StackStatusReason = fmt.Sprintf("resource %s: %v", logicalID, cerr)
				...
				b.addEvent(stack.StackID, stack.StackName, logicalID, physicalID, res.Type, "CREATE_COMPLETE", "")
			}
		}
	...
	b.addEvent(stack.StackID, stack.StackName, stack.StackName, stack.StackID, "AWS::CloudFormation::Stack", "UPDATE_COMPLETE", "")
...
func (b *InMemoryBackend) DeleteStack(ctx context.Context, nameOrID string) error {
	...
	b.addEvent(stack.StackID, stack.StackName, stack.StackName, stack.StackID, "AWS::CloudFormation::Stack", "DELETE_IN_PROGRESS", "User Initiated")

	for logicalID, res := range b.resources[stack.StackID] {
		b.addEvent(stack.StackID, stack.StackName, logicalID, res.PhysicalID, res.Type, "DELETE_IN_PROGRESS", "")
		_ = b.creator.Delete(ctx, res.Type, res.PhysicalID, res.Properties)
		b.addEvent(stack.StackID, stack.StackName, logicalID, res.PhysicalID, res.Type, "DELETE_COMPLETE", "")
	}

	now := time.Now()
	stack.DeletionTime = &now
	stack.StackStatus = "DELETE_COMPLETE"
	b.addEvent(stack.StackID, stack.StackName, stack.StackName, stack.StackID, "AWS::CloudFormation::Stack", "DELETE_COMPLETE", "")
...
func (b *InMemoryBackend) CreateChangeSet(_ context.Context, stackName, changeSetName, templateBody, description string, params []Parameter) (*ChangeSet, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	cs := &ChangeSet{
		ChangeSetID:   fmt.Sprintf("arn:aws:cloudformation:%s:%s:changeSet/%s/%s", b.region, b.accountID, changeSetName, csID),
	...
	if templateBody != "" {
		...
		if err == nil {
			for logicalID, res := range tmpl.Resources {
				action := "Add"
				if stack != nil {
					if b.resources[stack.StackID] != nil {
