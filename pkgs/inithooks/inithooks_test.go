...
func TestInithooks(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Run_SuccessfulScript", run: func(t *testing.T) {
			dir := t.TempDir()
		...
		{name: "Run_FailingScript_ContinuesNext", run: func(t *testing.T) {
			dir := t.TempDir()
			out := filepath.Join(dir, "output.txt")
		...
		{name: "Run_WithLogger", run: func(t *testing.T) {
		...
		{name: "Run_MultipleScripts", run: func(t *testing.T) {
			dir := t.TempDir()
