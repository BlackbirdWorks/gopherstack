...
describe('CloudWatch Page', () => {
	...
	it('displays loaded alarms', async () => {
		mockSend.mockResolvedValue({
			MetricAlarms: [
				{ AlarmName: 'high-cpu', StateValue: 'ALARM', Namespace: 'AWS/EC2', MetricName: 'CPUUtilization', Threshold: 80, ComparisonOperator: 'GreaterThanThreshold', Period: 300, EvaluationPeriods: 1 },
				{ AlarmName: 'low-disk', StateValue: 'OK', Namespace: 'AWS/EBS', MetricName: 'VolumeReadOps', Threshold: 10, ComparisonOperator: 'LessThanThreshold', Period: 60, EvaluationPeriods: 3 }
			]
		});
		render(CloudWatchPage);
		await waitFor(() => {
	...
	it('filters alarms via search', async () => {
		mockSend.mockResolvedValue({
			MetricAlarms: [
				{ AlarmName: 'cpu-alarm', StateValue: 'OK', Namespace: 'AWS/EC2', MetricName: 'CPUUtilization', Threshold: 80, ComparisonOperator: 'GreaterThanThreshold', Period: 300, EvaluationPeriods: 1 },
				{ AlarmName: 'disk-alarm', StateValue: 'ALARM', Namespace: 'AWS/EBS', MetricName: 'DiskRead', Threshold: 90, ComparisonOperator: 'GreaterThanThreshold', Period: 300, EvaluationPeriods: 1 }
			]
		});
		render(CloudWatchPage);
		await waitFor(() => {
