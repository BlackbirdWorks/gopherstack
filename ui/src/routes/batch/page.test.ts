...
describe('Batch Page', () => {
	...
	it('displays loaded job queues', async () => {
		mockSend.mockResolvedValue({
			jobQueues: [
				{ jobQueueName: 'high-priority-queue', state: 'ENABLED', status: 'VALID', priority: 100, computeEnvironmentOrder: [] },
				{ jobQueueName: 'low-priority-queue', state: 'DISABLED', status: 'VALID', priority: 10, computeEnvironmentOrder: [] }
			]
		});
		render(BatchPage);
		await waitFor(() => {
