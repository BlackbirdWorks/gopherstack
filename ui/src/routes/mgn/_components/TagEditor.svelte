<script lang="ts">
	// Reusable tag editor shared by every one of MGN's 12 taggable resource
	// kinds (services/mgn/tagging.go) -- TagResource/UntagResource/
	// ListTagsForResource all key off a caller-built ARN, so each panel only
	// needs to hand this component the resource's current tag map plus two
	// callbacks; the add/remove wire calls themselves are identical across
	// every kind.
