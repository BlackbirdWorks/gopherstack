...
describe('ECS Page', () => {
	...
	it('displays loaded clusters', async () => {
		mockSend.mockResolvedValueOnce({ clusterArns: [clusterArn] }); // ListClusters
		mockSend.mockResolvedValueOnce({ serviceArns: [] }); // ListServices
		mockSend.mockResolvedValueOnce({ taskArns: [] }); // ListTasks
		mockSend.mockResolvedValueOnce({ taskDefinitionArns: [] }); // ListTaskDefinitions
		render(ECSPage);
		await waitFor(() => {
