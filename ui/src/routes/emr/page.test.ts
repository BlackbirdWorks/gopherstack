...
describe('EMR Page', () => {
	...
	it('navigates to cluster detail', async () => {
		mockSend.mockResolvedValue({
			Clusters: [{ Id: 'j-abc', Name: 'my-cluster', Status: { State: 'RUNNING' }, NormalizedInstanceHours: 5 }]
		});
		render(EMRPage);
		...
		mockSend.mockResolvedValue({
			Cluster: {
				...
				Applications: [{ Name: 'Spark', Version: '3.5.0' }, { Name: 'Hadoop', Version: '3.3.6' }]
			}
		});
