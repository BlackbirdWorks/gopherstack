...
describe('STS Page', () => {
	...
	it('renders page title', () => {
		mockSend.mockResolvedValue({ Account: '123456789012', Arn: 'arn:aws:iam::123:user/test', UserId: 'AIDAI...' });
		render(STSPage);
		expect(screen.getByText('AWS Security Token Service')).toBeInTheDocument();
	});

	it('shows caller identity after load', async () => {
		mockSend.mockResolvedValue({ Account: '123456789012', Arn: 'arn:aws:iam::123:user/test', UserId: 'AIDAI12345' });
		render(STSPage);
		await waitFor(() => {
