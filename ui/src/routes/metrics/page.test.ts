	...
	return {
		dashboardClient: {
			streamMetrics: vi.fn().mockImplementation(async function* () {
				yield {
					dashboard: {
						runtime: { goroutines: 42, heapAllocMb: 10.5, heapSysMb: 20.0, heapInuseMb: 8.0, numServices: 3, totalAllocMb: 50.0 },
						operations: [
							{ service: 'DynamoDB', operation: 'DynamoDB::ListTables', count: 100, errorCount: 0, avgMs: 15.5, p50Ms: 12.0, p95Ms: 25.0, p99Ms: 30.0, maxMs: 45.0 }
						],
						workers: []
					}
				};
