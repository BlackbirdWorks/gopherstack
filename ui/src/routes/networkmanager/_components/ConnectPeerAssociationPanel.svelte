<script lang="ts">
	// Connect peer associations -- one of the five kinds AssociationsPanel
	// composes (services/networkmanager/PARITY.md family J), and the genuine
	// structural bridge to the Cloud WAN half: it binds an already-created
	// Cloud WAN ConnectPeer (see the Connect Peers tab) to an on-prem
	// Device/Link, same shape as its four Global-Networks-side siblings.
	import {
	...
	const rows = $derived(
		connectPeerAssociations.filter((a) => matchesSearch(searchQuery, a.ConnectPeerId, a.DeviceId))
	);
...
<Modal bind:this={createModal} title="Create Connect Peer association">
	{#snippet children()}
		<div class="space-y-3">
			<label class="flex flex-col gap-1 text-sm" for="nm-assoc-cp-id">
				Connect peer ID *
				<input id="nm-assoc-cp-id" bind:value={formForeignId} class="px-2 py-1 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white" />
			</label>
			<label class="flex flex-col gap-1 text-sm" for="nm-assoc-cp-device">
				Device ID *
				<input id="nm-assoc-cp-device" bind:value={formDeviceId} class="px-2 py-1 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white" />
			</label>
			<label class="flex flex-col gap-1 text-sm" for="nm-assoc-cp-link">
				Link ID (optional)
				<input id="nm-assoc-cp-link" bind:value={formLinkId} class="px-2 py-1 text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white" />
			</label>
