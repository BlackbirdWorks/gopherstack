...
describe('CodeBuild Page', () => {
	...
	it('creates a project via the modal form', async () => {
		...
		mockSend.mockResolvedValueOnce({ projects: [{ name: 'new-build', source: { type: 'GITHUB' } }] });
	...
	it('selects a project and loads builds', async () => {
		...
		mockSend.mockResolvedValueOnce({ projects: [{ name: 'service-build', source: { type: 'GITHUB' } }] });
