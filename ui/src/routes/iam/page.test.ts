...
const mockPolicy = { PolicyName: 'MyPolicy', Arn: 'arn:aws:iam::123:policy/MyPolicy', AttachmentCount: 2 };
...
describe('IAM Page', () => {
	...
	it('switches to Policies tab and shows policies', async () => {
		...
		await waitFor(() => expect(screen.getByText('No users found')).toBeInTheDocument(), { timeout: 3000 });
		const tabs = screen.getAllByRole('button');
		const policiesTab = tabs.find(t => t.textContent?.includes('Policies') && t.tagName === 'BUTTON');
