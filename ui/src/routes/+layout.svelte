	...
	<nav class="fixed top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-slate-200 dark:bg-slate-900/80 dark:border-white/5 shadow-sm">
		<div class="px-3 py-3 lg:px-5 lg:pl-3 flex items-center justify-between">
			...
			<div class="flex items-center gap-1">
				...
				<a href="/dashboard/docs" class={`inline-flex items-center justify-center w-9 h-9 text-slate-500 rounded-lg hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-white transition-colors ${isActiveTab('docs') ? 'bg-slate-100 text-indigo-600 dark:bg-slate-800 dark:text-indigo-400 font-semibold' : ''}`} title="Docs">
				...
				<div class="relative">
					<button onclick={() => themeDropdownOpen = !themeDropdownOpen} type="button" class="inline-flex items-center justify-center w-9 h-9 text-slate-500 rounded-lg hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-white transition-colors" title="Theme: {themeLabels[theme]}">
					...
					{#if themeDropdownOpen}
						<div class="absolute right-0 mt-1 w-36 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-50">
							<div class="py-1 text-sm">
								{#each themes as t}
									<button onclick={() => selectTheme(t)} class="w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 flex items-center gap-2 {theme === t ? 'bg-slate-50 dark:bg-slate-700 font-semibold text-indigo-600 dark:text-indigo-400' : ''}">
										<span class="w-3 h-3 rounded-full {t === 'light' ? 'bg-amber-400' : t === 'dark' ? 'bg-slate-600' : t === 'github' ? 'bg-gray-800' : 'bg-cyan-500'}"></span>
										{themeLabels[t]}
									</button>
								{/each}
							</div>
