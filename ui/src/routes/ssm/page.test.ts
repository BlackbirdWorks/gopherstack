...
describe('SSM Page', () => {
	...
	it('displays loaded parameters', async () => {
		mockSend.mockResolvedValueOnce({
			Parameters: [
				{ Name: '/myapp/db/password', Type: 'SecureString', LastModifiedDate: new Date(), Version: 3 },
	...
	it('closes modal on cancel', async () => {
		...
		await waitFor(() => {
			expect(screen.queryByPlaceholderText('e.g. /myapp/database/password')).not.toBeInTheDocument();
		});
	});
