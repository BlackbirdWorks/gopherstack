<script lang="ts">
	...
	async function loadData() {
		...
		try {
			...
			toast.error('Failed to load CodeStar Connections data: ' + String(e));
		} finally {
			loading = false;
		}
	}
...
<div class="p-6 space-y-6">
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-3">
			<Link2 class="w-7 h-7 text-orange-500" />
			<div>
				<h1 class="text-2xl font-bold text-gray-900 dark:text-white">AWS CodeStar Connections</h1>
				<p class="text-sm text-gray-500 dark:text-gray-400">Manage connections to third-party source control providers</p>
			</div>
		</div>
		<button
			onclick={loadData}
	...
	<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
		<div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 flex items-center gap-3">
			<div class="p-2 bg-orange-100 dark:bg-orange-900/30 rounded-lg">
				<Link2 class="w-5 h-5 text-orange-600 dark:text-orange-400" />
			</div>
			<div>
		...
		<div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 flex items-center gap-3">
			...
			<div>
				<p class="text-2xl font-bold text-gray-900 dark:text-white">{syncBlockerCount}</p>
	...
	<div class="flex gap-2 border-b border-slate-200 dark:border-slate-700">
		{#each [['connections', 'Connections'], ['hosts', 'Hosts'], ['repolinks', 'Repo Links']] as [tab, label]}
			<button
				...
				class="px-4 py-2 text-sm font-medium border-b-2 transition-colors {activeTab === tab ? 'border-orange-500 text-orange-600 dark:text-orange-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}"
			>{label}</button>
		{/each}
	</div>
						...
						{:else}
							{#each connections as conn}
								<button
									...
									class="w-full text-left p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 {selectedConnection?.ConnectionArn === conn.ConnectionArn ? 'bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800' : ''}"
								>
									<div class="flex items-center justify-between">
				...
				<div class="lg:col-span-2 space-y-4">
									...
									{#each syncConfigs as cfg}
										<div class="flex items-center justify-between p-2 rounded bg-gray-50 dark:bg-slate-700">
											...
											<div class="flex items-center gap-2">
												<span class="text-xs text-gray-500 dark:text-gray-400">{cfg.SyncType}</span>
					...
					{:else}
						<div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-8 text-center text-gray-500 dark:text-gray-400 text-sm">
							Select a connection to view sync configurations and blocker status
						</div>
					{/if}
				</div>
			</div>
