...
describe('WorkSpaces Page', () => {
	...
	it('creates a workspace via the modal form', async () => {
		mockSend.mockResolvedValueOnce({ Workspaces: [] });
		mockSend.mockResolvedValueOnce({ Bundles: [{ BundleId: 'wsb-std', Name: 'Standard', ComputeType: { Name: 'STANDARD' } }] });
		// After create: reload
		mockSend.mockResolvedValueOnce({ PendingRequests: [{ WorkspaceId: 'ws-new' }], FailedRequests: [] });
		mockSend.mockResolvedValueOnce({ Workspaces: [{ WorkspaceId: 'ws-new', UserName: 'new.user', State: 'PENDING', BundleId: 'wsb-std' }] });
		mockSend.mockResolvedValueOnce({ Bundles: [] });
