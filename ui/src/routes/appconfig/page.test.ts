...
describe('AppConfig Page', () => {
	...
	it('opens create modal when button is clicked', async () => {
		...
		expect(screen.getByPlaceholderText('e.g. gopherstack-microservices-config')).toBeInTheDocument();
	});
	...
	it('closes create modal on abort button click', async () => {
		...
		await waitFor(() => {
			expect(screen.queryByPlaceholderText('e.g. gopherstack-microservices-config')).not.toBeInTheDocument();
		});
	});
	...
	it('selects an app and loads its details', async () => {
		...
		mockSend.mockResolvedValueOnce({ Items: [{ Id: 'env-1', Name: 'production', State: 'READY_FOR_DEPLOYMENT' }] });
		mockSend.mockResolvedValueOnce({ Items: [{ Id: 'prof-1', Name: 'feature-flags', Type: 'AWS.AppConfig.FeatureFlags' }] });
		// ListDeployments
		mockSend.mockResolvedValueOnce({ Items: [] });
