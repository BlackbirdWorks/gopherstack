...
<div class="space-y-6">
				...
				{#each filteredBuses as bus}
					<div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 flex items-center justify-between">
						...
						<div class="flex items-center gap-2 ml-4">
							...
							{#if bus.Name !== 'default'}<button onclick={() => deleteBus(bus.Name ?? '')} class="p-1.5 text-slate-400 hover:text-red-500"><Trash2 class="w-4 h-4" /></button>{/if}
						</div>
					</div>
				{/each}
			</div>
	...
	{:else if activeTab === 'rules'}
		<div class="space-y-4">
			<div>
				{#if selectedBus}<p class="text-sm text-slate-600 dark:text-slate-400">Rules for: <span class="font-semibold text-slate-900 dark:text-white">{selectedBus.Name}</span></p>
