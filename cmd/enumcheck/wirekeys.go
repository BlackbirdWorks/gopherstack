...
// the result at all -- there is nothing to check it against. This bare-key
// map only backs the map[string]any/index-assignment paths now (checked
// against every candidate that shares the key, unresolved when 2+ disagree)
// -- the named-struct-literal path resolves through enumRegistry.sdkFieldTypes
// instead, precisely, one field at a time.
//
...
func wireGroundTruth(deserializersGoPath string, reg *enumRegistry) (map[string]wireKeyFact, error) {
	...
	return wireKeyFactsFromEnums(enums, polymorphic), nil
}
