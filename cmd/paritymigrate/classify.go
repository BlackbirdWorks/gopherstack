...
// drop reports whether an item is safe to auto-remove. Only
// verdictResolvedElsewhere qualifies -- the exact gopherstack-anjf shape (a
// claim in one place, a fix citation in a dated section elsewhere in the
// file). verdictSelfResolved looked promising but measured a ~50% false
// positive rate hand-checking ecs/codebuild/glue/s3 (confirmed live: ecs's
// "STALE ENTRY, CORRECTED" bullet and glue's MLTaskRun bullet both matched a
// stray positive+dated marker despite disclosing real, still-open remaining
// gaps in the same text) -- it is reported (see classification.verdict's
// String) but never auto-dropped.
func (v verdict) drop() bool {
	return v == verdictResolvedElsewhere
}
