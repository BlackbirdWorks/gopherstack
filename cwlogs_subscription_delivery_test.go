...
func readKinesisRecords(t *testing.T, kb *kinesisbackend.InMemoryBackend, stream string) [][]byte {
	...
	iter, err := kb.GetShardIterator(context.Background(), &kinesisbackend.GetShardIteratorInput{
		StreamName:        stream,
	...
	out, err := kb.GetRecords(
		context.Background(),
		&kinesisbackend.GetRecordsInput{ShardIterator: iter.ShardIterator, Limit: 100},
	)
	if err != nil {
