...
resource "aws_iam_role" "redshift" {
  name = "mega-batch-18-redshift-role-{{.Suffix}}"
...
resource "aws_kms_key" "this" {
  description = "mega-batch-18 kms key"
}

resource "aws_s3_bucket" "logs" {
  bucket = "mega-batch-18-logs-bucket-{{.Suffix}}"
}

resource "aws_sns_topic" "events" {
  name = "mega-batch-18-events-{{.Suffix}}"
}
...
resource "aws_redshift_subnet_group" "this" {
  name       = "mega-batch-18-subnet-group-{{.Suffix}}"
  subnet_ids = ["subnet-00000000", "subnet-11111111"]
}

resource "aws_redshift_parameter_group" "this" {
  name   = "mega-batch-18-params-{{.Suffix}}"
  family = "redshift-1.0"
...
resource "aws_redshift_cluster" "this" {
  cluster_identifier            = "mega-batch-18-cluster-{{.Suffix}}"
...
resource "aws_redshift_authentication_profile" "this" {
  authentication_profile_name = "mega-batch-18-auth-profile-{{.Suffix}}"
  authentication_profile_content = jsonencode({
    AllowDbUserOverride = "1"
    Client_ID           = "mega-batch-18"
    App_ID              = "mega-batch-18-app"
  })
}

resource "aws_redshift_hsm_client_certificate" "this" {
  hsm_client_certificate_identifier = "mega-batch-18-hsm-cert-{{.Suffix}}"
}

resource "aws_redshift_hsm_configuration" "this" {
  hsm_configuration_identifier  = "mega-batch-18-hsm-config-{{.Suffix}}"
  description                   = "mega batch 18 hsm configuration"
  hsm_ip_address                = "10.0.0.100"
  hsm_partition_name            = "mega-batch-18-partition"
...
resource "aws_redshift_snapshot_copy_grant" "this" {
  snapshot_copy_grant_name = "mega-batch-18-copy-grant-{{.Suffix}}"
  kms_key_id                = aws_kms_key.this.arn
}

resource "aws_redshift_snapshot_schedule" "this" {
  identifier  = "mega-batch-18-schedule-{{.Suffix}}"
  description = "mega batch 18 snapshot schedule"
...
resource "aws_redshift_cluster_snapshot" "this" {
  cluster_identifier  = aws_redshift_cluster.this.cluster_identifier
  snapshot_identifier = "mega-batch-18-snapshot-{{.Suffix}}"
}

resource "aws_redshift_event_subscription" "this" {
  name          = "mega-batch-18-events-sub-{{.Suffix}}"
...
resource "aws_redshift_logging" "this" {
  ...
  s3_key_prefix       = "mega-batch-18/"
}

resource "aws_redshift_scheduled_action" "this" {
  name     = "mega-batch-18-scheduled-action-{{.Suffix}}"
...
resource "aws_redshift_partner" "this" {
  ...
  cluster_identifier  = aws_redshift_cluster.this.cluster_identifier
  database_name       = "testdb"
  partner_name        = "mega-batch-18-partner"
}

resource "aws_redshift_endpoint_access" "this" {
  cluster_identifier      = aws_redshift_cluster.this.cluster_identifier
  endpoint_name            = "mb18-endpoint-{{.Suffix}}"
...
resource "aws_db_subnet_group" "aurora" {
  name       = "mega-batch-18-aurora-subnet-group-{{.Suffix}}"
  subnet_ids = ["subnet-00000000", "subnet-11111111"]
}

resource "aws_rds_cluster" "aurora" {
  cluster_identifier      = "mega-batch-18-aurora-{{.Suffix}}"
...
resource "aws_redshift_integration" "this" {
  integration_name = "mega-batch-18-integration-{{.Suffix}}"
