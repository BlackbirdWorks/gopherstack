...
type mockTRANSFERIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockTRANSFERIAMBackend() *mockTRANSFERIAMBackend {
	return &mockTRANSFERIAMBackend{
...
func (m *mockTRANSFERIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockTRANSFERIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockTRANSFERIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockTRANSFERIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupTRANSFEREnforcementServer(t *testing.T, iamBackend *mockTRANSFERIAMBackend) *httptest.Server {
	t.Helper()

	backend := transfer.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := transfer.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockTRANSFERIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIATRANSFERUSER2",
			target:      "TransferService.CreateServer",
			body:        `{"EndpointType":"PUBLIC"}`,
			setupBackend: func(b *mockTRANSFERIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIATRANSFERUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockTRANSFERIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupTRANSFEREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/transfer/aws4_request, SignedHeaders=host, Signature=mock",
			)
