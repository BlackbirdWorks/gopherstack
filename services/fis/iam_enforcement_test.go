...
type mockFISIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockFISIAMBackend() *mockFISIAMBackend {
	return &mockFISIAMBackend{
...
func (m *mockFISIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockFISIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockFISIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockFISIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupFISEnforcementServer(t *testing.T, iamBackend *mockFISIAMBackend) *httptest.Server {
	t.Helper()
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAFISUSER1",
			method:      "GET",
			path:        "/experiments",
			body:        ``,
			setupBackend: func(b *mockFISIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAFISUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockFISIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupFISEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/fis/aws4_request, SignedHeaders=host, Signature=mock",
			)
