...
// TestTypedSlice21RealClient drives fis's remaining typed-coverage-blind ops
// (gopherstack-n3zi slice 21) through the real aws-sdk-go-v2 client:
...
func TestTypedSlice21RealClient(t *testing.T) {
	...
	t.Run("action and target resource type catalog", func(t *testing.T) {
		t.Parallel()
		...
		getActionOut, err := client.GetAction(ctx, &fissdk.GetActionInput{Id: aws.String("aws:ec2:reboot-instances")})
	...
	t.Run("experiment template lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("experiment lifecycle", func(t *testing.T) {
		t.Parallel()
