...
const slice11RoleArn = "arn:aws:iam::000000000000:role/sfn-role"

// TestTypedSlice11RealClient drives stepfunctions's typed-coverage-blind ops
// (gopherstack-n3zi slice 11) through the real aws-sdk-go-v2 sfn client.
func TestTypedSlice11RealClient(t *testing.T) {
	t.Parallel()

	t.Run("activities and list state machines", func(t *testing.T) {
		t.Parallel()
		...
		createSM, err := client.CreateStateMachine(ctx, &sfnsdk.CreateStateMachineInput{
			...
			RoleArn:    aws.String(slice11RoleArn),
	...
	t.Run("send task failure and heartbeat", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("update state machine", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("redrive execution", func(t *testing.T) {
		t.Parallel()
