...
func TestParity_GetCredentialsForIdentity_EmptyLoginsBypass(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pool, err := b.CreateIdentityPool("creds-bypass-"+tt.name, true, false, "", nil, nil, nil)
			require.NoError(t, err)

			identity, err := b.GetID(pool.IdentityPoolID, "000000000000", tt.seedLogins)
			require.NoError(t, err)

			creds, err := b.GetCredentialsForIdentity(identity.IdentityID, tt.reqLogins)
