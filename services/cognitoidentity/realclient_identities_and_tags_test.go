...
// TestRealClient_IdentitiesAndTags drives cognitoidentity's typed-coverage-blind
// ops (gopherstack-n3zi) through the real aws-sdk-go-v2 client.
func TestRealClient_IdentitiesAndTags(t *testing.T) {
	t.Parallel()
	cases := []struct {
		...
		{name: "tags", run: func(t *testing.T) {
			...
			poolARN := "arn:aws:cognito-identity:us-east-1:000000000000:identitypool/" + aws.ToString(
				pool.IdentityPoolId,
			)
		...
		{name: "principal tag attribute map", run: func(t *testing.T) {
			t.Helper()
			...
			defaultOut, err := client.GetPrincipalTagAttributeMap(
				ctx,
				&cognitoidentitysdk.GetPrincipalTagAttributeMapInput{
		...
		{name: "identities lifecycle", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "merge developer identities", run: func(t *testing.T) {
			t.Helper()
