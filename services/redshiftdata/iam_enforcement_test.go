...
type mockREDSHIFTDATAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockREDSHIFTDATAIAMBackend() *mockREDSHIFTDATAIAMBackend {
	return &mockREDSHIFTDATAIAMBackend{
...
func (m *mockREDSHIFTDATAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockREDSHIFTDATAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockREDSHIFTDATAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockREDSHIFTDATAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupREDSHIFTDATAEnforcementServer(t *testing.T, iamBackend *mockREDSHIFTDATAIAMBackend) *httptest.Server {
	t.Helper()

	backend := redshiftdata.NewInMemoryBackend("000000000000", "us-east-1")
	handler := redshiftdata.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockREDSHIFTDATAIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIAREDSHIFTDATAUSER1",
			target:      "RedshiftData.ListDatabases",
			body:        `{"Database":"dev"}`,
			setupBackend: func(b *mockREDSHIFTDATAIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAREDSHIFTDATAUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAREDSHIFTDATAUSER2",
			target:      "RedshiftData.ExecuteStatement",
			body:        `{"ClusterIdentifier":"cl","Sql":"SELECT 1"}`,
			setupBackend: func(b *mockREDSHIFTDATAIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAREDSHIFTDATAUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockREDSHIFTDATAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupREDSHIFTDATAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/redshift-data/aws4_request, SignedHeaders=host, Signature=mock",
			)
