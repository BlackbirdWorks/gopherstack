...
type mockREDSHIFTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockREDSHIFTIAMBackend() *mockREDSHIFTIAMBackend {
	return &mockREDSHIFTIAMBackend{
...
func (m *mockREDSHIFTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockREDSHIFTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockREDSHIFTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockREDSHIFTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupREDSHIFTEnforcementServer(t *testing.T, iamBackend *mockREDSHIFTIAMBackend) *httptest.Server {
	t.Helper()

	backend := redshift.NewInMemoryBackend("000000000000", "us-east-1")
	handler := redshift.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockREDSHIFTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupREDSHIFTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/redshift/aws4_request, SignedHeaders=host, Signature=mock",
			)
