...
func TestBackend_AuthorizeEndpointAccess(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(b *redshift.InMemoryBackend) {
				_, _ = b.CreateCluster("ea-cluster", "", "", "", nil, "", redshift.CreateClusterOptions{})
			},
		...
		},
		{
			...
			setup: func(b *redshift.InMemoryBackend) {
				_, _ = b.CreateCluster("ea-cluster-vpcs", "", "", "", nil, "", redshift.CreateClusterOptions{})
			},
		...
		},
		{
			name: "duplicate",
			setup: func(b *redshift.InMemoryBackend) {
				_, _ = b.CreateCluster("dup-ea-cluster", "", "", "", nil, "", redshift.CreateClusterOptions{})
...
func TestAuthorizeEndpointAccess_AllowedAllVPCsWhenEmpty(t *testing.T) {
	...
	_, err := b.CreateCluster("ea-cluster", "dc2.large", "dev", "admin", nil, "", redshift.CreateClusterOptions{})
	require.NoError(t, err)
...
func TestAuthorizeEndpointAccess_DuplicateReturnsError(t *testing.T) {
	...
	_, err := b.CreateCluster("ea-dup-cluster", "dc2.large", "dev", "admin", nil, "", redshift.CreateClusterOptions{})
	require.NoError(t, err)
