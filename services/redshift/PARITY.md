...
### 2026-09-11 pass: Redshift Serverless reservation-capacity family built (bd gopherstack-ztx0)

`ListReservationOfferings`/`ListReservations` were entirely unimplemented and
unrouted -- an SDK caller got a visible `ValidationException`/"unknown
operation" rather than a disguised empty 200, which is why gopherstack-ztx0
was filed P3 rather than as a wire bug. This pass implements the whole
reservation family and supersedes the 2026-08-13 deliberate-deferral entry
below (`bd gopherstack-v4wu`), whose core objection -- `ReservationOffering`
carries free-floating AWS commercial pricing with no SDK-enumerable catalog
-- is resolved rather than argued around: pricing fields are left at their
honest zero value with a citation trail, not derived or invented.

- **All five reservation ops** the pinned SDK defines are now implemented
  (`serverless_reservations.go`): `CreateReservation`, `GetReservation`,
  `GetReservationOffering`, `ListReservationOfferings`, `ListReservations`.
  Confirmed exhaustive by grepping `api_op_*.go` in
  `redshiftserverless@v1.38.5` for every op whose name contains
  "Reservation" -- no sixth op exists, so nothing was left out of this
  "small family" on a size assumption.
- **Wire shapes** verified against `types.Reservation`/
  `types.ReservationOffering` and their `awsAwsjson11_deserializeDocument*`
  functions in `deserializers.go`: `Reservation{startDate, endDate,
  offering, reservationArn, reservationId, status, capacity}` --
  `startDate`/`endDate` are `DateTime` strings
  (`smithytime.ParseDateTime`), genuinely different from this same
  package's `ScheduledActionResponse.startTime`/`endTime`, which are
  epoch-seconds numbers (re-verified per-field rather than assumed from the
  nearer-looking sibling, the same discipline this file's other entries
  already apply). `ReservationOffering{offeringId, offeringType,
  currencyCode, hourlyCharge, upfrontCharge, duration}`. `OfferingType`
  (`types/enums.go`) has exactly two real values, `ALL_UPFRONT`/
  `NO_UPFRONT` -- no "Partial Upfront" tier for serverless reservations,
  unlike classic Redshift's `ReservedNodeOffering`.
- **Declared error sets**, from each op's own
  `awsAwsjson11_deserializeOpError*` switch in `deserializers.go`:
  `CreateReservation` -- `ConflictException`/`InternalServerException`/
  `ResourceNotFoundException`/`ServiceQuotaExceededException`/
  `ThrottlingException`/`TooManyTagsException`/`ValidationException`;
  `Get`/`ListReservation*` -- `InternalServerException`/
  `ThrottlingException`/`ValidationException`, plus `ResourceNotFoundException`
  on the two `Get` ops. `TooManyTagsException` is declared on
  `CreateReservation` but `CreateReservationInput` has no `tags` member at
  all (confirmed against `api_op_CreateReservation.go`) -- left genuinely
  unreachable rather than inventing a tags parameter just to exercise it;
  this is an AWS-declared-but-dead error path on the real API too, not a
  gopherstack gap.
- **The seeded catalog**: `serverlessReservationOfferingsCatalog` in
  `serverless_reservations.go` has exactly 4 entries, one per (duration,
  `OfferingType`) combination AWS's own docs enumerate -- 1-year and 3-year
  terms (`aws.amazon.com/redshift/pricing/`: "1-year and 3-year options"
  for serverless reservations, fetched 2026-09-11) crossed with
  `ALL_UPFRONT`/`NO_UPFRONT`. `HourlyCharge`/`UpfrontCharge` are left at
  `0`, disclosed in the catalog's own doc comment: the pricing page states
  discounts as percentages off on-demand ("up to 45% for a 3-year term or
  up to 24% for a 1-year term"), and its own linked reserved-capacity guide
  (`docs.aws.amazon.com/redshift/latest/mgmt/serverless-billing-reserved.html`,
  same fetch) states a THIRD, inconsistent pair of percentages elsewhere on
  the same page ("up to 20 percent with the no-upfront option, or up to 24
  percent when you pay all-upfront") and never gives a concrete
  `$`/RPU-hour or upfront-dollar example anywhere. On-demand RPU-hour
  pricing itself varies by region, so multiplying a region's on-demand rate
  by one of these conflicting percentages would produce a
  specific-looking-but-unverifiable number -- exactly the invented-price
  failure mode the 2026-08-13 deferral warned about. Left at zero rather
  than guessed. `OfferingId` format is undocumented by AWS (no example
  shown anywhere in the CLI/API doc); used a deterministic slug per
  (duration, type) (e.g. `"capacity-1yr-all-upfront"`), the same descriptive-ID
  judgment call `reserved_nodes.go`'s `defaultReservedNodeOfferings`
  already made for the identical problem in classic Redshift.
- **Lifecycle**: `Reservation.Status` is seeded `"payment-pending"` -- the
  real initial value per `types.Reservation`'s own doc comment ("Possible
  values include the following: payment-pending, active, payment-failed,
  retired") -- and never advances. This backend has no payment gateway and
  no janitor/reconciler wired to reservations (unlike classic Redshift's
  own per-cluster `reconciler.go`, checked and confirmed not reusable here:
  it drives cluster status transitions, not a generic resource-lifecycle
  mechanism), so there is nothing to honestly drive the state forward with.
  This mirrors `PurchaseReservedNodeOffering`'s own identical, already-audited
  limitation for classic `ReservedNode.State`, which also never leaves its
  initial `"pending-payment"` value. Note the word order is *reversed*
  between the two fields -- classic Redshift's is state-then-reason
  (`"pending-payment"`), serverless's is reason-then-state
  (`"payment-pending"`) -- re-verified from the SDK source directly (see
  `TestPurchaseReservedNodeOffering_State`'s own citation of this exact
  trap) rather than copied from the sibling field by pattern-matching.
- **Persistence**: a new `store.Table[Reservation]` (`slReservations`) is
  registered/reset/persisted through the standard `store.Registry`
  mechanism, plus a `sortedStringIndex` for deterministic `ListReservations`
  order, matching every other serverless resource table in this package.
  `ReservationOffering` itself is NOT a `store.Table` -- it is a static
  compiled-in catalog with no create path, the same precedent this file's
  `ManagedWorkgroupListItem` entry already established for a similarly
  non-creatable list. No snapshot version bump: the `Tables` map addition is
  purely additive, confirmed by `pkgs/persistence`'s
  `TestSnapshotVersionGuard` (golden refreshed with `-update`, diff is
  13 added lines, 0 removed, redshift-only).
- **Tests**: `serverless_reservations_sdk_test.go` drives the real
  `aws-sdk-go-v2/service/redshiftserverless` typed client (not the raw-JSON
  handler helpers other serverless tests use) end to end: catalog contents
  and deterministic order, `GetReservationOffering` hit/miss,
  `CreateReservation` -> `GetReservation`/`ListReservations` consistency
  (dates derived from the offering's own duration, embedded `Offering`
  fully populated), `CreateReservation` against an unknown offering ID
  (real `ResourceNotFoundException` via `errors.As`), and pagination on
  both `List` ops. Proven to fail against the unfixed code: temporarily
  removed the five new `slDispatchTable` entries (file edit, reverted
  after) and confirmed every new test flips to `ValidationException`/
  "unknown operation".

Gates this pass: `go build ./...` (whole module), `go vet ./...`,
`go test -count=1 ./services/redshift/... ./pkgs/persistence/...`,
`golangci-lint run ./services/redshift/...` -- all clean (0 lint issues,
after an `x/tools` `fieldalignment -fix` pass on the new `Reservation`
struct).
...
- **Reservation-capacity family deliberately NOT built**
  ...
  **Revisited and built 2026-09-11 (bd gopherstack-ztx0)**: the tension this
  bullet raised -- a "curated" catalog needs a real bound, and
  `ReservationOffering`'s price fields have none -- was resolved not by
  finding a bound for the price, but by dropping price out of the curated
  part entirely: the catalog curates only `(duration, OfferingType)`, which
  *does* have a small, real, AWS-documented bound (see the 2026-09-11 pass
  section above), and `HourlyCharge`/`UpfrontCharge` are left at their
  honest zero rather than filled with a plausible-looking number. See that
...
- Redshift Serverless (`handler_serverless.go`): separate JSON-protocol API
  ...
  (`CreateReservation`/`GetReservation`/`GetReservationOffering`/
  `ListReservationOfferings`/`ListReservations`), deferred at that pass, is
  now FIXED 2026-09-11 (bd gopherstack-ztx0, see the family row's addendum)
  -- `ReservationOffering`'s `HourlyCharge`/`UpfrontCharge` are seeded at 0
  with a citation trail rather than derived from AWS-set commercial rates
  this backend cannot verify; no longer in `sdk_completeness_test.go`'s
...
actions, reserved nodes and reserved-node exchange, tags, endpoint access,
data shares, partners, redshift-idc and QEv2-idc applications, zero-ETL
integrations, authentication profiles, resource policy, custom domains and
misc account/cluster info (`DescribeAccountAttributes`, cluster
DB-revisions/tracks/versions, orderable options, storage, failover,
cluster credentials, `GetIdentityCenterAuthToken`). 88 previously-uncovered
classic-Redshift ops driven through the real
`aws-sdk-go-v2/service/redshift` client for the first time (62/198 -> 150/198
typed-covered). Every remaining uncovered op (48) is now Redshift
Serverless -- classic Redshift itself is fully typed-covered.

Redshift Serverless ops from this issue's raw uncovered list
(`CreateNamespace`/`CreateWorkgroup`/`GetCredentials`/... -- confirmed by
cross-referencing `Handler.GetSupportedOperations` vs.
`ServerlessHandler.GetSupportedOperations`, ~48 of the raw 136) belong to a
distinct real AWS service and SDK module (`redshiftserverless`) served by
this package's separate `ServerlessHandler`; opcensus's directory-based
census merges both handlers' op lists under one "redshift" bucket, which is
why they appeared in this issue's raw uncovered set. Not in this task's
named priority families (all of which describe classic-cluster Redshift)
and already covered by `serverless_*_test.go` — out of scope for this pass.
8 ops (`DeleteCustomDomainAssociation`/`DeleteEndpointAccess`/
`DeleteResourcePolicy`/`DeleteScheduledAction`/`DeleteUsageLimit`/
`GetIdentityCenterAuthToken`/`GetResourcePolicy`/`PutResourcePolicy`) are
real, distinct ops in BOTH APIs under the same name; covered here via the
classic `redshiftsdk` client.

TWO real wire bugs found, both only by asserting on typed-client-decoded
fields:

1. `CreateTags`/`DeleteTags` read `ResourceName` and passed it straight to
   the backend as a bare cluster identifier, but real `CreateTagsInput`/
   `DeleteTagsInput.ResourceName` is always a full ARN (e.g.
   `arn:aws:redshift:<region>:<account>:cluster:<id>` --
   redshift@v1.65.4 doc: "This member is required", always the resource
   ARN) -- `b.clusters.Get(fullARN)` never matched, so both ops failed for
   every real client with `ClusterNotFound`, confirmed live. `DescribeTags`
   already did the correct ARN-suffix match; `CreateTags`/`DeleteTags` did
   not. Fixed with a shared `redshiftClusterIDFromResourceName` helper
   mirroring `DescribeTags`' existing logic. `handler_tags.go`.
2. `GetReservedNodeExchangeConfigurationOptions` was a disguised stub: its
   response had no `GetReservedNodeExchangeConfigurationOptionsResult`
   wrapper element at all (just `XMLName`/`Xmlns`) -- a real client's XML
   deserializer failed outright (`failed to decode response body,
   ...Result node not found`, confirmed live) regardless of what state
   existed. Fixed with a real `ReservedNodeConfigurationOption` model (new
   `GetReservedNodeExchangeConfigurationOptions` backend method) pairing
   every account-owned reserved node against this backend's existing
   static offering catalog (`defaultReservedNodeOfferings`, the same
   source `GetReservedNodeExchangeOfferings` already uses), wrapped in the
   real result element. `reserved_nodes.go`, `handler_reserved_nodes.go`,
   `interfaces.go`. Scoping gap (ClusterIdentifier/SnapshotIdentifier
   accepted but not filtered on) recorded in `items_still_open` above.

`CancelResize` was NOT a bug: this backend applies `ResizeCluster`
synchronously (`AllowCancelResize: false` is set the moment the resize
"completes"), so a real client's `CancelResize` immediately after
`ResizeCluster` correctly receives `InvalidClusterStateFault` -- the test
asserts that decoded error shape rather than expecting success.

No persisted-type field was added or changed (the new
`ReservedNodeConfigurationOption` type is computed on demand from existing
`reservedNodes`/`defaultReservedNodeOfferings` state, never stored), so
`pkgs/persistence`'s `TestSnapshotVersionGuard` needed no `-update` for
redshift; confirmed by its golden diff touching only glue rows this pass.
...
Gates: `go build ./...` clean, `go vet ./services/redshift/...` clean, `go
