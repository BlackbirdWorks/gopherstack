...
func TestReconciler_BackgroundAdvancesState(t *testing.T) {
	...
	_, err := b.CreateCluster("bg", "dc2.large", "dev", "admin", nil, "", redshift.CreateClusterOptions{})
	require.NoError(t, err)
...
func TestReconciler_DeleteCancelsPendingCreate(t *testing.T) {
	...
	_, err := b.CreateCluster("pending", "dc2.large", "dev", "admin", nil, "", redshift.CreateClusterOptions{})
	require.NoError(t, err)
