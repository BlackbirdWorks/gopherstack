...
// TestRealClient_SubscriptionAndSMS drives sns's typed-coverage-blind ops
// (gopherstack-n3zi) through the real aws-sdk-go-v2 client.
func TestRealClient_SubscriptionAndSMS(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "sms attributes and sandbox",
			run: func(t *testing.T) {
				t.Helper()
