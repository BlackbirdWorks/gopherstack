...
// services/secretsmanager and services/cloudwatchlogs use. GlobalClusters
// were already flat/partition-scoped (not region-nested), so it became a
// plain (non-composite-key) *store.Table[GlobalCluster]. It otherwise follows
// pkgs/store's package doc and the services/ec2 (commit 12e611a4, data-driven
// registration slice) / services/rds (commit 4179a2fc) conversions.
//
// clusterRoles (map[string]map[string][]string) and tags
// (map[string]map[string][]Tag) are deliberately NOT converted: store.Table
// requires a *V value with its own identity, but these hold a bare []string
// and a bare []Tag respectively, neither of which carries a key of its own.
// They remain plain nested maps, unchanged by this refactor (see the
// *Store helpers at the bottom of backend.go).
import (
...
func parameterGroupKeyFn(v *DBParameterGroup) string {
	return regionKey(v.region, v.DBParameterGroupName)
}
func parameterGroupRegionIndexKeyFn(v *DBParameterGroup) string { return v.region }

func clusterEndpointKeyFn(v *DBClusterEndpoint) string {
	return regionKey(v.region, v.DBClusterEndpointIdentifier)
}
func clusterEndpointRegionIndexKeyFn(v *DBClusterEndpoint) string { return v.region }

func eventSubscriptionKeyFn(v *EventSubscription) string {
	return regionKey(v.region, v.CustSubscriptionID)
}
...
// needed here (unlike the eight tables above).
...
// call (and, for the eight region-qualified tables, its companion
...
var tableRegistrations = []func(*InMemoryBackend){
	...
	func(b *InMemoryBackend) {
		b.parameterGroups = store.Register(b.registry, "parameterGroups", store.New(parameterGroupKeyFn))
		b.parameterGroupsByRegion = b.parameterGroups.AddIndex("byRegion", parameterGroupRegionIndexKeyFn)
	},
	func(b *InMemoryBackend) {
		b.clusterEndpoints = store.Register(b.registry, "clusterEndpoints", store.New(clusterEndpointKeyFn))
		b.clusterEndpointsByRegion = b.clusterEndpoints.AddIndex("byRegion", clusterEndpointRegionIndexKeyFn)
	},
	func(b *InMemoryBackend) {
