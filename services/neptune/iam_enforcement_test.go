...
type mockNEPTUNEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockNEPTUNEIAMBackend() *mockNEPTUNEIAMBackend {
	return &mockNEPTUNEIAMBackend{
...
func (m *mockNEPTUNEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockNEPTUNEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockNEPTUNEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockNEPTUNEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupNEPTUNEEnforcementServer(t *testing.T, iamBackend *mockNEPTUNEIAMBackend) *httptest.Server {
	t.Helper()

	backend := neptune.NewInMemoryBackend("000000000000", "us-east-1")
	handler := neptune.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockNEPTUNEIAMBackend)
		name          string
		accessKeyID   string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIANEPTUNEUSER2",
			body:        "Action=CreateDBCluster&DBClusterIdentifier=test-neptune&Engine=neptune&Version=2014-10-31",
			setupBackend: func(b *mockNEPTUNEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIANEPTUNEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockNEPTUNEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupNEPTUNEEnforcementServer(t, iamBackend)
			ctx := t.Context()
