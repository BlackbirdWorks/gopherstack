...
overall: A            # every previously-open gap this pass either genuinely fixed or re-verified as correct-as-is
                      ...
                      # neptuneClusterParameterCatalog/neptuneInstanceParameterCatalog (parameter_catalog.go)
                      # now transcribe AWS's actually-documented catalog verbatim from
                      # https://docs.aws.amazon.com/neptune/latest/userguide/parameters.html (fetched
                      # 2026-09-11): 13 cluster-level parameters (12 live + the still-documented deprecated
                      # neptune_enforce_ssl) and 4 instance-level parameters, as two genuinely separate
                      # catalogs instead of one 8-parameter list shared by both scopes (a real bug: e.g.
                      # neptune_streams is cluster-only and neptune_result_cache/UndoLogPurgeConfig are
                      # instance-only in real Neptune, but the old shared catalog let either scope modify
                      # the other's parameters). Also fixed two real validation gaps found while rebuilding
                      # the catalog: neptune_query_timeout and neptune_result_cache were modeled as
                      # ApplyType=dynamic, but parameters.html states both are static ("This parameter is
                      # static" under each one's own heading, and parameter-groups.html's blanket rule that
                      # only neptune_enable_slow_query_log/neptune_slow_query_log_threshold are the dynamic
                      # exceptions) -- ApplyMethod=immediate on either now correctly rejects. And
                      # Modify(DBCluster)ParameterGroup never checked a value against its parameter's
                      # documented AllowedValues at all (only name/modifiability), so e.g.
                      # neptune_result_cache=5 was silently stored -- validateParameterValue now enforces
                      # both enum (comma-list) and numeric-range ("min-max") AllowedValues forms, still
                      # under the existing InvalidParameterValue code (ModifyDBParameterGroup/
                      # ModifyDBClusterParameterGroup's declared error sets -- deserializers.go:6239/5849 --
                      # only name DBParameterGroupNotFound and InvalidDBParameterGroupState as typed faults,
                      # so a bad name or bad value both fall through to the generic-error path, matching the
                      # pre-existing convention). Parameter group families verified against
                      # https://docs.aws.amazon.com/neptune/latest/userguide/parameter-groups.html: added
                      # pgFamilyNeptune1 ("neptune1" -- for engine versions prior to 1.2.0.0) alongside the
                      # existing neptune1.2/1.3/1.4, though this backend's static engine-version catalog
                      # never models a version below 1.2.0.0 so no modeled cluster/instance defaults to it.
                      # Disclosed, not modeled: neptune_lookup_cache's documented default-flip to "1" when
                      # an R5d instance joins the cluster (this backend's override store isn't keyed by
                      # instance class). (2) GlobalCluster Failover/Switchover to an unresolvable
                      # TargetDbClusterIdentifier was a silent no-op (and, if resolvable to some real but
                      # unattached cluster, silently attached it as a surprise new writer) -- both
                      # operations' declared error sets are identical (DBClusterNotFoundFault,
                      # GlobalClusterNotFoundFault, InvalidDBClusterStateFault,
                      # InvalidGlobalClusterStateFault; verified against
                      # awsAwsquery_deserializeOpErrorFailoverGlobalCluster/...Switchover...,
                      # deserializers.go:5338/8147) and TargetDbClusterIdentifier's own doc comment names it
                      # "the secondary Neptune DB cluster" to promote. promoteGlobalClusterWriter now
                      # rejects: a target resolving to no DB cluster this backend tracks at all ->
                      # DBClusterNotFoundFault; a real cluster that is not a member of the named global
                      # cluster, or is already its writer -> InvalidDBClusterStateFault. The "no join-global-
                      # cluster op" limitation the original follow-up cited was already fixed in the
                      # 2026-08-29 pass below (CreateDBCluster's GlobalClusterIdentifier member registers
                      # real membership), so this pass only needed to make Failover/Switchover actually
                      # enforce membership against that already-real state rather than also building the
                      # join path. See parameter_catalog_test.go
                      # (TestParameterValueStore_DisallowedValueRejected,
                      # TestParameterValueStore_CrossScopeParameterRejected,
                      # TestDescribeEngineDefaultParameters_ReturnsCatalog),
                      # handler_global_clusters_test.go
                      # (TestGlobalCluster_FailoverGlobalCluster_UnknownTargetRejected,
                      # TestGlobalCluster_FailoverGlobalCluster_AlreadyPrimaryRejected), and the new
                      # handler_parameter_catalog_sdk_test.go / handler_global_cluster_failover_sdk_test.go
                      # (real aws-sdk-go-v2 client round-trips, including typed-fault assertions via
                      # errors.As(*types.DBClusterNotFoundFault)/(*types.InvalidDBClusterStateFault)).
...
### 2026-09-24 lakeformation-appsync-neptune-and-athena terraform coverage

DescribeDBClusterEndpoints wrongly returned a typed-but-undeclared NotFoundFault for an unmatched endpoint identifier, hanging every `aws_neptune_cluster_endpoint` destroy in terraform-provider-aws's own delete-timeout wait loop. Fixed to return an empty list; see the ClusterEndpoint ops note above. Also confirmed real (not a bug): DeleteDBInstance's "can't delete the only instance in a cluster" rule is genuine, documented AWS behavior -- a fixture that needs to destroy cleanly should give a cluster more than one instance, or none at all.
...
time: static parameters require `ApplyMethod=pending-reboot`
(`ApplyMethod=immediate` is rejected, matching real AWS's
static/dynamic-ApplyMethod compatibility rule), and a caller-supplied value
outside a parameter's documented `AllowedValues` is rejected rather than
silently stored.

**2026-09-11 (gopherstack-jd33): the catalog was rebuilt from AWS's actual
documented parameter list, split into two genuinely independent catalogs.**
The original catalog (this file's earlier passes) was an explicitly-disclosed
"representative approximation" -- 8 parameters, shared verbatim between
DBParameterGroup (instance-level) and DBClusterParameterGroup
(cluster-level), because nobody had gone and read AWS's parameter
documentation yet. It turns out AWS *does* document the full catalog
(https://docs.aws.amazon.com/neptune/latest/userguide/parameters.html), just
not as an SDK-side enum -- `DescribeEngineDefaultParameters`'s response shape
is generic (`Parameter` structs), so the catalog contents live only in this
prose page, not in `types.go`. Transcribing it verbatim: 13 cluster-level
parameters (`neptuneClusterParameterCatalog`) --
`neptune_enable_audit_log`, `neptune_enable_slow_query_log`,
`neptune_slow_query_log_threshold`, `neptune_lab_mode`, `neptune_query_timeout`,
`neptune_streams`, `neptune_streams_expiry_days`, `neptune_lookup_cache`,
`neptune_autoscaling_config`, `neptune_ml_iam_role`, `neptune_ml_endpoint`,
`neptune_enable_inline_server_generated_edge_id`, and the still-documented
deprecated `neptune_enforce_ssl` -- and 4 instance-level parameters
(`neptuneInstanceParameterCatalog`) -- `neptune_dfe_query_engine`,
`neptune_query_timeout`, `neptune_result_cache`, `UndoLogPurgeConfig`. Real
Neptune's two catalogs are NOT the same set (`neptune_streams` is cluster-only,
`neptune_result_cache`/`UndoLogPurgeConfig` are instance-only), so the old
shared-8-parameter design was itself a bug independent of coverage: a DB
parameter group could accept `neptune_streams` and a DB cluster parameter
group could accept `neptune_result_cache`, neither of which real AWS permits
at that scope. Rebuilding surfaced two more concrete wire-shape bugs beyond
missing coverage: `neptune_query_timeout` and `neptune_result_cache` were
modeled `ApplyType=dynamic`, but the doc states both are static (and
`parameter-groups.html` names only `neptune_enable_slow_query_log`/
`neptune_slow_query_log_threshold` as the dynamic exceptions among cluster
parameters, with ALL instance-level parameters unconditionally static); and
`neptune_result_cache`'s allowed values were invented as `ENABLED`/`DISABLED`
rather than the documented `0`/`1`. Parameter-group family names
(`neptune1`/`neptune1.2`/`neptune1.3`/`neptune1.4`, one per engine-version
line) were verified against `parameter-groups.html`; `pgFamilyNeptune1` was
added for completeness even though no engine version this backend's static
catalog models falls below 1.2.0.0. Disclosed, not modeled: `neptune_lookup_cache`
defaults to `1` instead of `0` whenever an R5d instance is created in the
cluster (the override store isn't keyed by instance class);
`neptune_slow_query_log_threshold` has no documented `AllowedValues` range
(only a default), so none is enforced.
...
`GlobalClusterIdentifier` parameter (`api_op_CreateDBCluster.go:129`); this
backend has modeled that join path (and its reciprocal `GlobalClusterMembers`
bookkeeping) since the 2026-08-29 pass. **2026-09-11 (gopherstack-jd33):**
Failover/Switchover themselves, however, still trusted an unverified
`TargetDbClusterIdentifier` -- an unresolvable target was a silent no-op, and
a real-but-unattached cluster was silently attached as a surprise new writer.
Both operations declare the identical error set (`DBClusterNotFoundFault`,
`GlobalClusterNotFoundFault`, `InvalidDBClusterStateFault`,
`InvalidGlobalClusterStateFault` -- `deserializers.go:5338`/`8147`), and
`TargetDbClusterIdentifier`'s doc comment names it "the secondary Neptune DB
cluster" to promote, i.e. it must already be a member. `promoteGlobalClusterWriter`
now rejects a target resolving to no tracked DB cluster at all with
`DBClusterNotFoundFault`, and a real cluster that either isn't a member of
the named global cluster or is already its writer with
...
## 2026-09-17 (bd gopherstack-xhu2t, reqfielddiff tier-1 sweep)

reqfielddiff tier-1 26 -> 4. Fixed, typed-client-proven: DBInstance
BackupRetentionPeriod/VpcSecurityGroups (cluster-inherited, like
NetworkType)/DBSecurityGroups/MonitoringInterval+MonitoringRoleArn/Iops/
DBPortNumber; ModifyDBCluster DBInstanceParameterGroupName (propagates to
member instances) and ApplyImmediately; DeleteDBInstance
SkipFinalSnapshot conflict validation; RestoreDBClusterToPointInTime
RestoreToTime/UseLatestRestorableTime exclusivity; CopyDBClusterSnapshot.
CopyTags; DescribeDBEngineVersions.DefaultOnly; Marker/MaxRecords wired on
8 Describe ops. Recorded: DescribeDBClusterSnapshots IncludePublic/
