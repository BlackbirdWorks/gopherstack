...
// TestHandler_OversizedBodySurfacesInternalFailure drives a real Neptune
...
// "api/neptune") -- verified against the pinned neptune@v1.48.4
// api_client.go's AddSDKAgentKeyValue call -- before ever reading the body,
// so a read failure past that point still belongs to this service; the
// matcher now claims it instead of 404ing. ExtractOperation/ExtractResource/
// Handler() were also migrated off r.ParseForm() onto httputils.ReadBody +
// url.ParseQuery: net/http's ParseForm caches an empty-but-non-nil
// r.PostForm after its first failed call, and the telemetry wrapper's
// ExtractOperation runs before Handler(), so Handler()'s own ParseForm call
// used to silently "succeed" empty on the second call, producing
// MissingAction instead of InternalFailure. httputils.ReadBody caches a read
// failure instead, so every call sees the same real error.
func TestHandler_OversizedBodySurfacesInternalFailure(t *testing.T) {
	...
	_, err := client.DescribeDBClusters(t.Context(), &neptunesdk.DescribeDBClustersInput{
		Marker: huge,
	}, func(o *neptunesdk.Options) { o.RetryMaxAttempts = 1 })
	require.Error(t, err)
