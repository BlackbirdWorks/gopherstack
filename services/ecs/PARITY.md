...
gaps:
  ...
  - "awslogs LogConfiguration (gopherstack-sv5q): PARTIAL fix. Added ecs.CWLogsBackend (EnsureLogGroupAndStream/PutLogLines, same shape as lambda.CWLogsBackend/firehose.CWLogsBackend) plus SetCWLogsBackend, wired in cli.go's wireEcsCWLogs. RunTask/StartTask now call EnsureLogGroupAndStream for every awslogs-driver ContainerDefinition, so awslogs-group/awslogs-stream-prefix become a real, discoverable CloudWatch Logs group/stream instead of only stored and echoed. NOT DONE: no container output is forwarded -- docker_runner.go's dockerClient interface still has no ContainerLogs method (confirmed absent from services/ecs/ at time of fix), so there is no source of container stdout/stderr to push via PutLogLines at all. Stream naming: with awslogs-stream-prefix set, follows the SDK's documented format verbatim (aws-sdk-go-v2 service/ecs@v1.90.0 types/types.go, LogConfiguration doc for awslogs-stream-prefix: 'the log stream takes the format prefix-name/container-name/ecs-task-id'); with no prefix, real ECS names the stream after the Docker-assigned container ID, which this backend does not have at this layer -- approximated with the task ID alone (own choice, not SDK-pinned)."
deferred:
