...
type mockMWAAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMWAAIAMBackend() *mockMWAAIAMBackend {
	return &mockMWAAIAMBackend{
...
func (m *mockMWAAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMWAAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMWAAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMWAAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMWAAEnforcementServer(t *testing.T, iamBackend *mockMWAAIAMBackend) *httptest.Server {
	t.Helper()

	backend := mwaa.NewInMemoryBackend("us-east-1", "000000000000")
	handler := mwaa.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAMWAAUSER1",
			method:      "GET",
			path:        "/environments",
			body:        ``,
			setupBackend: func(b *mockMWAAIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAMWAAUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMWAAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMWAAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/airflow/aws4_request, SignedHeaders=host, Signature=mock",
			)
