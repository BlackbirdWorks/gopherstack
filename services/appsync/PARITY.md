...
ops:
  ...
  GetIntrospectionSchema: {wire: ok, errors: ok, state: ok, persist: ok, note: "2026-09-04: format (SDL/JSON, a required input) and includeDirectives are accepted but not honored -- always returns raw SDL text regardless of requested format. Same structural gap as ListTypes/GetType/ListTypesByAssociation's format parameter (no SDL<->JSON introspection converter exists); previously undisclosed for this op specifically. Disclosed in code, not fixed."}
