...
type mockAPPSYNCIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockAPPSYNCIAMBackend() *mockAPPSYNCIAMBackend {
	return &mockAPPSYNCIAMBackend{
...
func (m *mockAPPSYNCIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockAPPSYNCIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockAPPSYNCIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockAPPSYNCIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupAPPSYNCEnforcementServer(t *testing.T, iamBackend *mockAPPSYNCIAMBackend) *httptest.Server {
	t.Helper()

	backend := appsync.NewInMemoryBackend("000000000000", "us-east-1", "")
	handler := appsync.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockAPPSYNCIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAAPPSYNCUSER1",
			method:      "GET",
			path:        "/v1/apis",
			body:        ``,
			setupBackend: func(b *mockAPPSYNCIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAAPPSYNCUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAAPPSYNCUSER2",
			method:      "POST",
			path:        "/v1/apis",
			body:        `{"name":"test-api","authenticationType":"API_KEY"}`,
			setupBackend: func(b *mockAPPSYNCIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAAPPSYNCUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockAPPSYNCIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupAPPSYNCEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/appsync/aws4_request, SignedHeaders=host, Signature=mock",
			)
