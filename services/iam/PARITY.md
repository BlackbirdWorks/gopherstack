...
overall: A   # sweep 13 (wrapper-key sweep, uncommitted as of this note): fixed
  ...
  # but left open (needs a StorageBackend surface change). See items_still_open and
  # the ops: entries for both for detail.
...
ops:
  ...
  ListEntitiesForPolicy: {wire: broken, errors: ok, state: ok, persist: n/a, note: "NOT FIXED (sweep 13, confirmed present, deliberately left open). Real ListEntitiesForPolicyInput (api_op_ListEntitiesForPolicy.go) declares PolicyArn, EntityFilter, PathPrefix, PolicyUsageFilter, Marker, MaxItems; the handler (handler_policies.go's iamEntitiesForPolicyDispatch) reads only PolicyArn and EntityFilter -- PathPrefix and PolicyUsageFilter are parsed nowhere, and IsTruncated is hardcoded false with no Marker/MaxItems pagination at all. Same dropped-parameter shape as the ListAttached* fix above but not fixed this sweep: PathPrefix here filters each returned ENTITY's own path (not the policy's), which needs a GetUser/GetGroup/GetRole lookup per entity (ListEntitiesForPolicy's backend currently returns bare name lists, no path data), and PolicyUsageFilter needs a per-entity boundary-vs-permissions-policy determination the backend does not currently expose -- both need a StorageBackend surface change, not just a handler-layer fix like ListAttached*'s GetPolicy lookup. Left as a named, disclosed gap for a future sweep rather than fabricated."}
...
items_still_open:
  - "2026-08-29 constraint-parameter sweep fixed PathPrefix+pagination truncation across ListUsers/ListRoles/ListGroups/ListInstanceProfiles/ListPolicies, and ListPolicies' OnlyAttached/PolicyUsageFilter (see the sweep's own section above for detail). Sweep 13 closed ListAttached{User,Role,Group}Policies' PathPrefix (see its own ops: entry) -- still open: ListEntitiesForPolicy's EntityFilter/PathPrefix/PolicyUsageFilter (confirmed present sweep 13, needs a StorageBackend surface change -- see its own ops: entry), and the pagination-only params on ListMFADevices/ListAccessKeys/ListSigningCertificates/ListSSHPublicKeys/ListServiceSpecificCredentials (not re-checked sweep 13)."
