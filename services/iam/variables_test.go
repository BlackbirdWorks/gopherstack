...
func TestSubstituteVariables(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "principaltag_absent_blank",
			doc:  `{"Resource":"${aws:principaltag/team}"}`,
			ctx:  iam.ConditionContext{},
			want: `{"Resource":""}`,
		},
		{
			name: "principaltag_resolved",
			doc:  `{"Resource":"arn:aws:s3:::${aws:PrincipalTag/team}-bucket/*"}`,
			ctx:  iam.ConditionContext{PrincipalTags: map[string]string{"team": "blue"}},
			want: `{"Resource":"arn:aws:s3:::blue-bucket/*"}`,
		},
		{
			name: "principaltag_case_insensitive_key",
			doc:  `{"Resource":"${aws:PrincipalTag/Team}"}`,
			ctx:  iam.ConditionContext{PrincipalTags: map[string]string{"Team": "green"}},
			want: `{"Resource":"green"}`,
		},
		{
			name: "requesttag_resolved",
			doc:  `{"Resource":"${aws:RequestTag/env}"}`,
			ctx:  iam.ConditionContext{RequestTags: map[string]string{"env": "prod"}},
			want: `{"Resource":"prod"}`,
		},
		{
			name: "requesttag_absent_blank",
			doc:  `{"Resource":"x-${aws:RequestTag/env}-y"}`,
			ctx:  iam.ConditionContext{},
			want: `{"Resource":"x--y"}`,
		},
		{
			name: "resourcetag_recognised_blank",
			doc:  `{"Resource":"${aws:ResourceTag/owner}"}`,
			ctx:  iam.ConditionContext{},
			want: `{"Resource":""}`,
		},
		{
