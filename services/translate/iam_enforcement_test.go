...
type mockTRANSLATEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockTRANSLATEIAMBackend() *mockTRANSLATEIAMBackend {
	return &mockTRANSLATEIAMBackend{
...
func (m *mockTRANSLATEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockTRANSLATEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockTRANSLATEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockTRANSLATEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupTRANSLATEEnforcementServer(t *testing.T, iamBackend *mockTRANSLATEIAMBackend) *httptest.Server {
	t.Helper()

	backend := translate.NewInMemoryBackend("000000000000", "us-east-1")
	handler := translate.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockTRANSLATEIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIATRANSLATEUSER1",
			target:      "AWSShineFrontendService_20170701.TranslateText",
			body:        `{"Text":"hello","SourceLanguageCode":"en","TargetLanguageCode":"es"}`,
			setupBackend: func(b *mockTRANSLATEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIATRANSLATEUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockTRANSLATEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupTRANSLATEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/translate/aws4_request, SignedHeaders=host, Signature=mock",
			)
