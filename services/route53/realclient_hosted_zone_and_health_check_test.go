...
// TestRealClient_HostedZoneAndHealthCheck drives route53's typed-coverage-
// blind ops (gopherstack-n3zi) through the real aws-sdk-go-v2 client.
func TestRealClient_HostedZoneAndHealthCheck(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "hosted zone count and comment",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "health check status and last failure reason",
			run: func(t *testing.T) {
				t.Helper()
				...
				reasonOut, err := client.GetHealthCheckLastFailureReason(
					ctx,
					&route53sdk.GetHealthCheckLastFailureReasonInput{
						HealthCheckId: aws.String(hcID),
					},
				)
				require.NoError(t, err)
		...
		{
			name: "checker ip ranges health check count delegation set limit",
			run: func(t *testing.T) {
				t.Helper()
				...
				limitOut, err := client.GetReusableDelegationSetLimit(
					ctx,
					&route53sdk.GetReusableDelegationSetLimitInput{
