...
// TestInMemoryBackend_StartDocumentTextDetectionWithOptions_TrimmedBeforeReadback covers gopherstack-0ho6.
func TestInMemoryBackend_StartDocumentTextDetectionWithOptions_TrimmedBeforeReadback(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			job, err := b.StartDocumentTextDetectionWithOptions(
				context.Background(), "s3://bucket/doc.png", nil, nil, "", "",
			)
