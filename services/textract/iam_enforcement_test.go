...
type mockTEXTRACTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockTEXTRACTIAMBackend() *mockTEXTRACTIAMBackend {
	return &mockTEXTRACTIAMBackend{
...
func (m *mockTEXTRACTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockTEXTRACTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockTEXTRACTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockTEXTRACTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupTEXTRACTEnforcementServer(t *testing.T, iamBackend *mockTEXTRACTIAMBackend) *httptest.Server {
	t.Helper()

	backend := textract.NewInMemoryBackend("000000000000", "us-east-1")
	handler := textract.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockTEXTRACTIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIATEXTRACTUSER1",
			target:      "Textract.DetectDocumentText",
			body:        `{"Document":{"Bytes":"e30="}}`,
			setupBackend: func(b *mockTEXTRACTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIATEXTRACTUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIATEXTRACTUSER2",
			target:      "Textract.AnalyzeDocument",
			body:        `{"Document":{"Bytes":"e30="},"FeatureTypes":["TABLES"]}`,
			setupBackend: func(b *mockTEXTRACTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIATEXTRACTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockTEXTRACTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupTEXTRACTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/textract/aws4_request, SignedHeaders=host, Signature=mock",
			)
