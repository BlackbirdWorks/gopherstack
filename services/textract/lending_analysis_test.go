...
// TestInMemoryBackend_StartLendingAnalysisWithOptions_TrimmedBeforeReadback covers gopherstack-0ho6.
func TestInMemoryBackend_StartLendingAnalysisWithOptions_TrimmedBeforeReadback(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			job, err := b.StartLendingAnalysisWithOptions(
				context.Background(), "s3://bucket/loan.pdf", nil, nil, "", "",
			)
