...
// TestRealClient_AdaptersAndAnalysis drives textract's remaining typed-
...
func TestRealClient_AdaptersAndAnalysis(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				adapterArn := "arn:aws:textract:" + tagsRTRegion + ":" + tagsRTAccountID + ":adapter/" + aws.ToString(
					adapterID,
				)
		...
		{
			name: "AnalyzeExpense and AnalyzeID synchronous",
			run: func(t *testing.T) {
				t.Helper()
					...
					DocumentPages: []textracttypes.Document{
						{
							S3Object: &textracttypes.S3Object{
								Bucket: aws.String("bucket"),
								Name:   aws.String("id-front.png"),
							},
						},
						{
							S3Object: &textracttypes.S3Object{
								Bucket: aws.String("bucket"),
								Name:   aws.String("id-back.png"),
							},
						},
					},
				})
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				getLendingSummaryOut, err := client.GetLendingAnalysisSummary(
					ctx,
					&textractsdk.GetLendingAnalysisSummaryInput{
						JobId: lendingJobOut.JobId,
					},
				)
				require.NoError(t, err)
