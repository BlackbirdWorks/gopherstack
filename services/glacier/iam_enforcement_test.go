...
type mockGLACIERIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockGLACIERIAMBackend() *mockGLACIERIAMBackend {
	return &mockGLACIERIAMBackend{
...
func (m *mockGLACIERIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockGLACIERIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockGLACIERIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockGLACIERIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupGLACIEREnforcementServer(t *testing.T, iamBackend *mockGLACIERIAMBackend) *httptest.Server {
	t.Helper()

	backend := glacier.NewInMemoryBackend()
	handler := glacier.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockGLACIERIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAGLACIERUSER1",
			method:      "GET",
			path:        "/000000000000/vaults",
			body:        ``,
			setupBackend: func(b *mockGLACIERIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAGLACIERUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAGLACIERUSER2",
			method:      "PUT",
			path:        "/000000000000/vaults/test-vault",
			body:        ``,
			setupBackend: func(b *mockGLACIERIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAGLACIERUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockGLACIERIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupGLACIEREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/glacier/aws4_request, SignedHeaders=host, Signature=mock",
			)
