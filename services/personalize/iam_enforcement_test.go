...
type mockPERSONALIZEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockPERSONALIZEIAMBackend() *mockPERSONALIZEIAMBackend {
	return &mockPERSONALIZEIAMBackend{
...
func (m *mockPERSONALIZEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockPERSONALIZEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockPERSONALIZEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockPERSONALIZEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupPERSONALIZEEnforcementServer(t *testing.T, iamBackend *mockPERSONALIZEIAMBackend) *httptest.Server {
	t.Helper()

	backend := personalize.NewInMemoryBackend("000000000000", "us-east-1")
	handler := personalize.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAPERSONALIZEUSER1",
			target:      "AmazonPersonalize.ListDatasetGroups",
			body:        `{}`,
			setupBackend: func(b *mockPERSONALIZEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAPERSONALIZEUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockPERSONALIZEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupPERSONALIZEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/personalize/aws4_request, SignedHeaders=host, Signature=mock",
			)
