...
// TestRealClient_QueryAndScheduledQuery drives timestreamquery's typed-
// coverage-blind ops (gopherstack-n3zi) through the real aws-sdk-go-v2
// client.
func TestRealClient_QueryAndScheduledQuery(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "scheduled query update and tags",
			run: func(t *testing.T) {
				t.Helper()
