...
ops:
  ...
  CreateScheduledQuery: {wire: fixed, errors: ok, state: fixed, persist: ok, note: "ClientToken was parsed by the SDK request but never read by the handler/backend, so an SDK-auto-retried create (aws-sdk-go-v2 auto-generates ClientToken via idempotency-token-autofill middleware) hit ConflictException instead of replaying the original success. Now caches ClientToken->Arn for 8h and replays. Also: KmsKeyId (types.go:659) was accepted nowhere on the request and returned nowhere on ScheduledQueryDescription (types.go:659, same field name) -- a silent-drop, not just missing encryption. Now parsed, stored, and echoed back on DescribeScheduledQuery (still no actual at-rest encryption -- see gaps). 2026-08-20: TargetConfiguration.TimestreamConfiguration.DimensionMappings/TimeColumn (both required subfields of types.TimestreamConfiguration whenever TargetConfiguration is set, api_op_CreateScheduledQuery.go) plus MeasureNameColumn/MixedMeasureMappings/MultiMeasureMappings (optional) were accepted nowhere on create and echoed nowhere on describe -- confirmed with a failing real-SDK round-trip test before the fix. Now modeled end-to-end via ScheduledQueryTargetDetail, parsed in parseTargetConfiguration, and echoed in targetConfigurationView. 2026-08-23: closed the previously-disclosed ErrorReportConfiguration.S3Configuration.EncryptionOption/ObjectKeyPrefix gap (both real, optional members of types.S3Configuration) -- only BucketName was parsed/stored/echoed; now both are threaded through the same accepted-and-echoed pattern as KmsKeyId (still no actual encryption applied, honestly scoped, see gaps)."}
...
gaps:
  ...
  - "FIXED 2026-08-23: ErrorReportConfiguration.S3Configuration.EncryptionOption (types.S3EncryptionOption: SSE_S3|SSE_KMS) and ObjectKeyPrefix, both optional members of the real S3Configuration (api_op_CreateScheduledQuery.go), are now parsed on CreateScheduledQuery and echoed on DescribeScheduledQuery (createScheduledQueryInput/ScheduledQuery/scheduledQueryToView). This emulator still applies no actual S3 encryption or key-prefixing -- 'we do not encrypt', not 'we lose the setting', same honest scoping as KmsKeyId. Proven via Test_SDKRoundTrip_ScheduledQuery_ErrorReportS3Configuration (wire_scheduledquery_errorreport_test.go), hand-reverted to confirm it fails with both fields empty pre-fix."
deferred:
...
Followed up on the 2026-08-20 pass's own disclosed-not-fixed gap: `S3Configuration.
EncryptionOption`/`ObjectKeyPrefix` (both real, optional members of
`types.S3Configuration`, `api_op_CreateScheduledQuery.go`, confirmed against
`aws-sdk-go-v2/service/timestreamquery@v1.39.4/types/types.go:538-554`) were accepted
nowhere on `CreateScheduledQuery` and echoed nowhere on `DescribeScheduledQuery` -- only
`BucketName` was ever parsed. Same silent-drop class as the already-fixed `KmsKeyId`.
Fixed by threading `errorReportEncryptionOption`/`errorReportObjectKeyPrefix` through
`createScheduledQueryInput` (`handler_scheduled_queries.go`), two new
`ScheduledQuery.ErrorReportEncryptionOption`/`ErrorReportObjectKeyPrefix` fields
(`models.go`), two new trailing `StorageBackend.CreateScheduledQuery` parameters
(`interfaces.go`, `scheduled_queries.go`), and `scheduledQueryToView`'s
`ErrorReportConfiguration.S3Configuration` echo. `EncryptionOption` is passed through
as an opaque string (no gopherstack-side enum constant to diverge, same treatment as
every other pass-through enum in this file) -- neither field is required by the real
SDK's client-side `validateS3Configuration` (only `BucketName` is), so no new
`ValidationException` branch was added. This emulator still performs no actual S3
encryption or key-prefixing -- honestly scoped ("we do not encrypt", not "we lose the
setting"), identical framing to `KmsKeyId`'s existing gap entry.

Proven via a real `aws-sdk-go-v2/service/timestreamquery` client round trip
(`wire_scheduledquery_errorreport_test.go`,
`Test_SDKRoundTrip_ScheduledQuery_ErrorReportS3Configuration`): `CreateScheduledQuery`
with `EncryptionOption: SSE_KMS`/`ObjectKeyPrefix: "errors/tsq/"` followed by
`DescribeScheduledQuery`, asserting both round-trip. Hand-reverted (`models.go`,
`interfaces.go`, `scheduled_queries.go`, `handler_scheduled_queries.go`, plus the three
test files whose `CreateScheduledQuery` call sites needed the two new trailing
parameters -- `isolation_test.go`/`persistence_test.go`/`scheduled_queries_test.go` --
reverted together via `cp` from `git show HEAD:<path>` so the reverted interface and its
reverted callers stayed consistent) and confirmed the new test fails exactly as
predicted (`EncryptionOption`/`ObjectKeyPrefix` both empty), then restored and
`md5sum`-verified byte-identical against the pre-revert working tree.

Blast radius: 18 existing direct `backend.CreateScheduledQuery(...)` positional call
sites across `isolation_test.go`/`persistence_test.go`/`scheduled_queries_test.go` grew
two trailing string arguments (`""` for the 17 not exercising either field). No test
assertions changed meaning. The SDK-client-driven wire tests
(`handler_create_tags_test.go`, `wire_scheduledquery_timestamps_test.go`,
`wire_scheduledquery_targetconfig_test.go`, `wire_output_required_r80d_test.go`) call
through `tqsdk.CreateScheduledQueryInput{}`, not the backend's positional signature, so
none needed changes.

Gates: `go build ./...` clean; `go vet ./services/timestreamquery/...` clean; `gofmt -l`
empty; `go test -race -count=1 ./services/timestreamquery/...` green; `golangci-lint run
./services/timestreamquery/...` 0 issues. No persisted struct's on-disk shape changed in
an incompatible way: the two new `ScheduledQuery` fields are additive/`omitempty` on an
already-JSON-serialized struct (`store.Table[ScheduledQuery]`), so old snapshots decode
unchanged with both new fields defaulting to `""` -- no `timestreamquerySnapshotVersion`
bump needed (mirrors the KmsKeyId/TargetDetail precedent already established in this
file).
