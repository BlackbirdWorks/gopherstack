...
// TestRealClient_VirtualInterfaceAndGatewayAssociation drives directconnect's remaining typed-
// coverage-blind ops (gopherstack-n3zi) through the real
...
func TestRealClient_VirtualInterfaceAndGatewayAssociation(t *testing.T) {
	t.Parallel()
	cases := []struct {
		run  func(t *testing.T)
		name string
	}{
		{name: "hosted connection confirm and ConfirmCustomerAgreement", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "allocate and confirm public and transit VIFs", run: func(t *testing.T) {
			t.Helper()
			...
			pubOut, err := client.AllocatePublicVirtualInterface(
				ctx,
				&directconnectsdk.AllocatePublicVirtualInterfaceInput{
		...
		{name: "direct connect gateway association proposal lifecycle", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "interconnect describe, LOA, delete", run: func(t *testing.T) {
			t.Helper()
