...
type mockCODESTARCONNECTIONSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCODESTARCONNECTIONSIAMBackend() *mockCODESTARCONNECTIONSIAMBackend {
	return &mockCODESTARCONNECTIONSIAMBackend{
...
func (m *mockCODESTARCONNECTIONSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCODESTARCONNECTIONSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCODESTARCONNECTIONSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCODESTARCONNECTIONSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}
...
func setupCODESTARCONNECTIONSEnforcementServer(t *testing.T, iamBackend *mockCODESTARCONNECTIONSIAMBackend) *httptest.Server {
	...
	handler := codestarconnections.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockCODESTARCONNECTIONSIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIACODESTARCONNECTIONSUSER1",
			target:      "CodeStar_connections_20191201.ListConnections",
			body:        `{}`,
			setupBackend: func(b *mockCODESTARCONNECTIONSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACODESTARCONNECTIONSUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCODESTARCONNECTIONSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCODESTARCONNECTIONSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/codestar-connections/aws4_request, SignedHeaders=host, Signature=mock",
			)
