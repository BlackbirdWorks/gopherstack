...
overall: A                # 2026-08-07 (gopherstack-n7gh): USER_SRP_AUTH/ADMIN_USER_SRP_AUTH
                       # now implement real SRP-6a (see srp.go, srp_test.go, and the closed
                       # gap in gaps: below) -- previously InitiateAuth required
                       # AuthParameters[PASSWORD] directly for this flow, which a real SRP
                       # client never sends, so every real-client USER_SRP_AUTH attempt had
                       # always failed with NotAuthorizedException. ADMIN_USER_SRP_AUTH was
                       # worse: that exact flow name was not even in authenticate()'s accepted
                       # set, so it failed with InvalidUserPoolConfigurationException outright.
                       # This pass did NOT re-walk the rest of gopherstack-n7gh's stated scope
                       # (UserMigration ForgotPassword trigger source; domain
                       # AWSAccountId/ManagedLoginVersion/S3Bucket; user_import_jobs/devices/
                       # webauthn/managed_login_branding/risk_config/terms/log_delivery
                       # op-by-op; identity_providers/resource_servers diff) -- left open, see
                       # gaps below. Everything else carries forward unchanged from the
                       # 2026-07-25 parity-4 pass (not re-walked this pass).
                       #
