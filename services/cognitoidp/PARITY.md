...
### 2026-09-12 (gopherstack-n3zi slice 3: typed real-client coverage, cognitoidp's last 32)

Added `typed_slice3_realclient_test.go` (`TestSlice3_Cognitoidp_RealClient`, one
outer `t.Parallel()` test with 12 `t.Run` subtests, each also `t.Parallel()`),
covering every op slice 1 left unreached: managed login branding (Create/
...
(129/129, up from 97/129 at slice-1 handoff).
