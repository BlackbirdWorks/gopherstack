...
ops:
  ...
  CreateUserPool: {wire: ok, errors: ok, state: ok, persist: ok, note: "CLOSED 2026-09-11 (gopherstack-5f20): Policies.SignInPolicy.AllowedFirstAuthFactors is now real (previously an always-empty placeholder struct); see the user_auth family entry above"}
  DescribeUserPool: {wire: ok, errors: ok, state: ok, persist: ok, note: "Policies.SignInPolicy now echoes the pool's real AllowedFirstAuthFactors (gopherstack-5f20)"}
  UpdateUserPool: {wire: ok, errors: ok, state: ok, persist: ok, note: "PasswordPolicy, MfaConfiguration, LambdaConfig(stored only, see gaps), AccountRecoverySetting, DeletionProtection all settable; SignInPolicy now settable too (gopherstack-5f20)"}
...
Adjudicated all 6 `items_still_open` + 6 `deferred` entries. Removed 10 stale
CLOSED entries (already fixed at HEAD, verified against code, not just the
tool's flag: MFAOptions, DefaultRedirectURI/Schema, devices device_status,
risk_config LastModifiedAt, USER_AUTH, cursor-sweep pagination, terms, SRP,
UserMigration_ForgotPassword/domain fields, op-by-op re-walk). Fixed 1 real
gap: MFA_SETUP's ChallengeParameters now carries MFAS_CAN_SETUP (mfa.go),
proven via TestMFASetupSession_CompletesSignInWithoutAccessToken + hand-revert.
Kept 2, tightened to one line each: domains Routing/Version (no multi-region
routing model), MFA_SETUP session rotation semantics (AWS doc prose never
states it). `items_still_open` now holds exactly those 2; `deferred: []`.
...
ops (realclient_users_groups_and_identity_providers_test.go), grouped by family: groups (GetGroup/
...
### 2026-09-12 (gopherstack-n3zi: typed real-client coverage, cognitoidp's last 32)

Added `realclient_managed_login_and_provisioning_test.go` (`TestRealClient_ManagedLoginAndProvisioning`, one
table-driven `t.Parallel()` test with 12 cases, each also `t.Parallel()`),
covering every op the previous pass left unreached: managed login branding (Create/
Describe/DescribeByClient/Update/DeleteManagedLoginBranding), terms (Delete/
List/UpdateTerms), user import jobs (Create/Describe/List/Start/StopUserImportJob,
GetCSVHeader), user pool replicas (Create/List/Update/DeleteUserPoolReplica),
WebAuthn (CompleteWebAuthnRegistration/DeleteWebAuthnCredential), auth event
feedback (AdminListUserAuthEvents/AdminUpdateAuthEventFeedback/
UpdateAuthEventFeedback, seeded via the existing test-only
`SeedAuthEventForTest` since this backend has no sign-in-flow hook that
synthesizes risk events), admin provider linking (AdminDisableProviderForUser/
AdminLinkProviderForUser), AdminRespondToAuthChallenge (via a real
NEW_PASSWORD_REQUIRED challenge from AdminInitiateAuth), provisioned limits
(Get/UpdateProvisionedLimit), log delivery configuration (Get/
SetLogDeliveryConfiguration), GetTokensFromRefreshToken, and SetUserSettings.
Census confirms cognitoidp is now at **0 typed-client-uncovered ops**
(129/129, up from 97/129 before this pass).

One real bug found and fixed, caught only by decoding the *typed* GetUser
response after a real SetUserSettings call (no prior test decoded
`GetUserOutput.MFAOptions` at all):

1. **GetUser silently dropped MFAOptions.** `handleGetUserAccurate`
   (handler_users.go) built its `getUserWithMFAOutput` from `Username`,
   `UserAttributes`, `UserMFASettingList` and `PreferredMfaSetting` only --
   the legacy `MFAOptions []MFAOptionType` field that real
   `GetUserOutput` still carries (cognitoidentityprovider@1.67.4
   api_op_GetUser.go:72, "This response parameter is no longer supported...")
   was never populated, even though the sibling `AdminGetUser` path
   (`toAdminUserJSON`) already included it via `toMFAOptionsWire`. A real
   client calling SetUserSettings then GetUser always saw an empty
   MFAOptions slice regardless of what was just set. Fixed by adding
   `MFAOptions []mfaOptionType` to `getUserWithMFAOutput`
   (models_users.go) and populating it with the same
   `toMFAOptionsWire(user.MFAOptions)` helper `toAdminUserJSON` already
   uses. Confirmed by hand-reverting the fix and rerunning
   `set_user_settings`: fails (`0 items, expected 1`) on the old code,
   passes restored.

No accept-and-drop members found this pass beyond the one pre-existing,
already-documented gap (admin provider linking's `LinkedProviders` is stored
but not exposed on any read op's wire shape -- see the test's own comment;
not a new finding, `AdminLinkProviderForUser`/`AdminDisableProviderForUser`
are legitimate void-result ops both before and after this pass).
...
bumped: **no** (getUserWithMFAOutput is a wire-response-only struct, not part
of `backendSnapshot`; `pkgs/persistence/testdata/snapshot_inventory.json` is
unaffected for cognitoidp).
