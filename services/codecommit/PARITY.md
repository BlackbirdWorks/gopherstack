...
ops:
  ...
  DeleteFile: {wire: fixed, errors: fixed, state: fixed, persist: ok, note: "deleting a non-existent path silently fabricated a commit before; now FileDoesNotExistException. blobId in response was hardcoded empty; now the removed file's real blob id. This pass: parentCommitId was accepted and silently ignored (documented gap) — now required and validated against the branch tip (ParentCommitIdRequiredException/ParentCommitIdOutdatedException), matching real AWS's DeleteFileInput.ParentCommitId (a required field per the SDK's validators.go). Also never recorded fileHistory — now recorded. FIXED 2026-09-12 (gopherstack-xhu2t slice 3): KeepEmptyFolders, same fix and shared helper as CreateCommit -- see that row."}
  ...
  GetMergeCommit: {wire: fixed, errors: ok, state: ok, persist: ok, note: "FIXED 2026-08-30 (gopherstack-4a8v): the decode struct declared a mergeOption field that is not a real GetMergeCommitInput member at all (confirmed against api_op_GetMergeCommit.go and awsAwsjson11_serializeOpDocumentGetMergeCommitInput in serializers.go -- a real client never sends it). Deleted rather than wired up, per this campaign's fabricated-field convention. No observable runtime behavior changed (the field was already never read), so no new regression test was written for the deletion itself -- existing tests (TestHandler_GetMergeCommit et al.) still pass sending the now-ignored key, since an unrecognized JSON key is silently dropped by encoding/json either way. FIXED 2026-09-12 (typed slice 32): sourceCommitId/destinationCommitId were echoed as the raw, unresolved request specifiers (which can be branch names) instead of real commit IDs; now resolved via the new exported ResolveCommitSpecifier, which also makes an unresolvable specifier 404 instead of silently succeeding. baseCommitId remains a documented gap (no real merge-base algorithm), see dated section below. errcodeaudit 2026-09-12 FIX (gopherstack-r3pr): an empty commit specifier raised ErrValidation, mapped to the fabricated \"InvalidParameterException\" (no such type in codecommit@v1.36.4's SDK module at all). Renamed to ErrCommitSpecifierRequired, mapped to the real \"CommitRequiredException\" this op's own deserializeOpError models. Verified via TestMergeOps_EmptyCommitSpecifier_CommitRequiredException (real client, errors.As)."}
...
The mapper's final `ErrValidation` entry (handler.go's errCodeLookup) emitted
`InvalidParameterException`, a name absent from codecommit@v1.36.4's SDK
module entirely (189 exception types, grepped, no match). Its only raise
site is `resolveCommitSpecifier`'s empty-specifier check (merges.go),
reached by GetMergeCommit and MergeBranchesBy{Squash,ThreeWay,FastForward}.
All four operations' own `deserializeOpError` switches declare
`CommitRequiredException` — confirmed by reading each one directly in
deserializers.go, not inferred from the family. Renamed the sentinel
`ErrValidation` -> `ErrCommitSpecifierRequired` (it had exactly one use, so
no ripple) and changed the mapped code to `CommitRequiredException`.

New test `TestMergeOps_EmptyCommitSpecifier_CommitRequiredException`
(orphan_code_sentinel_fixes_test.go), table-driven across the four affected
ops, drives the real SDK client and asserts `errors.As(err,
&types.CommitRequiredException{})` — fails against the pre-fix code, where
none of the four ops' deserializers match "InvalidParameterException" to any
case, so the real client only ever got an untyped `smithy.GenericAPIError`.
...
Worked all 32 tier-1 findings from `cmd/reqfielddiff` for this service.
codecommit is not the generic `jsonOp(h.Backend.<Op>)` shape (slice-1's blind
spot) -- every op decodes its own hand-written anonymous request struct -- so
the tool's findings here are all genuine undecoded fields, not fields read
via a shape the tool can't see. 0 false positives.

7 of 32 were DROPPED PARAMETER, fixed with tests in
`reqfield_slice3_realclient_test.go` (`TestReqFieldSlice3_CodeCommit`, 7
subtests, real typed `aws-sdk-go-v2/service/codecommit` client):

- `DescribePullRequestEvents.MaxResults`, `GetCommentReactions.MaxResults`,
  `GetCommentsForComparedCommit.MaxResults`, `GetCommentsForPullRequest.MaxResults`:
  each op decoded MaxResults/NextToken nowhere and always returned its
  entire result set. Now paginated via `pkgs/page`, matching this service's
  existing `GetDifferences`/`ListFileCommitHistory` pattern. The two
  comment-family ops paginate the `comments` list nested inside the single
  group this backend already returns (see their PARITY.md notes for why: no
  per-diff-position grouping is modeled, so the outer group list is always
  <=1 element).
- `GetDifferences.AfterPath`: decoded nowhere; MaxResults/NextToken were
  already correctly wired (see the existing ops-table row) but the path
  filter that's supposed to narrow the diff set entirely was dropped. Now
  filters to an exact path match or directory-prefix match.
- `CreateCommit.KeepEmptyFolders` / `DeleteFile.KeepEmptyFolders`: decoded
  nowhere. This backend's folders are synthetic -- `GetFolder`/`GetFolderFiles`
  derive them by scanning existing file paths for a prefix match, so a
  folder emptied by a deletion already vanished from those listings,
  matching AWS's documented `KeepEmptyFolders=false` default. `true` was a
  real, silent no-op. Fixed by a new shared `keepEmptyFoldersLocked`
  (`commits.go`) that drops a `.gitkeep` marker file under a folder a
  deletion just emptied, keeping it visible via `GetFolder`. `PutResourceConfig`-
  style prefix matching factored into `pathMatchesFilter`, reused by
  `GetDifferences.AfterPath` above.

25 of 32 were MISSING FEATURE, recorded under `items_still_open` (not
synthesized): `ConflictDetailLevel`/`ConflictResolutionStrategy` on
`BatchDescribeMergeConflicts`, `CreateUnreferencedMergeCommit`,
`DescribeMergeConflicts`, `GetMergeCommit`, `GetMergeConflicts`,
`GetMergeOptions`, `MergeBranchesBySquash`, `MergeBranchesByThreeWay`,
`MergePullRequestBySquash`, `MergePullRequestByThreeWay` (20 fields) and
`KeepEmptyFolders` on the five merge ops in that same list that never model a
file-level deletion at all (5 fields) -- all trace to the same pre-existing,
already-extensively-documented root cause: `File` has no per-branch identity
(flat `repoName|filePath` keying), so there is no second version of any file
for these ops to diff or merge, and no deletion for the merge-side
`KeepEmptyFolders` fields to apply to. See the `items_still_open` entries
above (some pre-dating this pass) for the full root-cause writeup; this pass
only added the two new bullets naming the specific tier-1 fields.

No persisted struct fields changed, no version bump.

Gates: `go build ./...`, `go vet ./services/codecommit/...`, `go test -race
