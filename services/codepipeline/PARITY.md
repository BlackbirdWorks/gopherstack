...
sdk_module: aws-sdk-go-v2/service/codepipeline@v1.49.0   # version audited against
last_audit_commit: d50d1410                              # stale -- git usage disallowed this pass; see last_audit_date
last_audit_date: 2026-07-30
overall: B            # re-diffed the 5 families the 2026-07-23 pass left un-diffed (webhooks/customActionTypes/jobsAndThirdPartyJobs/ruleOps) plus OverrideStageCondition; fixed real bugs in webhooks (Tags dropped), jobs+thirdPartyJobs (PollForThirdPartyJobs wrong wire shape/leaked Nonce, PollForJobs/GetThirdPartyJobDetails missing Data.ActionTypeId), and OverrideStageCondition's conditionType validation (ON_SUCCESS wrongly rejected) -- BUT surfaced one SEVERE, unfixed gap: UpdateActionType's real input (ActionTypeDeclaration, with a required Executor/Lambda-or-JobWorker model) is structurally unrelated to what this handler parses, which is CreateCustomActionType's legacy shape. Downgraded A->B for that unfixed severe finding, matching how hybrid-AD was handled in directoryservice this same campaign.
...
families:
  ...
  customActionTypes: {status: partial, note: "Re-diffed against types.ActionType/ActionTypeId/CreateCustomActionTypeOutput this pass. CreateCustomActionType/GetActionType/ListActionTypes are all correctly shaped (id nested under 'id', actionConfigurationProperties correctly named, inputArtifactDetails/outputArtifactDetails/settings all correct) -- genuinely clean, no changes needed. SEVERE finding: UpdateActionType is NOT clean -- see gaps. ListActionTypesInput.RegionFilter is parsed but never applied (see gaps, low severity)."}
...
gaps:                     # known divergences NOT fixed — link bd issue ids
  ...
  - "UpdateActionType (customActionTypes family) accepts and stores CreateCustomActionType-shaped input (id/settings/configurationProperties/inputArtifactDetails/outputArtifactDetails), but AWS's real UpdateActionTypeInput.ActionType is types.ActionTypeDeclaration -- a structurally different, newer shape requiring Executor (*ActionTypeExecutor, REQUIRED: Configuration+Type=Lambda-or-JobWorker, optional JobTimeout/PolicyStatementsTemplate), Id (*ActionTypeIdentifier, a same-4-fields-different-Go-type sibling of ActionTypeId), InputArtifactDetails/OutputArtifactDetails (*ActionTypeArtifactDetails, a different Go type from the plain ArtifactDetails used elsewhere -- same 2 int32 fields, so wire-compatible at the leaf level, but the required Executor/Permissions/Properties/Urls members this backend never parses at all mean a real client's update is silently incomplete). This is a real, previously-undetected V2 custom-action-type-executor feature this backend does not model in any way -- Get/List never expose Executor/Permissions/Properties/Urls either. Not fixed this pass: this is a new-subsystem-sized gap, not a field patch, matching the hybrid-AD finding in directoryservice this same campaign. Flagged as the primary reason for this pass's overall grade (A->B)."
...
deferred:                 # consciously not audited this pass (scope) — next pass targets
  ...
  - "UpdateActionType real-shape redesign (see gaps) -- requires modeling ActionTypeExecutor/ActionTypeIdentifier/ActionTypeArtifactDetails/ActionTypePermissions/ActionTypeProperty/ActionTypeUrls and exposing them consistently across Create/Get/List/UpdateActionType, not just Update."
