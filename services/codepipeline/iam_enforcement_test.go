...
type mockCODEPIPELINEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCODEPIPELINEIAMBackend() *mockCODEPIPELINEIAMBackend {
	return &mockCODEPIPELINEIAMBackend{
...
func (m *mockCODEPIPELINEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCODEPIPELINEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCODEPIPELINEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCODEPIPELINEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCODEPIPELINEEnforcementServer(t *testing.T, iamBackend *mockCODEPIPELINEIAMBackend) *httptest.Server {
	t.Helper()

	backend := codepipeline.NewInMemoryBackend("000000000000", "us-east-1")
	handler := codepipeline.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCODEPIPELINEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCODEPIPELINEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/codepipeline/aws4_request, SignedHeaders=host, Signature=mock",
			)
