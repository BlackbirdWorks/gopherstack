...
type mockCLOUDTRAILIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCLOUDTRAILIAMBackend() *mockCLOUDTRAILIAMBackend {
	return &mockCLOUDTRAILIAMBackend{
...
func (m *mockCLOUDTRAILIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCLOUDTRAILIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCLOUDTRAILIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCLOUDTRAILIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCLOUDTRAILEnforcementServer(t *testing.T, iamBackend *mockCLOUDTRAILIAMBackend) *httptest.Server {
	t.Helper()

	backend := cloudtrail.NewInMemoryBackend("123456789012", "us-east-1")
	handler := cloudtrail.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockCLOUDTRAILIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIACLOUDTRAILUSER2",
			target:      "CloudTrail_20131101.CreateTrail",
			body:        `{"Name":"test-trail","S3BucketName":"test-bucket"}`,
			setupBackend: func(b *mockCLOUDTRAILIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIACLOUDTRAILUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCLOUDTRAILIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCLOUDTRAILEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/cloudtrail/aws4_request, SignedHeaders=host, Signature=mock",
			)
