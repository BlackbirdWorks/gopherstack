...
last_audit_date: 2026-09-04   # gopherstack-h6a: DeleteTrail/DeleteEventDataStore/DeleteChannel ghost-row fix
...
gaps:                     # known divergences NOT fixed — link bd issue ids
  ...
  - "gopherstack-h6a (2026-09-04, newly documented, not previously recorded): CreateTrail/UpdateTrail's S3BucketName is stored and echoed but never validated or used for delivery -- no check that the bucket exists (real AWS returns S3BucketDoesNotExistException), and no log files are ever actually written to the S3 backend regardless of trail/StartLogging state. Cross-service (would require services/cloudtrail to call into services/s3's backend, or a shared delivery pkg); left unfixed as a structural gap outside a single-pass mechanical fix, same class as the already-documented 'import execution not real' and 'Insights event synthesis' gaps. LookupEvents itself is unaffected: it reads CloudTrail's own captured Event History (RecordManagementEvent, wired via pkgs/service/cloudtrail_capture.go), which per real AWS semantics is independent of trails/S3 delivery entirely."
