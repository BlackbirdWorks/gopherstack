...
// TestWireFieldFixesSweep29 covers six pre-fix wire-shape bugs, each a
// mismatch between the response element the backend emitted and the one a
// real client's deserializer actually reads (ec2@v1.319.1
// deserializers.go), so a real client always decoded the field as empty/nil
// regardless of what the backend returned.
func TestWireFieldFixesSweep29(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			// handleDescribeAggregateIDFormat wrapped its status list under
			// "statuses" instead of "statusSet"
			// (awsEc2query_deserializeOpDocumentDescribeAggregateIdFormatOutput,
			// deserializers.go:196919).
			name: "aggregate id format statuses",
			run: func(t *testing.T, _ *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
				...
				assert.NotEmpty(
					t,
					out.Statuses,
					"Statuses empty - pre-fix wrapper key was \"statuses\" not \"statusSet\"",
				)
			},
		},
		{
			// handleDescribePrincipalIDFormat wrapped its list under
			// "principals" instead of "principalSet"
			// (awsEc2query_deserializeOpDocumentDescribePrincipalIdFormatOutput,
			// deserializers.go:203012), and flattened each entry to a bare
			// IdFormat instead of the PrincipalIdFormat{Arn, Statuses} shape
			// (awsEc2query_deserializeDocumentPrincipalIdFormat,
			// deserializers.go:143696).
			name: "principal id format principals",
			run: func(t *testing.T, _ *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
				...
				require.Len(
					t, out.Principals, 1,
					"Principals empty - pre-fix wrapper key was \"principals\" not \"principalSet\"",
				)
				assert.NotEmpty(
					t,
					out.Principals[0].Statuses,
					"nested Statuses empty - pre-fix flattened item shape dropped it",
				)
			},
		},
		{
			// exportTaskItem wrapped the instance details under
			// "instanceExportDetails" instead of "instanceExport"
			// (awsEc2query_deserializeDocumentExportTask,
			// deserializers.go:100167).
			name: "export task instance export details",
			run: func(t *testing.T, b *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
				...
				require.NotNil(
					t,
					createOut.ExportTask.InstanceExportDetails,
		...
		{
			// instanceImageMetadataItem put ImageId/ImageState directly on
			// the top-level element instead of nesting them under
			// "imageMetadata"
			// (awsEc2query_deserializeDocumentInstanceImageMetadata,
			// deserializers.go:112881, imageMetadata shape at
			// deserializers.go:107294).
			name: "instance image metadata",
			run: func(t *testing.T, b *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
		...
		{
			// snapshotLockItem emitted the lock duration under
			// "lockDurationDays" instead of "lockDuration"
			// (awsEc2query_deserializeDocumentLockedSnapshotsInfo,
			// deserializers.go:132176).
			name: "locked snapshot lock duration",
			run: func(t *testing.T, b *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
		...
		{
			// iamProfileSpec emitted the profile's second member under
			// "name" instead of "id"
			// (awsEc2query_deserializeDocumentIamInstanceProfile,
			// deserializers.go:105766).
			name: "iam instance profile association id",
			run: func(t *testing.T, b *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
				...
				assocOut, err := client.AssociateIamInstanceProfile(
					t.Context(),
		...
		{
			// importSnapshotTaskItem/importSnapshotResponse put status
			// directly at the top level instead of nesting it under
			// "snapshotTaskDetail"
			// (awsEc2query_deserializeDocumentImportSnapshotTask,
			// deserializers.go:109707, detail shape at
			// deserializers.go:158042) -- there is no top-level "status"
			// case at all.
			name: "import snapshot task detail",
			run: func(t *testing.T, _ *ec2.InMemoryBackend, client *ec2sdk.Client) {
				t.Helper()
				...
				describeOut, err := client.DescribeImportSnapshotTasks(
					t.Context(),
					&ec2sdk.DescribeImportSnapshotTasksInput{
