...
type mockCODEDEPLOYIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCODEDEPLOYIAMBackend() *mockCODEDEPLOYIAMBackend {
	return &mockCODEDEPLOYIAMBackend{
...
func (m *mockCODEDEPLOYIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCODEDEPLOYIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCODEDEPLOYIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCODEDEPLOYIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCODEDEPLOYEnforcementServer(t *testing.T, iamBackend *mockCODEDEPLOYIAMBackend) *httptest.Server {
	t.Helper()

	backend := codedeploy.NewInMemoryBackend("000000000000", "us-east-1")
	handler := codedeploy.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockCODEDEPLOYIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIACODEDEPLOYUSER1",
			target:      "CodeDeploy_20141006.ListApplications",
			body:        `{}`,
			setupBackend: func(b *mockCODEDEPLOYIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACODEDEPLOYUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCODEDEPLOYIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCODEDEPLOYEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/codedeploy/aws4_request, SignedHeaders=host, Signature=mock",
			)
