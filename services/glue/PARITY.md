...
Adjudicated 22 items_still_open + 6 deferred + j1b7's 2 items. Removed 11 stale/
already-fixed entries (closed bd tickets qd3.1-3.4, qd4.1-4.2, a250/awzv history).
Fixed for real: session PROVISIONING/STOPPING lifecycle; IntegrationResourceProperty's
ResourcePropertyArn (+ dropped a fabricated CreateTime field); StartDataQualityRulesetEvaluationRun's
Role/DataSource/AdditionalDataSources/AdditionalRunOptions; StartColumnStatisticsTaskRun's
SampleSize/SecurityConfiguration/ColumnNameList. j1b7's schema-diffing/DQDL items NOT
built -- re-confirmed package-sized; WebFetch to AWS docs returned no usable content here.
9 entries kept, tightened to one line each (unmodeled subsystems). 22 -> 9 open.
