...
type mockGLUEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockGLUEIAMBackend() *mockGLUEIAMBackend {
	return &mockGLUEIAMBackend{
...
func (m *mockGLUEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockGLUEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockGLUEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockGLUEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupGLUEEnforcementServer(t *testing.T, iamBackend *mockGLUEIAMBackend) *httptest.Server {
	t.Helper()

	backend := glue.NewInMemoryBackend("000000000000", "us-east-1")
	handler := glue.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockGLUEIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIAGLUEUSER1",
			target:      "AWSGlue.GetDatabases",
			body:        `{}`,
			setupBackend: func(b *mockGLUEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAGLUEUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAGLUEUSER2",
			target:      "AWSGlue.CreateDatabase",
			body:        `{"DatabaseInput":{"Name":"test-db"}}`,
			setupBackend: func(b *mockGLUEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAGLUEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockGLUEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupGLUEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/glue/aws4_request, SignedHeaders=host, Signature=mock",
			)
