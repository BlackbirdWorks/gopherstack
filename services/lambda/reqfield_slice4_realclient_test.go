...
// TestReqFieldDiffSlice4_Lambda drives every dropped-parameter fix from the
// reqfielddiff slice-4 pass through the real aws-sdk-go-v2 client
// (newTestLambdaClient, shared with typed_slice21_realclient_test.go) and
// asserts the observable effect of each.
func TestReqFieldDiffSlice4_Lambda(t *testing.T) {
	t.Parallel()
