...
// TestTypedSlice23RealClient drives managedblockchain's remaining typed-
...
func TestTypedSlice23RealClient(t *testing.T) {
	...
	t.Run("accessor CRUD", func(t *testing.T) {
		t.Parallel()
		...
		require.NoError(t, err, "GetAccessor on a deleted accessor still succeeds, reporting PENDING_DELETION status")
	...
	t.Run("proposal lifecycle: create, vote, invitation, reject", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, aws.ToString(proposeOut.ProposalId), aws.ToString(listProposalsOut.Proposals[0].ProposalId))
		...
		require.NotNil(t, invitationID, "the approved proposal's Invitations action must produce a real invitation")
	...
	t.Run("member update and node lifecycle", func(t *testing.T) {
		t.Parallel()
		...
		assert.True(t, aws.ToBool(getMemberOut.Member.LogPublishingConfiguration.Fabric.CaLogs.Cloudwatch.Enabled))
