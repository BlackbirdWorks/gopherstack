...
overall: A            # gopherstack-mhnk pass fixed: (1) GetSendStatistics Bounces/Complaints were hardcoded
                       ...
                       # 2026-09-11 (gopherstack-uve/ssk/nbp): LimitExceededException and
                       # MailFromDomainNotVerifiedException, both listed above as "correctly gapped", are now
                       # FIXED (real code, not fabricated) -- see families.limit_exceeded_enforcement and
                       # families.mail_from_domain_not_verified; the mailbox-simulator quota/label handling
                       # (gopherstack-uve) was also refined -- see families.mailbox_simulator_quota_exemption.
                       # FromArn/TemplateArn/Rejects remain genuinely gapped -- see gaps.
...
ops:
  ...
  SetIdentityMailFromDomain: {wire: ok, errors: ok, state: ok, persist: ok, note: "wire fixed + BehaviorOnMXFailure now accepted/validated/persisted/returned (was silently dropped). gopherstack-nbp (2026-09-11): MailFromDomain must now be a strict subdomain of the identity's own domain -> InvalidParameterValue otherwise, matching api_op_SetIdentityMailFromDomain.go's doc comment ('The MAIL FROM domain must 1) be a subdomain of the verified identity'); see families.mail_from_domain_not_verified."}
  ...
  GetSendStatistics: {wire: ok, errors: ok, state: ok, persist: ok, note: "gopherstack-mhnk pass: DeliveryAttempts was already accurate (real sends), but Bounces/Complaints were HARDCODED zero even for sends that would deterministically bounce/complain per real AWS's documented mailbox simulator (bounce@/complaint@/suppressionlist@/success@simulator.amazonses.com — https://docs.aws.amazon.com/ses/latest/dg/send-an-email-from-console.html#send-email-simulator). Fixed: SendEmail/SendTemplatedEmail (and SendRawEmail/SendBulkTemplatedEmail, which delegate to them) now classify recipients against these addresses and set Email.Bounced/Email.Complained; GetSendStatistics buckets these into real Bounces/Complaints counts. Rejects remains 0 — no content/virus-scanning concept exists anywhere in this backend, no documented deterministic trigger exists for it (unlike Bounces/Complaints), so it is left honestly at 0 rather than fabricated. gopherstack-uve (2026-09-11): re-verified against the dev guide's 'Using the mailbox simulator manually' section (fetched live) -- confirmed suppressionlist@ is documented as producing 'a hard bounce as if the recipient's address is on the global suppression list', i.e. genuinely a Bounce, not a Reject (the issue's own premise that it might be a Reject was stale/unverified — it is not); confirmed there is no Reject-triggering simulator address at all (the doc's own 'Testing Reject events' section requires an EICAR antivirus test attachment instead, which needs a content scanner this backend doesn't have — Rejects staying 0 remains correct, not merely undecided). Also added: (1) label support (bounce+label@simulator.amazonses.com behaves like bounce@, per the doc's 'supports labeling' note) via simulatorLocalPart (email_sending.go); (2) simulator-only sends (every recipient a simulator address) are excluded from the 24-hour Max24HourSend quota but remain subject to the per-second MaxSendRate, per the doc's own distinction ('are limited by your account's maximum sending rate, but they don't affect your daily sending quota') — see families.mailbox_simulator_quota_exemption."}
  CreateTemplate: {wire: ok, errors: ok, state: ok, persist: ok, note: "gopherstack-ssk (2026-09-11): enforces the 20,000-templates-per-region cap -> LimitExceededException; see families.limit_exceeded_enforcement."}
  ...
  CreateConfigurationSet: {wire: ok, errors: ok, state: ok, persist: ok, note: "gopherstack-ssk (2026-09-11): enforces the 10,000-configuration-sets cap -> LimitExceededException; see families.limit_exceeded_enforcement."}
...
families:
  mailbox_simulator_quota_exemption: {status: ok, note: "gopherstack-uve (2026-09-11): the mailbox simulator's own dev-guide page ('Using the mailbox simulator manually' -> 'Important considerations', fetched live) documents two DISTINCT exemptions that were conflated in the original issue text: simulator sends 'are limited by your account's maximum sending rate, but they don't affect your daily sending quota.' Implemented precisely: Email gained a SimulatorOnly bool (models.go, new omitempty field, additive -- no sesSnapshotVersion bump, see pkgs/persistence/testdata/snapshot_inventory.json), set true when every recipient (To+Cc+Bcc) is a simulator address (allRecipientsAreSimulator, email_sending.go). sentLast24HoursLocked (sending_stats.go) now excludes SimulatorOnly rows -- this feeds BOTH the Max24HourSend enforcement gate (checkAccountAndQuotaLocked) and GetSendQuota's reported SentLast24Hours, so a simulator-only send neither consumes the quota nor can itself be blocked by an already-exhausted one. sentLastSecondLocked deliberately does NOT exclude them -- MaxSendRate still applies, per the doc's own wording. A message mixing a simulator address with a real recipient is treated as an ordinary (quota-consuming) send: the doc's guarantee is scoped to the simulator address itself, not to any message that happens to mention one -- a disclosed simplification, since AWS doesn't document per-recipient accounting within a single message. Also added: simulatorLocalPart (email_sending.go) recognizes the documented label suffix ('supports labeling ... bounce+label1@simulator.amazonses.com') on all 5 simulator addresses, not just exact matches -- previously only exact-match bounce@/suppressionlist@/complaint@ were recognised at all. Re-verified suppressionlist@ against the live doc: it is documented as producing 'a hard bounce ... as if the recipient's address is on the global suppression list' -- a genuine Bounce, not a Reject as the original issue speculated; left unchanged (already correctly bucketed as Bounces before this pass). See GetSendStatistics's own row and the Rejects gap entry (gaps) for what remains unfixed."}
  limit_exceeded_enforcement: {status: ok, note: "gopherstack-ssk (2026-09-11) FIX. Previously LimitExceededException (types/errors.go: ErrorCode() returns unsuffixed 'LimitExceeded') was never returned by any op, despite 9 ops declaring it in their own deserializeOpError switch (aws-sdk-go-v2/service/ses@v1.37.4/deserializers.go, verified per-op by reading each awsAwsquery_deserializeOpError<Op> function body, not assumed from symmetry): CloneReceiptRuleSet, CreateConfigurationSet, CreateConfigurationSetEventDestination, CreateCustomVerificationEmailTemplate, CreateReceiptFilter, CreateReceiptRule, CreateReceiptRuleSet, CreateTemplate, UpdateReceiptRule. Deliberately did NOT enforce a cap on VerifyEmailIdentity/VerifyDomainIdentity (the real 10,000-identities-per-region cap exists per the dev guide FAQ) because neither op's own error switch declares LimitExceededException -- enforcing a cap the SDK's own error model doesn't recognise for that op would itself be a fabrication, per the no-invented-errors rule. Caps and citations (limits.go): receiptRuleSets=40, rulesPerRuleSet=200, actionsPerRule=10, recipientsPerRule=500, receiptFilters=100 (all quotas.html), eventDestinationsPerSet=10 (quotas.html), configurationSets=10000 (quotas.html), emailTemplates=20000 (quotas.html), customVerificationTemplates=50 (quotas.html omits this one entirely -- sourced from the dev guide FAQ instead, creating-identities.html#send-email-verify-address-custom-faq-q1: 'You can create up to 50 custom verification email templates per Amazon SES account.'). All 9 caps are named constants in one place (limits.go) rather than scattered magic numbers, and are overridable per-backend via WithResourceLimits (store.go, ResourceLimits struct) -- the same constructor-option pattern WithEmailTTL already established, survives Reset() via the same configuredEmailTTL-style precedent (configuredLimits) -- so tests exercising the 10,000/20,000-sized caps (limits_test.go) don't create that many real resources. CreateReceiptRule/UpdateReceiptRule additionally enforce the two per-rule sub-limits (actionsPerRule/recipientsPerRule) via a shared validateReceiptRuleLimitsLocked (receipt_rules.go) -- UpdateReceiptRule does NOT re-check rulesPerRuleSet since it replaces an existing rule rather than adding one. One pre-existing test (TestListReceiptRuleSets_Pagination, handler_sdk_roundtrip_test.go) needed WithResourceLimits to raise its cap to 200: it creates 101 rule sets purely to exercise the unrelated 100-per-page pagination boundary, which the real 40-per-account cap would otherwise make unreachable -- a genuine real-AWS inconsistency (you can never fill a 100-item page under a 40-item cap), not a test bug."}
  mail_from_domain_not_verified: {status: ok, note: "gopherstack-nbp (2026-09-11) FIX, two parts. (a) SetIdentityMailFromDomain now enforces 'The MAIL FROM domain must 1) be a subdomain of the verified identity' (api_op_SetIdentityMailFromDomain.go doc comment, ses@v1.37.4) via isStrictSubdomainOf/identityDomain (identities.go) -> InvalidParameterValue for an equal, unrelated, or superdomain value; an empty MailFromDomain (clearing the setting) is exempt, matching the same doc's 'A value of null disables the custom MAIL FROM setting'. (b) checkMailFromLocked (identities.go) is now real code, called from sendEmailLocked/sendTemplatedEmailLocked (and therefore SendRawEmail/SendBulkTemplatedEmail, which delegate to them) after the existing isVerifiedLocked check: when the resolved identity's MailFromStatus is not Success and BehaviorOnMXFail is RejectMessage, it returns ErrMailFromDomainNotVerified (wire code MailFromDomainNotVerifiedException, confirmed declared for all four Send* ops via deserializers.go). This service's identity verification is instant everywhere (VerifyEmailIdentity/VerifyDomainIdentity/VerifyDomainDkim all skip the real Pending window, a deliberate documented convention -- see Notes below) and SetIdentityMailFromDomain follows the same convention (always sets MailFromStatus to Success immediately), so in normal client-driven operation this branch is DEAD CODE -- there is no client-reachable path to a non-Success MailFromStatus, and per the campaign instructions no config flag was invented to create one. It is nonetheless real, tested code, not a stub: TestSendEmail_MailFromDomainNotVerified (mail_from_internal_test.go, package ses -- a white-box test, not an export_test.go export) reaches into b.identities directly (the same internal field-access pattern every other backend method in this package already uses) to force a non-Success status, proving the branch fires correctly and that BehaviorOnMXFailure=UseDefaultValue stays permissive on the same non-Success status. Also refactored: isVerifiedLocked now delegates to a new shared resolveVerifiedIdentityLocked (identities.go) so checkMailFromLocked applies the identical exact-then-domain-fallback identity resolution order -- zero behavior change to isVerifiedLocked itself (traced case-by-case against the original inline logic)."}
  ...
  send_quota_enforcement: {status: ok, note: "GetSendQuota advertised Max24HourSend=200 (the real AWS SES sandbox default) but nothing enforced it — accounts could send arbitrarily many emails despite reporting a 200/24h cap. Added enforcement in checkSendingAllowedLocked using the same sentLast24HoursLocked() counting logic GetSendQuota already used (factored out, not duplicated). FIXED 2026-09-11 (gopherstack-a6y): MaxSendRate (1 msg/sec) is now also enforced, via the same maxSendRate constant GetSendQuota reports (checkSendRateLocked, sentInWindowLocked(time.Second) -- shares the sliding-window helper sentLast24HoursLocked already used, just parameterized on window duration; no new persisted or transient state, no injected clock -- the window is derived from b.emails' own Timestamp, same source of truth as Max24HourSend). Deliberate design choice, not a gap: the rate check runs once per SendEmail/SendTemplatedEmail/SendRawEmail call, but SendBulkTemplatedEmail checks it exactly ONCE per call (not once per destination) -- real AWS returns a per-destination BulkEmailStatus (including a dedicated AccountThrottled value, ses@v1.37.4/types/enums.go:61) rather than failing an entire bulk request over one rate-limited destination, and this backend's SendBulkTemplatedEmailOutput doesn't model that per-destination status shape (pre-existing gap, not introduced here) -- so throttling per destination inside one bulk call would incorrectly fail legitimate multi-destination requests this backend can't otherwise partially-report. Each destination still separately consumes the 24-hour quota, unchanged. Error shape: HTTP 400, code \"Throttling\", message \"Maximum sending rate exceeded.\" verbatim (no typed exception exists for this in ses@v1.37.4/types/errors.go -- confirmed absent -- so this follows https://docs.aws.amazon.com/ses/latest/dg/manage-sending-quotas.html's documented generic error code, the same treatment this backend already gives other untyped SES errors)."}
  ...
  receipt_action_sqs_removed: {status: ok, note: "gopherstack-brmq (2026-09-08) FIX: removed the fabricated SQS receipt action (ReceiptActionTypeSQS, xmlSQSAction, the Rule.Actions.member.N.SqsAction.{QueueArn,TopicArn} parser branch, and the receiptActionToXML serialization case) -- ses@v1.37.4's types.ReceiptAction union has exactly 8 members (AddHeaderAction/BounceAction/ConnectAction/LambdaAction/S3Action/SNSAction/StopAction/WorkmailAction) and none is SQS; a case-insensitive search for 'sqsaction' across the whole pinned SDK module returns nothing, so no real client could ever send or receive this shape. CreateReceiptRule/UpdateReceiptRule now silently drop an SqsAction submission (same as any other unrecognized action key, e.g. WorkmailAction/ConnectAction today) instead of accepting and round-tripping it. ReceiptAction.SQSQueueARN/SQSTopicARN struct fields were deliberately left in place (models.go), now dead code documented as vestigial: removing them would change backendSnapshot's persisted shape (confirmed present in pkgs/persistence/testdata/snapshot_inventory.json) without a version bump being warranted for a P2 cleanup of an inert, unreachable-from-any-real-client shape -- sesSnapshotVersion was NOT bumped. Verified via TestSnapshotVersionGuard (0 ses violations after the change) and TestReceiptActionSQS_NotAWireShape (neutered pre-fix: produced `<SqsAction><QueueArn>...` in the DescribeReceiptRule XML body). One pre-existing test, TestReceiptActionToXML_AllTypes's \"sqs_action\" subtest, asserted the fabrication round-tripped through CreateReceiptRule->DescribeReceiptRule (wantContains: \"QueueArn\") -- removed, not weakened: it was asserting behavior that shouldn't exist. WorkmailAction/ConnectAction (real union members gopherstack does not model) are out of scope for this fix -- see gaps."}
...
2 tier-1 undeclared request fields, both recorded as a missing feature, 0 fixed: `SendBounce.Explanation`
and `.MessageDsn` are accepted-then-silently-dropped, but SendBounce's own backend method stores no
bounce-message content at all (no email/message record, unlike SendEmail), so there is nothing for
either field to alter -- see items_still_open. tier1 count 2 -> 2 (structural, not code changes).
...
2026-09-08 (gopherstack-brmq): removed the fabricated SQS receipt action --
see `receipt_action_sqs_removed` in `families` for the full account. Assessed
persistence impact first: `ReceiptAction.SQSQueueARN`/`SQSTopicARN` are listed
in `pkgs/persistence/testdata/snapshot_inventory.json`, so removing those
struct fields (not just the wire code) would be a backendSnapshot shape
change; decided to remove only the live wire surface (constant, XML struct,
parser branch, serializer case) and leave the two struct fields in place,
undocumented dead weight now explicitly commented as vestigial-for-persistence-
compat, rather than bump `sesSnapshotVersion` for a P2 cleanup of an inert
shape. `TestSnapshotVersionGuard` confirms this: 0 ses violations before or
after. WorkmailAction/ConnectAction (real `ReceiptAction` union members
gopherstack does not model at all) are unchanged by this fix and remain a gap
-- a candidate for a separate follow-up issue, not folded into this one.

### 2026-09-11 (gopherstack-uve/ssk/nbp: three previously-deferred issues made real)

Three issues carried in `gaps` as "deferred as low value" were re-investigated
against the pinned SDK (`ses@v1.37.4`) and the live AWS SES dev guide, per this
pass's instructions to make each real to the extent it can be honestly
sourced and disclose precisely what remains:

- **gopherstack-uve** (mailbox simulator quota/label handling) -- FIXED, see
  `families.mailbox_simulator_quota_exemption`. Simulator-only sends are now
  excluded from the 24-hour `Max24HourSend` quota but remain subject to the
  per-second `MaxSendRate` (the dev guide draws exactly this distinction);
  label suffixes (`bounce+label@simulator.amazonses.com`) are now recognised
  on all 5 simulator addresses, not just exact matches. `suppressionlist@`
  was re-verified live as a genuine Bounce (not a Reject as the issue text
  speculated) -- no code change needed there, already correct. `Rejects`
  remains honestly 0 (RE-CONFIRMED structural: the dev guide's own "Testing
  Reject events" section requires an EICAR-file virus-scan trigger this
  backend has no scanner to honor -- see `gaps`).
- **gopherstack-ssk** (`LimitExceededException`) -- FIXED, see
  `families.limit_exceeded_enforcement`. 9 ops now enforce their real,
  non-adjustable SES v1 caps (receipt rule sets, rules/actions/recipients per
  rule, IP filters, configuration sets, event destinations per set, email
  templates, custom verification templates), each cited to
  `quotas.html` or (for the one value that page omits) the dev guide FAQ.
  Deliberately NOT enforced: the 10,000-identity cap on
  `VerifyEmailIdentity`/`VerifyDomainIdentity`, because neither op's own SDK
  error switch declares `LimitExceededException` -- confirmed absent, not
  assumed.
- **gopherstack-nbp** (`MailFromDomainNotVerifiedException`) -- FIXED, see
  `families.mail_from_domain_not_verified`. Rule (a), MAIL FROM domain must
  be a strict subdomain of the identity, is now enforced unconditionally.
  Rule (b) is real, exercised code with no client-reachable trigger (this
  service's instant-verification convention means `MailFromStatus` is always
  `Success`) -- proven via a white-box internal test
  (`mail_from_internal_test.go`, `package ses`) that pokes the unexported
  field directly, the same seam every other backend method in this package
  already uses. No config flag was invented.

Gates: `go build ./...` (whole module, clean), `go vet ./services/ses/...`
(clean), `go test -race -count=1 ./services/ses/...` (pass, including the new
`limits_test.go`/`mail_from_test.go`/`mail_from_internal_test.go`/
`mailbox_simulator_test.go`), `golangci-lint run ./services/ses/...` (0
issues), `go test -count=1 ./pkgs/persistence/...` (pass after
`-run TestSnapshotVersionGuard -update`; the only golden diff is the new
additive `Email.SimulatorOnly` field plus pre-existing `mgn` rows from a
concurrent, unrelated agent's uncommitted work in this tree -- left
untouched, not reverted). `sesSnapshotVersion` was NOT bumped -- the new
field is `omitempty` and additive, matching the guard's own "bookkeeping,
not a version-bump case" message. One pre-existing test
(`TestListReceiptRuleSets_Pagination`) needed a `WithResourceLimits` override
