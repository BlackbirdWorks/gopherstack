...
type mockSESIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockSESIAMBackend() *mockSESIAMBackend {
	return &mockSESIAMBackend{
...
func (m *mockSESIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockSESIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockSESIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockSESIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupSESEnforcementServer(t *testing.T, iamBackend *mockSESIAMBackend) *httptest.Server {
	t.Helper()

	backend := ses.NewInMemoryBackend()
	handler := ses.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockSESIAMBackend)
		name          string
		accessKeyID   string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIASESUSER2",
			body:        "Action=VerifyEmailIdentity&EmailAddress=test@example.com&Version=2010-12-01",
			setupBackend: func(b *mockSESIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIASESUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockSESIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupSESEnforcementServer(t, iamBackend)
			ctx := t.Context()
