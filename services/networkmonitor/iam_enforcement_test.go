...
type mockNETWORKMONITORIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockNETWORKMONITORIAMBackend() *mockNETWORKMONITORIAMBackend {
	return &mockNETWORKMONITORIAMBackend{
...
func (m *mockNETWORKMONITORIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockNETWORKMONITORIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockNETWORKMONITORIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockNETWORKMONITORIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupNETWORKMONITOREnforcementServer(t *testing.T, iamBackend *mockNETWORKMONITORIAMBackend) *httptest.Server {
	t.Helper()

	backend := networkmonitor.NewInMemoryBackend("us-east-1", "000000000000")
	handler := networkmonitor.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockNETWORKMONITORIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIANETWORKMONITORUSER1",
			method:      "GET",
			path:        "/monitors",
			body:        ``,
			setupBackend: func(b *mockNETWORKMONITORIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIANETWORKMONITORUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIANETWORKMONITORUSER2",
			method:      "POST",
			path:        "/monitors",
			body:        `{"monitorName":"test-m"}`,
			setupBackend: func(b *mockNETWORKMONITORIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIANETWORKMONITORUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockNETWORKMONITORIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupNETWORKMONITOREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/networkmonitor/aws4_request, SignedHeaders=host, Signature=mock",
			)
