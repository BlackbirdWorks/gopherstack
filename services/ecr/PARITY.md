...
ops:
  ...
  PutImageTagMutability: {wire: fixed, errors: ok, state: ok, persist: ok, note: "exclusion filters (WILDCARD + literal) enforced correctly. FIXED 2026-09-11 (gopherstack-mven required-output sweep): ImageTagMutabilityExclusionFilter.Filter carried omitempty despite being required (types/types.go:577); as *string, the client-side required check only rejects nil, not an empty string, so a legitimate empty filter value was silently dropped."}
...
Added `realclient_policy_and_registry_configuration_test.go`, covering this service's last 28
