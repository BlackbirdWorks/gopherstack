...
type mockMEDIALIVEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMEDIALIVEIAMBackend() *mockMEDIALIVEIAMBackend {
	return &mockMEDIALIVEIAMBackend{
...
func (m *mockMEDIALIVEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMEDIALIVEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMEDIALIVEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMEDIALIVEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMEDIALIVEEnforcementServer(t *testing.T, iamBackend *mockMEDIALIVEIAMBackend) *httptest.Server {
	t.Helper()

	backend := medialive.NewInMemoryBackend("000000000000", "us-east-1")
	handler := medialive.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockMEDIALIVEIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAMEDIALIVEUSER1",
			method:      "GET",
			path:        "/prod/channels",
			body:        ``,
			setupBackend: func(b *mockMEDIALIVEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAMEDIALIVEUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAMEDIALIVEUSER2",
			method:      "POST",
			path:        "/prod/channels",
			body:        `{"name":"test-c"}`,
			setupBackend: func(b *mockMEDIALIVEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMEDIALIVEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMEDIALIVEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMEDIALIVEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/medialive/aws4_request, SignedHeaders=host, Signature=mock",
			)
