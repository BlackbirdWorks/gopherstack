...
// TestRealClient_DeleteReservation covers medialive's DeleteReservation op
// (gopherstack-n3zi): DeleteReservation only allows deleting an EXPIRED
// reservation, so this test uses the existing ForceReservationEnd test
// export to backdate a purchased reservation's term instead of waiting out
// a real lease.
func TestRealClient_DeleteReservation(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
