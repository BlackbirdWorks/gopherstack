...
// TestSlice6_MediaLive_RealClient covers medialive's highest-priority typed-
// client-uncovered op families (gopherstack-n3zi slice 6): channels, inputs,
// input security groups, multiplexes/programs, schedules, reservations/
// offerings, signal maps, event bridge rule templates, cloudwatch alarm
// templates, nodes/clusters/networks, input devices, batch ops, and tags.
// Each subtest creates real state through the typed aws-sdk-go-v2 medialive
// client and asserts decoded response values.
func TestSlice6_MediaLive_RealClient(t *testing.T) {
	t.Parallel()
