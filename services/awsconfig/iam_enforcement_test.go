...
type mockAWSCONFIGIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockAWSCONFIGIAMBackend() *mockAWSCONFIGIAMBackend {
	return &mockAWSCONFIGIAMBackend{
...
func (m *mockAWSCONFIGIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockAWSCONFIGIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockAWSCONFIGIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockAWSCONFIGIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupAWSCONFIGEnforcementServer(t *testing.T, iamBackend *mockAWSCONFIGIAMBackend) *httptest.Server {
	t.Helper()

	backend := awsconfig.NewInMemoryBackend()
	handler := awsconfig.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockAWSCONFIGIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAAWSCONFIGUSER2",
			target:      "StarlingDoveService.PutConfigurationRecorder",
			body:        `{"ConfigurationRecorder":{"name":"default"}}`,
			setupBackend: func(b *mockAWSCONFIGIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAAWSCONFIGUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockAWSCONFIGIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupAWSCONFIGEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/config/aws4_request, SignedHeaders=host, Signature=mock",
			)
