...
type mockTRANSCRIBEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockTRANSCRIBEIAMBackend() *mockTRANSCRIBEIAMBackend {
	return &mockTRANSCRIBEIAMBackend{
...
func (m *mockTRANSCRIBEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockTRANSCRIBEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockTRANSCRIBEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockTRANSCRIBEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupTRANSCRIBEEnforcementServer(t *testing.T, iamBackend *mockTRANSCRIBEIAMBackend) *httptest.Server {
	t.Helper()

	backend := transcribe.NewInMemoryBackend()
	handler := transcribe.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockTRANSCRIBEIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIATRANSCRIBEUSER2",
			target:      "Transcribe.StartTranscriptionJob",
			body:        `{"TranscriptionJobName":"job1","Media":{"MediaFileUri":"s3://b/k"},"LanguageCode":"en-US"}`,
			setupBackend: func(b *mockTRANSCRIBEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIATRANSCRIBEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockTRANSCRIBEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupTRANSCRIBEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/transcribe/aws4_request, SignedHeaders=host, Signature=mock",
			)
