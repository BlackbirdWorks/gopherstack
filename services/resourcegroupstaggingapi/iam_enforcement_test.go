...
type mockRESOURCEGROUPSTAGGINGAPIIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockRESOURCEGROUPSTAGGINGAPIIAMBackend() *mockRESOURCEGROUPSTAGGINGAPIIAMBackend {
	return &mockRESOURCEGROUPSTAGGINGAPIIAMBackend{
...
func (m *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}
...
func setupRESOURCEGROUPSTAGGINGAPIEnforcementServer(t *testing.T, iamBackend *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) *httptest.Server {
	...
	handler := resourcegroupstaggingapi.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockRESOURCEGROUPSTAGGINGAPIIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIARESOURCEGROUPSTAGGINGAPIUSER1",
			target:      "ResourceGroupsTaggingAPI_20170126.GetResources",
			body:        `{}`,
			setupBackend: func(b *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIARESOURCEGROUPSTAGGINGAPIUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIARESOURCEGROUPSTAGGINGAPIUSER2",
			target:      "ResourceGroupsTaggingAPI_20170126.TagResources",
			body:        `{"ResourceARNList":["arn:aws:s3:::bucket"],"Tags":{"env":"prod"}}`,
			setupBackend: func(b *mockRESOURCEGROUPSTAGGINGAPIIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIARESOURCEGROUPSTAGGINGAPIUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockRESOURCEGROUPSTAGGINGAPIIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupRESOURCEGROUPSTAGGINGAPIEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/tagging/aws4_request, SignedHeaders=host, Signature=mock",
			)
