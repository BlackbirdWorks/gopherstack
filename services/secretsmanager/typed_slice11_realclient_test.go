...
const (
	slice11PrimaryRegion = "us-east-1"
	slice11ReplicaRegion = "us-west-2"
)

// TestTypedSlice11RealClient drives secretsmanager's typed-coverage-blind ops
// (gopherstack-n3zi slice 11) through the real aws-sdk-go-v2 client.
func TestTypedSlice11RealClient(t *testing.T) {
	t.Parallel()

	t.Run("batch get secret value", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("cancel rotate secret", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("validate resource policy", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("replicate and remove regions from replication", func(t *testing.T) {
		t.Parallel()

		h := secretsmanager.NewHandler(secretsmanager.NewInMemoryBackend())
		primary := newTestSMClientWithRegion(t, h, slice11PrimaryRegion)
		replica := newTestSMClientWithRegion(t, h, slice11ReplicaRegion)
		ctx := t.Context()
		...
		replOut, err := primary.ReplicateSecretToRegions(ctx, &secretsmanagersdk.ReplicateSecretToRegionsInput{
			...
			AddReplicaRegions: []smtypes.ReplicaRegionType{
				{Region: aws.String(slice11ReplicaRegion)},
			},
		})
		require.NoError(t, err)
		require.Len(t, replOut.ReplicationStatus, 1)
		assert.Equal(t, slice11ReplicaRegion, aws.ToString(replOut.ReplicationStatus[0].Region))
		...
		removeOut, err := primary.RemoveRegionsFromReplication(
			ctx, &secretsmanagersdk.RemoveRegionsFromReplicationInput{
				SecretId:             aws.String("s11-replicated"),
				RemoveReplicaRegions: []string{slice11ReplicaRegion},
			},
		)
		require.NoError(t, err)
	...
	t.Run("stop replication to replica", func(t *testing.T) {
		t.Parallel()
		...
		require.NoError(t, err)
		require.Equal(t, slice11PrimaryRegion, aws.ToString(beforeDesc.PrimaryRegion))
		...
		assert.Equal(t, slice11ReplicaRegion, aws.ToString(afterDesc.PrimaryRegion),
		...
		for _, st := range primaryReplStatus.ReplicationStatus {
			assert.NotEqual(t, slice11ReplicaRegion, aws.ToString(st.Region),
