...
// TestMediaConvert_TypedSlice25 drives every remaining uncovered op through
...
func TestMediaConvert_TypedSlice25(t *testing.T) {
	t.Parallel()

	role := "arn:aws:iam::123456789012:role/MediaConvert_Default_Role"

	t.Run("job lifecycle: list, cancel, search, jobsquery", func(t *testing.T) {
		t.Parallel()
		...
		queryOut, err := client.GetJobsQueryResults(ctx, &mediaconvertsdk.GetJobsQueryResultsInput{Id: startOut.Id})
		require.NoError(t, err)
	...
	t.Run("job template lifecycle: get, update, delete", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("queue update", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("tags: tag and untag a resource", func(t *testing.T) {
		t.Parallel()
		...
		tagsOut, err := client.ListTagsForResource(ctx, &mediaconvertsdk.ListTagsForResourceInput{Arn: aws.String(arn)})
		require.NoError(t, err)
	...
	t.Run("DescribeEndpoints", func(t *testing.T) {
		t.Parallel()
