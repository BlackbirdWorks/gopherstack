...
type mockMEDIACONVERTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMEDIACONVERTIAMBackend() *mockMEDIACONVERTIAMBackend {
	return &mockMEDIACONVERTIAMBackend{
...
func (m *mockMEDIACONVERTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMEDIACONVERTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMEDIACONVERTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMEDIACONVERTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMEDIACONVERTEnforcementServer(t *testing.T, iamBackend *mockMEDIACONVERTIAMBackend) *httptest.Server {
	t.Helper()

	backend := mediaconvert.NewInMemoryBackend("000000000000", "us-east-1")
	handler := mediaconvert.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockMEDIACONVERTIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAMEDIACONVERTUSER1",
			method:      "GET",
			path:        "/2017-08-29/queues",
			body:        ``,
			setupBackend: func(b *mockMEDIACONVERTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAMEDIACONVERTUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAMEDIACONVERTUSER2",
			method:      "POST",
			path:        "/2017-08-29/queues",
			body:        `{"name":"test-q"}`,
			setupBackend: func(b *mockMEDIACONVERTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMEDIACONVERTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMEDIACONVERTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMEDIACONVERTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/mediaconvert/aws4_request, SignedHeaders=host, Signature=mock",
			)
