...
families:
  generic_resource_plane: {status: ok, note: "one path walker (resourceid.go's ParseGenericResourcePath/ParseGenericResourceListPath) handles every namespace/type/nested-child-type shape; registry.go falls back to a metadata-only generic store for any namespace without a dedicated ResourceProvider, so future RPs (M8 ServiceBus, M9 DocumentDB, M11 KeyVault, M12 AppConfiguration) are additive. Namespace dispatch is case-insensitive (registry.go keys/looks up by lowercased namespace), matching ARM's own case-insensitive resource-provider-namespace semantics."}
