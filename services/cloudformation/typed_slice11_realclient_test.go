...
// TestTypedSlice11RealClient drives cloudformation's typed-coverage-blind
// ops (gopherstack-n3zi slice 11) through the real aws-sdk-go-v2 client.
func TestTypedSlice11RealClient(t *testing.T) {
	t.Parallel()

	t.Run("describe change set hooks", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("generated template describe and get", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("organizations access", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("resource scan describe and related resources", func(t *testing.T) {
		t.Parallel()
		...
		relatedOut, err := client.ListResourceScanRelatedResources(ctx, &cfnsdk.ListResourceScanRelatedResourcesInput{
			ResourceScanId: aws.String(scanID),
		...
		require.NoError(t, err)
		// gopherstack doesn't model cross-resource relationships (disclosed
		// gap): the honest response is an empty related-resources list, not
		// a fabricated relationship.
		assert.Empty(t, relatedOut.RelatedResources)
	})

	t.Run("type registration and test type", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("signal resource", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("list hook results", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("execute stack refactor", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("update stack instances", func(t *testing.T) {
		t.Parallel()
		...
		require.NoError(t, err)
		// Real AWS: UpdateStackInstances reports the same UPDATE action as
		// UpdateStackSet, not a fabricated "UPDATE_INSTANCES" value (fixed
		// gopherstack bug, see stack_sets.go's recordStackSetOperation doc).
