...
// newNetworkAndKMSServiceBackends creates a ServiceBackends with all phase-6 backends populated.
func newNetworkAndKMSServiceBackends(t *testing.T) *cloudformation.ServiceBackends {
	t.Helper()

	b := newExtraServiceBackends(t)
...
// TestResourceCreator_NetworkAndKMSTypes_NilBackends ensures every phase-6 resource type returns a stub
// physical ID (no panic, no error) when the backing service is nil.
func TestResourceCreator_NetworkAndKMSTypes_NilBackends(t *testing.T) {
	t.Parallel()
...
// TestResourceCreator_NetworkAndKMSTypes_RealBackends validates create and delete round-trips with
// real in-memory backends for a representative subset of phase-6 resource types.
func TestResourceCreator_NetworkAndKMSTypes_RealBackends(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			backends := newNetworkAndKMSServiceBackends(t)
...
// EC2NetworkAcl validates NetworkAcl round-trip with a real VPC.
func TestResourceCreator_EC2NetworkAcl(t *testing.T) {
	t.Parallel()
...
func TestResourceCreator_KMSReplicaKey(t *testing.T) {
	t.Parallel()
