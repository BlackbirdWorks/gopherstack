...
type mockBACKUPIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockBACKUPIAMBackend() *mockBACKUPIAMBackend {
	return &mockBACKUPIAMBackend{
...
func (m *mockBACKUPIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockBACKUPIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockBACKUPIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockBACKUPIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupBACKUPEnforcementServer(t *testing.T, iamBackend *mockBACKUPIAMBackend) *httptest.Server {
	t.Helper()
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockBACKUPIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIABACKUPUSER1",
			method:      "GET",
			path:        "/backup-vaults",
			body:        ``,
			setupBackend: func(b *mockBACKUPIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIABACKUPUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIABACKUPUSER2",
			method:      "PUT",
			path:        "/backup-vaults/test-vault",
			body:        `{}`,
			setupBackend: func(b *mockBACKUPIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIABACKUPUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockBACKUPIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupBACKUPEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/backup/aws4_request, SignedHeaders=host, Signature=mock",
			)
