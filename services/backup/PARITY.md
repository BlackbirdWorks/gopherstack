...
ops:
  ...
  ListBackupVaults: {wire: ok, errors: ok, state: fixed, persist: ok, note: "gopherstack-muzq (2026-08-21): see DescribeBackupVault's note -- same vaultStateFor(v) fix, same bug."}
  ...
  ListScanJobs: {wire: fixed, errors: ok, state: ok, persist: n/a, note: "2026-08-21 (gopherstack-r80d batch 11): same fix as DescribeScanJob -- ListScanJobsOutput.ScanJobs is []types.ScanJob, sharing the same 13 required members that were previously dropped to ScanJobId/Status. See ops.DescribeScanJob/families.ScanJob."}
  ...
  ListBackupJobSummaries: {wire: ok, errors: ok, state: ok, persist: n/a, note: "gopherstack-jqh2: GAP CLOSED -- GET /audit/backup-job-summaries had real handler+backend code (handler_backup_jobs.go) but was NEVER routed; parseBackupPath/parseBackupJobFamilyPath had no case for any /audit/*-job-summaries path, so every real client request 404'd. Route added."}
  ListCopyJobSummaries: {wire: ok, errors: ok, state: ok, persist: n/a, note: "gopherstack-jqh2: same unroutable bug as ListBackupJobSummaries (GET /audit/copy-job-summaries); fixed alongside it."}
  ListRestoreJobSummaries: {wire: ok, errors: ok, state: ok, persist: n/a, note: "gopherstack-jqh2: same unroutable bug as ListBackupJobSummaries (GET /audit/restore-job-summaries); fixed alongside it."}
  ListScanJobSummaries: {wire: ok, errors: ok, state: ok, persist: n/a, note: "gopherstack-jqh2: same unroutable bug as ListBackupJobSummaries (GET /audit/scan-job-summaries); fixed alongside it."}
...
gaps:
  - "ListBackupJobs/ListCopyJobs/ListRestoreJobsByProtectedResource/ListScanJobs read their date-range query filters under wrong keys: handler code does q.Get(\"byCreatedAfter\")/q.Get(\"byCreatedBefore\") (and byState/byAccountId/byParentJobId/byResourceArn/byResourceType for the non-timestamp filters on the same ops) but the real serializers.go for these ops encodes createdAfter/createdBefore/accountId/parentJobId/resourceArn/resourceType with no \"by\" prefix (confirmed at serializers.go:4636-4649 for ListBackupJobs, :5216-5229 ListCopyJobs, :5820-5825 ListRestoreJobsByProtectedResource, :6311-6324 ListScanJobs). The filter's own timestamp FORMAT parsing is correct (see families.timestamps) -- this is a wrong parameter NAME, so every real client's date-range/other filters on these 4 ops are silently ignored (ParseTimeFilter always receives an empty string, filter fields stay nil). Found during the 2026-08-29 timestamp-encoding pattern hunt but left unfixed as out of that pass's scope (a key-name bug, not an encoding bug) and no bd issue filed yet."
