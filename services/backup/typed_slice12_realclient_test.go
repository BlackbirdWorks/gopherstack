...
// newSlice12Client returns a fresh backend/handler/typed-client trio for one
// subtest.
func newSlice12Client(t *testing.T) (*backup.InMemoryBackend, *backupsdk.Client) {
	t.Helper()
...
func TestTypedSlice12_Backup(t *testing.T) {
	t.Parallel()

	t.Run("logically_air_gapped_vault_and_mpa_and_restore_access", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("vault_access_policy_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("vault_lock_configuration_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("vault_notifications_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
		...
		assert.ElementsMatch(t,
			[]types.BackupVaultEvent{types.BackupVaultEventBackupJobCompleted, types.BackupVaultEventBackupJobFailed},
	...
	t.Run("legal_hold_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("tiering_configuration_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("framework_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
		...
		// Update omitted FrameworkControls -- real AWS treats that as "no
		// change" (see handleUpdateFramework's doc comment), so the original
		// control must survive, not be wiped.
	...
	t.Run("report_plan_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("backup_plan_extras", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("backup_selection_conditions_roundtrip", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("backup_job_and_copy_job_extras", func(t *testing.T) {
		t.Parallel()
		b, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("recovery_point_extras", func(t *testing.T) {
		t.Parallel()
		b, client := newSlice12Client(t)
		ctx := t.Context()
		...
		require.NoError(t, err)
		assert.Equal(t, "arn:aws:ec2:us-east-1:123456789012:instance/i-rp-slice12", aws.ToString(descRP.ResourceArn))
		...
		updIdxOut, err := client.UpdateRecoveryPointIndexSettings(ctx, &backupsdk.UpdateRecoveryPointIndexSettingsInput{
			BackupVaultName:  aws.String("rp-vault"),
			RecoveryPointArn: rpArn,
		...
		_, err = client.DisassociateRecoveryPointFromParent(ctx, &backupsdk.DisassociateRecoveryPointFromParentInput{
			BackupVaultName:  aws.String("rp-vault"),
			RecoveryPointArn: rpArn,
		})
		require.NoError(t, err)
		...
		// DeleteRecoveryPoint is a separate op from DisassociateRecoveryPoint
		// -- seed a second recovery point directly to exercise it too.
	...
	t.Run("restore_job_extras", func(t *testing.T) {
		t.Parallel()
		b, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("global_and_region_settings", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("pitr_malware_scan_results", func(t *testing.T) {
		t.Parallel()
		b, client := newSlice12Client(t)
		ctx := t.Context()
		...
		// Seed a completed recovery point directly -- PITR results are
		// scoped to an existing recovery point in the vault.
	...
	t.Run("tags_lifecycle", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
	...
	t.Run("delete_restore_testing_selection", func(t *testing.T) {
		t.Parallel()
		_, client := newSlice12Client(t)
		ctx := t.Context()
