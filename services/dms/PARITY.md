...
ops:
  ...
  ListTagsForResource: {wire: ok, errors: ok, state: ok, persist: ok, note: "FIXED 2026-09-23 -- findResourceTags never checked certificatesByARN at all (Certificate had no Tags field either, see ImportCertificate); also added an ARN-suffix fallback (subgrp:/es: prefixes) for terraform-provider-aws builds a tag-lookup ARN using its own account id, which is empty under skip_requesting_account_id=true and so can miss this emulator's own account-qualified ARNs."}
  ...
  ImportCertificate: {wire: ok, errors: ok, state: ok, persist: ok, note: "FIXED 2026-07-31 -- the backend stored CertificatePem on Import but the response wire shape (certificateJSON) never returned it, on Import or Describe, even though the real Certificate type carries CertificatePem. Now returned on both. FIXED 2026-09-23 -- Certificate had no Tags field at all (Tags silently dropped on Import); now tagged like every other resource, wired into Reset/persistence."}
...
- **2026-09-23, terraform coverage sweep**: S3Settings is now modeled (stored
  ...
  always sends it fully populated; Certificate gained tagging support (was untagged).

- **2026-09-12 (reqfielddiff, gopherstack-xhu2t)**: worked all 14
  ...
  Proven via `realclient_resource_identifier_and_dropped_fields_test.go` driving the real
...
- **2026-09-12 (typed coverage sweep, gopherstack-n3zi)**: added
  `realclient_schema_conversion_test.go`, driving all 22 previously
...
client (`realclient_replication_and_migration_management_test.go`, 12 cases) -- census
