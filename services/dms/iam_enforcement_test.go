...
type mockDMSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockDMSIAMBackend() *mockDMSIAMBackend {
	return &mockDMSIAMBackend{
...
func (m *mockDMSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockDMSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockDMSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockDMSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupDMSEnforcementServer(t *testing.T, iamBackend *mockDMSIAMBackend) *httptest.Server {
	t.Helper()

	backend := dms.NewInMemoryBackend("000000000000", "us-east-1")
	handler := dms.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockDMSIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIADMSUSER1",
			target:      "AmazonDMSv20160101.DescribeReplicationInstances",
			body:        `{}`,
			setupBackend: func(b *mockDMSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIADMSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIADMSUSER2",
			target:      "AmazonDMSv20160101.CreateReplicationInstance",
			body:        `{"ReplicationInstanceIdentifier":"test-rep","ReplicationInstanceClass":"dms.t3.micro"}`,
			setupBackend: func(b *mockDMSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIADMSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockDMSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupDMSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/dms/aws4_request, SignedHeaders=host, Signature=mock",
			)
