...
ops:
  ...
  RevokeCertificate: {wire: ok, errors: partial, state: ok, persist: ok, note: "InvalidArnException wired. FIXED (gopherstack-hnyl): validRevocationReason was a hand-copied 10-entry allowlist missing SUPERCEDED, the deprecated misspelling types.RevocationReason still keeps alongside SUPERSEDED for back-compat -- now derives from types.RevocationReason.Values(). 2026-09-07 pass #2 (gopherstack-bzyl): RevokeCertificate's real error set (deserializers.go, v1.43.4: AccessDeniedException/ConflictException/InvalidArnException/ResourceInUseException/ResourceNotFoundException/ThrottlingException/ValidationException) has no InvalidStateException. FIXED the PENDING_VALIDATION guard to ConflictException (its doc text -- 'trying to update a resource ... already being created or updated, wait for the previous operation to finish and try again' -- matches a cert mid-validation). LEFT the already-revoked guard as InvalidStateException: no declared code's doc text fits a terminal, one-time-only state the way ConflictException fit the in-progress one; landmined rather than guessed, splitting the ambiguous pair the prior pass (gopherstack-ftkd) deferred as one."}
...
## Notes (2026-09-07 pass #2 — gopherstack-bzyl, 20 findings after today's per-emission-site grouping change)

Re-ran `cmd/errtargetaudit` after its grouping-by-emission-site change landed
today; it surfaced 20 acm findings split across ValidationException (12),
ResourceNotFoundException (6), and InvalidStateException (2) -- a third code
the filing issue's own framing had not mentioned, confirmed here rather than
taken on faith. All 20 sites trace to just 3 root causes, all in
RequestCertificate/RevokeCertificate:

1. **`setTags`'s tag-key/value length checks (handler_tags.go:77,80) still
   hardcoded ErrInvalidParameter (3 findings, FIXED).** gopherstack-ftkd's
   2026-09-07 pass #1 parameterized `setTags` with an `invalidTagErr`
   sentinel so each caller could supply its own declared code, and fixed the
   empty-key/reserved-prefix branches to use it -- but missed the two length
   branches immediately below them, which kept the pre-parameterization
   hardcoded ErrInvalidParameter regardless of caller. For the 3 legacy
   certificate-tag callers (RequestCertificate, AddTagsToCertificate,
   ImportCertificate, all passing ErrInvalidTag), a too-long key or value
   returned ValidationException instead of InvalidTagException; only
   RequestCertificate's real error set lacks ValidationException entirely,
   so only it showed up as a finding. Fixed: both branches now use
   `invalidTagErr`. New test:
   `TestACMHandler_LegacyTagOps_KeyOrValueTooLong_InvalidTagException`.
2. **RequestCertificate's own validators use ValidationException, which its
   real deserializer never declares (9 findings, FIXED).** Confirmed via
   `deserializeOpErrorRequestCertificate` (deserializers.go:3346-3400+,
   acm@v1.43.4): the declared set is InvalidArnException/
   InvalidDomainValidationOptionsException/InvalidParameterException/
   InvalidTagException/LimitExceededException/TagPolicyException/
   TooManyTagsException -- no ValidationException. `validateRequestCertInput`
   (DomainName-required, SAN wrap), `checkIdempotency` (token-reuse
   mismatch), `validateManagedBy`, and `jsonRequestCertificate`'s
   malformed-JSON-body case are all RequestCertificate-exclusive (grepped
   every call site to confirm), so fixed directly to a new
   `ErrRequestCertInvalidParameter` sentinel (InvalidParameterException --
   "An input parameter was invalid.", a clean word-for-word match).
   `validateDomainName` is shared with CreateAcmeDomainValidation (real set:
   ValidationException, no InvalidParameterException), so it was
   parameterized with a caller-supplied `invalidErr` instead of globally
   renamed -- RequestCertificate's two call sites pass
   `ErrRequestCertInvalidParameter`, CreateAcmeDomainValidation's keeps
   `ErrInvalidParameter`. STILL OPEN: the RSA_1024 weak-key check (crypto.go,
   shared with RenewCertificate) returns ValidationException for
   RequestCertificate too but wasn't a flagged finding this pass -- needs
   the same treatment, see deferred. New tests:
   `TestACMHandler_RequestCertificate_InvalidParameterException` (6 cases),
   `TestACMHandler_RequestCertificate_IdempotencyMismatch_InvalidParameterException`,
   and `TestACMHandler_CreateAcmeDomainValidation_InvalidDomainName_StillValidationException`
   (guards the un-touched sibling).
3. **RevokeCertificate's InvalidStateException guards (2 findings, SPLIT).**
   RevokeCertificate's real set (AccessDeniedException/ConflictException/
   InvalidArnException/ResourceInUseException/ResourceNotFoundException/
   ThrottlingException/ValidationException) has no InvalidStateException.
   gopherstack-ftkd's pass #1 found this and deliberately left both sites as
   one ambiguous pair ("no way to pick [ConflictException vs.
   ResourceInUseException] from the declared set alone"). Read the two
   guarded conditions separately instead of as a pair: the PENDING_VALIDATION
   guard matches ConflictException's own doc text ("trying to update a
   resource ... that is already being created or updated. Wait for the
   previous operation to finish and try again") almost word for word --
   FIXED. The already-revoked guard describes a permanent, one-time-only
   terminal state that neither ConflictException's "wait and retry" framing
   nor ResourceInUseException's "in use by another service" framing fits --
   LEFT as InvalidStateException, landmined with the reasoning in place
   rather than guessed. Updated the two pre-existing tests that pinned
   InvalidStateException for the PENDING_VALIDATION case specifically
   (`TestACMHandler_RevokeCertificate_PendingValidationRejected`,
   renamed `..._Returns_InvalidStateException` ->
   `..._Returns_ConflictException`) -- both loudly annotated with why the
   old assertion pinned an undeclared code. The already-revoked test
   (`TestACMHandler_RevokeCertificate/already_revoked`) was left untouched.

**Confirmed NOT bugs, no code changed:** the remaining 6 ResourceNotFoundException
findings (certificates.go:146/191/219, handler_certificates.go:300/304/308,
`ApplyDomainValidationOverrides`/`SetExportPreference`/`SetManagedBy`) are the
same false positive gopherstack-ftkd's pass #1 already found and left: they
guard a certificate ARN that RequestCertificate itself just minted, in the
same synchronous call, before the ARN is exposed to any other caller --
structurally unreachable, not a defect a client can trigger.

**Verification**: pre-fix failures captured via a `git worktree` at HEAD with
only the new/modified test files copied in (see method note in the
2026-08-19 pass above) -- every new/modified assertion failed with exactly
the predicted wrong code (`ValidationException` where `InvalidParameterException`
or `InvalidTagException` was expected; `InvalidStateException` where
`ConflictException` was expected) before the fix, and passes after.

Gates on `./services/acm/...`: `go build`, `go vet`, `gofmt -l` (empty),
`go test -race` (all pass), `golangci-lint run` (0 issues). Re-running
...
Verified all 6 census-flagged List ops member-by-member against structfielddiff.
ListAcmeAccounts/ListAcmeDomainValidations/ListAcmeEndpoints/
ListAcmeExternalAccountBindings: exact matches, false-positive census hits
(each Summary type is field-identical to its Describe counterpart in this
new ACME family). ListCertificates: found and fixed a real gap
(CertificateSummary.CertificateKeyPairOrigin never emitted). Also found
ListCertificateDomainValidations entirely unimplemented and implemented it
