...
type mockACMIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockACMIAMBackend() *mockACMIAMBackend {
	return &mockACMIAMBackend{
...
func (m *mockACMIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockACMIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockACMIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockACMIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupACMEnforcementServer(t *testing.T, iamBackend *mockACMIAMBackend) *httptest.Server {
	t.Helper()

	backend := acm.NewInMemoryBackend("000000000000", "us-east-1")
	handler := acm.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockACMIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAACMUSER2",
			target:      "CertificateManager.RequestCertificate",
			body:        `{"DomainName":"example.com"}`,
			setupBackend: func(b *mockACMIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAACMUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockACMIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupACMEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/acm/aws4_request, SignedHeaders=host, Signature=mock",
			)
