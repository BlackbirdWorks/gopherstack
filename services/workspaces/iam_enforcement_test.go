...
type mockWORKSPACESIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockWORKSPACESIAMBackend() *mockWORKSPACESIAMBackend {
	return &mockWORKSPACESIAMBackend{
...
func (m *mockWORKSPACESIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockWORKSPACESIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockWORKSPACESIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockWORKSPACESIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupWORKSPACESEnforcementServer(t *testing.T, iamBackend *mockWORKSPACESIAMBackend) *httptest.Server {
	t.Helper()

	backend := workspaces.NewInMemoryBackend("000000000000", "us-east-1")
	handler := workspaces.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockWORKSPACESIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAWORKSPACESUSER2",
			target:      "WorkspacesService.CreateWorkspaces",
			body:        `{"Workspaces":[{"BundleId":"wsb-123","DirectoryId":"d-123","UserName":"User"}]}`,
			setupBackend: func(b *mockWORKSPACESIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAWORKSPACESUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockWORKSPACESIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupWORKSPACESEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/workspaces/aws4_request, SignedHeaders=host, Signature=mock",
			)
