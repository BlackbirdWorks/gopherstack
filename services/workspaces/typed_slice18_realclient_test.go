...
// TestTypedSlice18Workspaces_RealClient drives every op the census still
// listed as uncovered before this pass (gopherstack-n3zi typed-client
// coverage slice 18). newTestHandlerAndClient (sdk_roundtrip_helper_test.go)
// stands up a fresh backend/handler/client triple per call.
func TestTypedSlice18Workspaces_RealClient(t *testing.T) {
	t.Parallel()

	t.Run("account_links", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
	...
	t.Run("ip_groups", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
		...
		afterUpdate, err := client.DescribeIpGroups(ctx, &wssdk.DescribeIpGroupsInput{GroupIds: []string{groupID}})
		require.NoError(t, err)
	...
	t.Run("tags", func(t *testing.T) {
		...
		desc, err := client.DescribeTags(ctx, &wssdk.DescribeTagsInput{ResourceId: aws.String("ws-slice18tags")})
		require.NoError(t, err)
		...
		afterDelete, err := client.DescribeTags(ctx, &wssdk.DescribeTagsInput{ResourceId: aws.String("ws-slice18tags")})
		require.NoError(t, err)
		assert.Empty(t, afterDelete.TagList)
	})

	t.Run("connect_client_addins", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
	...
	t.Run("connection_aliases", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
	...
	t.Run("account", func(t *testing.T) {
		...
		cidrs, err := client.ListAvailableManagementCidrRanges(ctx, &wssdk.ListAvailableManagementCidrRangesInput{
	...
	t.Run("client_branding", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
		...
		assert.Equal(t, wstypes.ReconnectEnumEnabled, props.ClientPropertiesList[0].ClientProperties.ReconnectEnabled)
	})

	t.Run("workspace_lifecycle", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
		...
		_, err = client.RestoreWorkspace(ctx, &wssdk.RestoreWorkspaceInput{WorkspaceId: aws.String(workspaceID)})
		require.NoError(t, err)
	...
	t.Run("bundles", func(t *testing.T) {
		...
		_, err = client.DeleteWorkspaceBundle(ctx, &wssdk.DeleteWorkspaceBundleInput{BundleId: aws.String(bundleID)})
		require.NoError(t, err)
	...
	t.Run("images", func(t *testing.T) {
		...
		_, err = client.DeleteWorkspaceImage(ctx, &wssdk.DeleteWorkspaceImageInput{ImageId: aws.String(updatedImageID)})
		require.NoError(t, err)
	})

	t.Run("application_associations", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
		...
		disassoc, err := client.DisassociateWorkspaceApplication(ctx, &wssdk.DisassociateWorkspaceApplicationInput{
			WorkspaceId:   aws.String(workspaceID),
			ApplicationId: aws.String("wsa-slice18"),
		})
		require.NoError(t, err)
	...
	t.Run("pools", func(t *testing.T) {
		...
		_, err = client.TerminateWorkspacesPool(ctx, &wssdk.TerminateWorkspacesPoolInput{PoolId: aws.String(poolID)})
		require.NoError(t, err)
	})
	...
	t.Run("directories_and_streaming", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newTestHandlerAndClient(t)
