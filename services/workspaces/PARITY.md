...
ops:
  ...
  DescribeWorkspaceDirectories: {wire: fixed, errors: ok, state: fixed, persist: ok, note: "FIXED 2026-08-28 (gopherstack-6flj/21my wrapper-key/silent-drop sweep): dirResp (handler_directories.go) carried only DirectoryId/DirectoryName/DirectoryType/Alias/State/SubnetIds -- every one of these real WorkspaceDirectory members (workspaces@v1.73.1 deserializers.go's awsAwsjson11_deserializeDocumentWorkspaceDirectory case list) was silently dropped despite this backend already holding the data via the 7 `Modify*` directory-settings ops (see DirectoryModifyOps below) and AssociateIpGroups: EndpointEncryptionMode, CertificateBasedAuthProperties, SamlProperties, SelfservicePermissions, WorkspaceAccessProperties, WorkspaceCreationProperties, and ipGroupIds (note the unusual lowercase-led wire key, deserializers.go:18124). Real AWS has no separate Describe op for any of these settings -- DescribeWorkspaceDirectories is the only place a real client ever reads them back, so this was an accept-and-drop bug across the whole family, not a mere omission. Fixed by reading storedDirSettings.Properties' existing prefixed keys (CertAuth_/Saml_/SelfSvc_/Access_/Creation_) and b.directoryIpGroups back into the new WorkspaceDirectory fields (interfaces.go), threaded through dirResp. Pointer sub-structs stay nil (omitted) for a directory never touched by the corresponding Modify op. See TestDescribeWorkspaceDirectories_RealSDKClient_SettingsRoundTrip in wire_field_fixes_test.go. NOT fixed this pass: WorkspaceAccessProperties.AccessEndpointConfig (ModifyWorkspaceAccessProperties' handler never accepted it as input either -- genuine unbuilt feature, not accept-and-drop) and StreamingProperties (ModifyStreamingProperties only threads StreamingExperiencePreferredProtocol through as a flat string; UserSettings/GlobalAccelerator/StorageConnectors are accepted on the input struct in the handler but never passed to the backend at all -- see gaps below, disclosed not fixed, out of scope for this pass' budget). FIXED 2026-09-24: (1) DirectoryName was never written anywhere by RegisterWorkspaceDirectory, so it was always \"\" on the wire -- see RegisterWorkspaceDirectory's note below for the fix. (2) The documented `WorkspaceDirectoryNames` request parameter was entirely ignored (only `DirectoryIds` was filtered); now implemented alongside DirectoryIds via the same buildFilter/matchesFilter helper, matched against the now-populated DirectoryName. (3) `Limit` was ignored (page size was hardcoded to the AWS-default 50); now caps the page at min(50, Limit) when Limit>0. NextToken was already correct (advanceDirCursor) -- audited, no change needed. NOT implemented: `Filters` (USER_IDENTITY_TYPE/WORKSPACE_TYPE) -- see items_still_open."}
  RegisterWorkspaceDirectory: {wire: fixed, errors: ok, state: fixed, persist: ok, note: "FIXED — re-registering an already-registered directory silently 200'd (unconditionally idempotent); now returns ResourceAlreadyExistsException, matching real AWS. FIXED 2026-09-24 (bug report): DirectoryName (real WorkspaceDirectory.Name, e.g. the AD FQDN corp.example.com) was never written anywhere in this backend, so DescribeWorkspaceDirectories always echoed \"\". Now populated at registration time by looking up directoryID in the services/directoryservice backend (wired lazily via SetAppConfig/GetDirectoryServiceHandler, see cross_service.go -- the established cross-service pattern documented on pkgs/service.AppContext, mirroring services/grafana/cross_service.go): DirectoryName<-Directory.Name, Alias<-Directory.Alias, DnsIpAddresses<-Directory.DNSIPAddrs, and for an AD Connector directory, CustomerUserName<-Directory.ConnectSettings.CustomerUserName (all real, unambiguous 1:1 field correspondences). DirectoryType is derived only for SimpleAD->SIMPLE_AD and ADConnector->AD_CONNECTOR (also unambiguous); MicrosoftAD/SharedMicrosoftAD are left unmapped rather than guessed -- see items_still_open. When Directory Service isn't wired or the directory isn't found there (e.g. most unit tests, which construct InMemoryBackend directly), DirectoryName falls back to the caller-supplied `WorkspaceDirectoryName` request field (now captured; previously decoded nowhere) rather than fabricating a value; if neither source has a name, DirectoryName stays honestly empty. Proven by TestRegisterWorkspaceDirectory_PopulatesDirectoryInfo (table-driven, typed SDK client: SimpleAD lookup, AD Connector lookup, no-DS fallback-to-request-name, no-DS-and-no-name stays empty)."}
...
items_still_open:
  ...
  - "DescribeWorkspaceDirectoriesInput.Filters (DescribeWorkspaceDirectoriesFilter{Name: USER_IDENTITY_TYPE|WORKSPACE_TYPE, Values}) is not implemented -- the request field is accepted and silently ignored. Real AWS filters on the directory's UserIdentityType/WorkspaceType, neither of which this backend captures at all today: RegisterWorkspaceDirectoryInput's real UserIdentityType/WorkspaceType members aren't decoded, stored, or echoed anywhere. Implementing Filters honestly requires modeling those two fields end to end first (a separate, larger gap than the DirectoryName bug this pass fixed); half-implementing Filters against an always-empty stored value would make any filtered DescribeWorkspaceDirectories call wrongly return zero directories, which is worse than the current no-op. Found, not fixed (2026-09-24)."
  - "RegisterWorkspaceDirectory's DirectoryType derivation only covers SimpleAD->SIMPLE_AD and ADConnector->AD_CONNECTOR (services/directoryservice.DirectoryType values with an unambiguous, same-concept WorkspaceDirectoryType equivalent). Directory Service's MicrosoftAD/SharedMicrosoftAD directories leave WorkspaceDirectory.DirectoryType empty rather than guessing CUSTOMER_MANAGED or AWS_IAM_IDENTITY_CENTER -- AWS's own docs list CUSTOMER_MANAGED as a valid WorkspaceDirectoryType value without documenting which Directory Service directory type(s) it corresponds to, so mapping it would be fabrication, not derivation. Found, not fixed (2026-09-24)."
deferred: []
