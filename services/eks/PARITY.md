...
ops:
  CreateCluster: {wire: deferred, errors: ok, state: ok, persist: ok, note: "2026-08-21 (gopherstack-y1zn): confirmed real bug, deferred. The real kubernetesNetworkConfig request/response object (types.KubernetesNetworkConfigRequest/Response) has ipFamily/serviceIpv4Cidr/serviceIpv6Cidr AND elasticLoadBalancing as sibling members of ONE \"kubernetesNetworkConfig\" key. This backend splits ElasticLoadBalancing into a second, separately-named top-level request field (\"networkingConfig\") and a matching separate response key -- a real client's ElasticLoadBalancing setting inside kubernetesNetworkConfig is never read at all (the parser only looks for a nonexistent sibling \"networkingConfig\" key), and the response key is also wrong. Fixing requires merging ElasticLoadBalancingConfig into the KubernetesNetworkConfig model/JSON struct and removing the separate NetworkingConfig type across clusters.go/models.go/handler_clusters.go (ClusterOptionalConfig, resolveClusterOptionalConfig, clusterNetConfigJSON, appendClusterOptionalInfra) -- contained to this package but a multi-function structural change, not a key rename. Not attempted this pass; budget went to the 526-key triage breadth instead."}
