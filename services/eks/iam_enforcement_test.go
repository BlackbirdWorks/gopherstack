...
type mockEKSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockEKSIAMBackend() *mockEKSIAMBackend {
	return &mockEKSIAMBackend{
...
func (m *mockEKSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockEKSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockEKSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockEKSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupEKSEnforcementServer(t *testing.T, iamBackend *mockEKSIAMBackend) *httptest.Server {
	t.Helper()

	backend := eks.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := eks.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAEKSUSER1",
			method:      "GET",
			path:        "/clusters",
			body:        ``,
			setupBackend: func(b *mockEKSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAEKSUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockEKSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupEKSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/eks/aws4_request, SignedHeaders=host, Signature=mock",
			)
