...
flat 300 TPS SendMessage-only limiter is now a per-API-method model (SendMessage,
ReceiveMessage, DeleteMessage each with their own 300-calls/sec + 3,000-messages/sec
budget), with FifoThroughputLimit choosing queue-wide vs per-MessageGroupId scope,
and batch calls (SendMessageBatch/DeleteMessageBatch) correctly consuming one
call-slot per scope touched instead of one per entry. See families.fifo_throughput_limit
above for the full writeup and doc citations.
