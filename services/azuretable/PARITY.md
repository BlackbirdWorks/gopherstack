...
ops:
  ...
  MergeEntity: {wire: ok, errors: ok, state: ok, persist: ok, note: "PATCH (aztables' real wire method) or literal MERGE /<account>/<table>(..), same If-Match semantics as ReplaceEntity but merges properties (unlisted properties survive) instead of replacing wholesale."}
...
(`odata_filter.go`/`odata_filter_eval.go`), modeled on
`services/dynamodb/expr`'s identical shape -- not string matching. Recursion
