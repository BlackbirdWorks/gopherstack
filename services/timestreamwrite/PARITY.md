...
ops:
  ...
  ResumeBatchLoadTask: {wire: ok, errors: ok, state: ok, persist: ok, note: "(2026-09-04) noticed but not changed: batch_load_tasks.go's ResumeBatchLoadTask doc comment says resumable from PENDING_RESUME or FAILED, but the guard checks PROGRESS_STOPPED or FAILED. The pinned SDK's api_op_ResumeBatchLoadTask.go has no operation doc comment and types/enums.go's BatchLoadStatus values carry no semantics per-value, so which status is the correct resume precondition cannot be determined from the SDK alone. See gaps."}
families:
