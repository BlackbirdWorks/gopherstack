...
type mockWAFV2IAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockWAFV2IAMBackend() *mockWAFV2IAMBackend {
	return &mockWAFV2IAMBackend{
...
func (m *mockWAFV2IAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockWAFV2IAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockWAFV2IAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockWAFV2IAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupWAFV2EnforcementServer(t *testing.T, iamBackend *mockWAFV2IAMBackend) *httptest.Server {
	t.Helper()

	backend := wafv2.NewInMemoryBackend("000000000000", "us-east-1")
	handler := wafv2.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockWAFV2IAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupWAFV2EnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/wafv2/aws4_request, SignedHeaders=host, Signature=mock",
			)
