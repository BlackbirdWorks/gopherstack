...
overall: A            # RESTORED A-->A (parity-5, 2026-07-31, follow-up pass): the
                      ...
                      # list) before applying the fix. See the 2026-09-17 Notes entry for the
                      # AgentsHandler deletion. Everything else: every named gap fixed for real;
...
repo's tests. `services/bedrock/realclient_guardrails_evaluation_and_model_governance_test.go` added, 15
...
and tags/resource policy -- the named priority families for this pass.
...
## gopherstack-n3zi: AgentsHandler typed-client coverage sweep (2026-09-12)
...
somewhere in this repo's tests. `services/bedrock/realclient_agents_flows_and_knowledge_bases_test.go`
added, 16 cases, drives all 74 of the prior pass's deliberately-skipped
...
## 2026-09-17 (gopherstack-m2eiu): AgentsHandler deleted, dead code

AgentsHandler and its whole agent/flow/prompt/knowledge-base/data-source/
ingestion-job domain (models, backend, handler, tests) are deleted: every
route it registered lost to services/bedrockagent.Handler (MatchPriority 87
vs 85), so its ops never executed in production. Two ops it advertised,
GetAgentMemory/DeleteAgentMemory, belong to bedrock-agent-runtime, a module
