...
// TestTypedSlice35RealClient drives elasticsearch's typed-coverage-blind ops
...
func TestTypedSlice35RealClient(t *testing.T) {
	...
	t.Run("inbound cross-cluster connections", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("domain lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("domain config extras", func(t *testing.T) {
		t.Parallel()
		...
		// DescribeElasticsearchDomainsOutput (api_op_DescribeElasticsearchDomains.go)
		// has only DomainStatusList -- no unprocessed-names member exists on the
		// real wire shape, so an unknown domain name is silently omitted from the
		// result rather than reported.
	...
	t.Run("reserved instances", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, "s35-reservation", aws.ToString(instancesOut.ReservedElasticsearchInstances[0].ReservationName))
	})
}
