...
DeleteVpcEndpoint/RevokeVpcEndpointAccess), inbound cross-cluster connections
(Accept/Reject/DeleteInboundCrossClusterSearchConnection, seeded via the
existing test-only `AddInboundConnectionInternal`), packages
(DeletePackage/UpdatePackage/GetPackageVersionHistory/ListDomainsForPackage/
ListPackagesForDomain), domain lifecycle (Cancel/
StartElasticsearchServiceSoftwareUpdate/UpgradeElasticsearchDomain/
GetUpgradeHistory/GetUpgradeStatus/DescribeElasticsearchInstanceTypeLimits/
ListElasticsearchInstanceTypes/DeleteElasticsearchServiceRole), versions
(GetCompatibleElasticsearchVersions/ListElasticsearchVersions), domain config
extras (UpdateElasticsearchDomainConfig/DescribeElasticsearchDomains/
DescribeDomainAutoTunes/DescribeDomainChangeProgress), reserved instances
(DescribeReservedElasticsearchInstanceOfferings/
PurchaseReservedElasticsearchInstanceOffering/
DescribeReservedElasticsearchInstances).

Four real bugs found and fixed, every one caught only by asserting on the
decoded typed-client value (all four returned 200 OK, so a status-only or
raw-JSON-contains test never caught any of them):

1. **`DescribeDomainChangeProgress`'s response used a fabricated `"Status"`
   key** (handler_domain_config.go) -- `types.ChangeProgressStatusDetails`
   has no `Status` member at all; the real deserializer reads
   `ConfigChangeStatus` (deserializers.go:10112,
   elasticsearchservice@v1.45.4), so every real client's
   `ChangeProgressStatus.ConfigChangeStatus` came back permanently empty.
   Fixed the key name, and also corrected the enum value itself --
   `types.ConfigChangeStatusCompleted` is mixed-case `"Completed"`, not
   `"COMPLETED"`.
2. **`AcceptInboundCrossClusterSearchConnection` set the wrong enum value**
   (inbound_connections.go) -- set `ConnectionStatus = "ACTIVE"`, but
   `types.InboundCrossClusterSearchConnectionStatusCode` has no `ACTIVE`
   value at all (enums.go:435-444); the correct accepted-connection status is
   `APPROVED`. `ACTIVE` belongs only to the sibling
   `OutboundCrossClusterSearchConnectionStatusCode` enum -- the two were
   conflated. The existing raw-body unit test
   (`TestElasticsearchHandler_AcceptInboundCrossClusterSearchConnection`)
   asserted the bug-matching `"ACTIVE"` string; updated to `"APPROVED"`, not
   weakened.
3. **`UpdatePackage` silently discarded the required `PackageSource`
   field** (handler_packages.go/packages.go) -- `UpdatePackageInput.
   PackageSource` is a real *required* member (the whole point of the op is
   uploading a new version's S3 source), but the handler only parsed
   `PackageID`/`PackageDescription` and the backend's `UpdatePackage`
   signature had no parameter for it at all. Every real client's new
   S3BucketName/S3Key was silently lost on every call. Fixed:
   `UpdatePackage` now takes a `PackageSource` parameter and stores it.
4. **`CancelElasticsearchServiceSoftwareUpdate`'s `AutomatedUpdateDate` was
   typed as a plain JSON string** (handler_domain_lifecycle.go) --
   restjson1's `unixTimestamp` wire format requires a JSON Number
   (deserializers.go:14366), so the always-empty-string field made every
   real client's decode fail outright (`"expected DeploymentCloseDateTimeStamp
   to be a JSON Number, got string instead"`), not just decode wrong.
   Fixed: field is now `*float64` with `omitempty`, always nil (this backend
   tracks no scheduled-update date), so the key is omitted rather than sent
   as an invalid empty string.

No accept-and-drop findings beyond ones already disclosed in
`items_still_open`.

Gates: `go build ./...` (whole module) clean. `go vet ./services/elasticsearch/...`
clean. `go test -race -count=1 ./services/elasticsearch/...` and
`./pkgs/persistence/...` both `ok`. `golangci-lint run --new-from-rev=HEAD
./services/elasticsearch/...` 0 issues. No `backendSnapshot` field changed,
