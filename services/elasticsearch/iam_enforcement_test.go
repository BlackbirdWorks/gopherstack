...
type mockELASTICSEARCHIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockELASTICSEARCHIAMBackend() *mockELASTICSEARCHIAMBackend {
	return &mockELASTICSEARCHIAMBackend{
...
func (m *mockELASTICSEARCHIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockELASTICSEARCHIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockELASTICSEARCHIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockELASTICSEARCHIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupELASTICSEARCHEnforcementServer(t *testing.T, iamBackend *mockELASTICSEARCHIAMBackend) *httptest.Server {
	t.Helper()

	backend := elasticsearch.NewInMemoryBackend("000000000000", "us-east-1")
	handler := elasticsearch.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockELASTICSEARCHIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAELASTICSEARCHUSER1",
			method:      "GET",
			path:        "/2015-01-01/es/domain",
			body:        ``,
			setupBackend: func(b *mockELASTICSEARCHIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAELASTICSEARCHUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAELASTICSEARCHUSER2",
			method:      "POST",
			path:        "/2015-01-01/es/domain",
			body:        `{"DomainName":"test-domain"}`,
			setupBackend: func(b *mockELASTICSEARCHIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAELASTICSEARCHUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockELASTICSEARCHIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupELASTICSEARCHEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/es/aws4_request, SignedHeaders=host, Signature=mock",
			)
