...
type mockIOTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockIOTIAMBackend() *mockIOTIAMBackend {
	return &mockIOTIAMBackend{
...
func (m *mockIOTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockIOTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockIOTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockIOTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupIOTEnforcementServer(t *testing.T, iamBackend *mockIOTIAMBackend) *httptest.Server {
	t.Helper()
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockIOTIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAIOTUSER1",
			method:      "GET",
			path:        "/things",
			body:        ``,
			setupBackend: func(b *mockIOTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAIOTUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAIOTUSER2",
			method:      "POST",
			path:        "/things/test-thing",
			body:        `{}`,
			setupBackend: func(b *mockIOTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAIOTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockIOTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupIOTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/iot/aws4_request, SignedHeaders=host, Signature=mock",
			)
