...
type mockBEDROCKRUNTIMEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockBEDROCKRUNTIMEIAMBackend() *mockBEDROCKRUNTIMEIAMBackend {
	return &mockBEDROCKRUNTIMEIAMBackend{
...
func (m *mockBEDROCKRUNTIMEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockBEDROCKRUNTIMEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockBEDROCKRUNTIMEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockBEDROCKRUNTIMEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupBEDROCKRUNTIMEEnforcementServer(t *testing.T, iamBackend *mockBEDROCKRUNTIMEIAMBackend) *httptest.Server {
	t.Helper()

	backend := bedrockruntime.NewInMemoryBackend("000000000000", "us-east-1")
	handler := bedrockruntime.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockBEDROCKRUNTIMEIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
		...
		},
		{
			...
			accessKeyID: "AKIABEDROCKRUNTIMEUSER2",
			method:      "POST",
			path:        "/model/anthropic.claude-v2/response-stream",
			body:        `{"prompt":"hello"}`,
			setupBackend: func(b *mockBEDROCKRUNTIMEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIABEDROCKRUNTIMEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockBEDROCKRUNTIMEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupBEDROCKRUNTIMEEnforcementServer(t, iamBackend)
			ctx := t.Context()
