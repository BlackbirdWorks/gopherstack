...
// TestRealClient_ConverseAndInvoke drives bedrockruntime's typed-coverage-blind
// ops (gopherstack-n3zi) through the real aws-sdk-go-v2 client.
func TestRealClient_ConverseAndInvoke(t *testing.T) {
	t.Parallel()
	cases := []struct {
		run  func(t *testing.T)
		name string
	}{
		{name: "converse", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "count tokens", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "async invoke lifecycle", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "invoke guardrail checks", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "invoke model with bidirectional stream", run: func(t *testing.T) {
			t.Helper()
