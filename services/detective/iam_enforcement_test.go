...
type mockDETECTIVEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockDETECTIVEIAMBackend() *mockDETECTIVEIAMBackend {
	return &mockDETECTIVEIAMBackend{
...
func (m *mockDETECTIVEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockDETECTIVEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockDETECTIVEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockDETECTIVEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupDETECTIVEEnforcementServer(t *testing.T, iamBackend *mockDETECTIVEIAMBackend) *httptest.Server {
	t.Helper()

	backend := detective.NewInMemoryBackend("000000000000", "us-east-1")
	handler := detective.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockDETECTIVEIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIADETECTIVEUSER1",
			method:      "POST",
			path:        "/graphs/list",
			body:        `{}`,
			setupBackend: func(b *mockDETECTIVEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIADETECTIVEUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIADETECTIVEUSER2",
			method:      "POST",
			path:        "/graph",
			body:        `{}`,
			setupBackend: func(b *mockDETECTIVEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIADETECTIVEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockDETECTIVEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupDETECTIVEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/detective/aws4_request, SignedHeaders=host, Signature=mock",
			)
