...
// newRealClient stands up the real aws-sdk-go-v2 detective client
...
func newRealClient(t *testing.T, h *Handler) *detectivesdk.Client {
	t.Helper()
...
// TestRealClient_MemberInvestigationAndOrgAdmin drives detective's remaining typed-client-blind
// ops through the real aws-sdk-go-v2 client (gopherstack-n3zi).
func TestRealClient_MemberInvestigationAndOrgAdmin(t *testing.T) {
	t.Parallel()
	cases := []struct {
		run  func(t *testing.T)
		name string
	}{
		{name: "member lifecycle create get delete list", run: func(t *testing.T) {
			t.Helper()

			backend := NewInMemoryBackend("000000000000", "us-east-1")
			client := newRealClient(t, NewHandler(backend))
			ctx := t.Context()
		...
		{name: "member self status transitions reject disassociate monitor", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "datasource packages batch get and update", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "investigations get list update state", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "organization admin lifecycle", run: func(t *testing.T) {
			t.Helper()
			...
			listOut, err := client.ListOrganizationAdminAccounts(
				ctx,
				&detectivesdk.ListOrganizationAdminAccountsInput{},
			)
			require.NoError(t, err)
			...
			listOut2, err := client.ListOrganizationAdminAccounts(
				ctx,
				&detectivesdk.ListOrganizationAdminAccountsInput{},
			)
			require.NoError(t, err)
