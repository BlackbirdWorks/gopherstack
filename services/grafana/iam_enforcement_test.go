...
type mockGRAFANAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockGRAFANAIAMBackend() *mockGRAFANAIAMBackend {
	return &mockGRAFANAIAMBackend{
...
func (m *mockGRAFANAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockGRAFANAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockGRAFANAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockGRAFANAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupGRAFANAEnforcementServer(t *testing.T, iamBackend *mockGRAFANAIAMBackend) *httptest.Server {
	t.Helper()

	backend := grafana.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := grafana.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAGRAFANAUSER1",
			method:      "GET",
			path:        "/workspaces",
			body:        ``,
			setupBackend: func(b *mockGRAFANAIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAGRAFANAUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockGRAFANAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupGRAFANAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/grafana/aws4_request, SignedHeaders=host, Signature=mock",
			)
