...
type mockDAXIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockDAXIAMBackend() *mockDAXIAMBackend {
	return &mockDAXIAMBackend{
...
func (m *mockDAXIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockDAXIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockDAXIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockDAXIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupDAXEnforcementServer(t *testing.T, iamBackend *mockDAXIAMBackend) *httptest.Server {
	t.Helper()

	backend := dax.NewInMemoryBackend("000000000000", "us-east-1")
	handler := dax.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIADAXUSER1",
			target:      "AmazonDAXV3.DescribeClusters",
			body:        `{}`,
			setupBackend: func(b *mockDAXIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIADAXUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockDAXIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupDAXEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/dax/aws4_request, SignedHeaders=host, Signature=mock",
			)
