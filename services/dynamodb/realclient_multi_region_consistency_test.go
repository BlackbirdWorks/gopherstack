...
// UpdateTable.MultiRegionConsistency (reqfielddiff, gopherstack-xhu2t)
...
func TestUpdateTable_MultiRegionConsistency_SurvivesWireConversion(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "explicit STRONG survives", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "omitted defaults to EVENTUAL when promoted to global table", run: func(t *testing.T) {
			t.Helper()
			...
			assert.Equal(
				t,
				dynamodbtypes.MultiRegionConsistencyEventual,
				updOut.TableDescription.MultiRegionConsistency,
			)
		...
		{name: "not a global table leaves consistency unset", run: func(t *testing.T) {
			t.Helper()
