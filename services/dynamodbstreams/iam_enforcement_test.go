...
type mockDDBSTREAMSSTREAMSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockDDBSTREAMSSTREAMSIAMBackend() *mockDDBSTREAMSSTREAMSIAMBackend {
	return &mockDDBSTREAMSSTREAMSIAMBackend{
...
func (m *mockDDBSTREAMSSTREAMSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockDDBSTREAMSSTREAMSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockDDBSTREAMSSTREAMSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockDDBSTREAMSSTREAMSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}
...
func setupDDBSTREAMSSTREAMSEnforcementServer(t *testing.T, iamBackend *mockDDBSTREAMSSTREAMSIAMBackend) *httptest.Server {
	...
	handler := dynamodbstreams.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockDDBSTREAMSSTREAMSIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIADDBSTREAMSUSER2",
			target:      "DynamoDBStreams_20120810.DescribeStream",
			body:        `{"StreamArn":"arn:aws:dynamodb:us-east-1:000000000000:table/t/stream/2026"}`,
			setupBackend: func(b *mockDDBSTREAMSSTREAMSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIADDBSTREAMSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockDDBSTREAMSSTREAMSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupDDBSTREAMSSTREAMSEnforcementServer(t, iamBackend)
			ctx := t.Context()
