...
// kinesisvideoSnapshotVersion identifies the shape of [backendSnapshot]. Bump
...
const kinesisvideoSnapshotVersion = 1

// backendSnapshot is the top-level on-disk shape for the backend. Tables holds
// one JSON-encoded array per registered table name (streams, channels -- see
// store_setup.go), produced by b.registry.SnapshotAll().
...
func (b *InMemoryBackend) Snapshot(ctx context.Context) []byte {
	...
	if err != nil {
		logger.Load(ctx).WarnContext(ctx, "kinesisvideo: snapshot table marshal failed", "error", err)

		return nil
	}

	snap := backendSnapshot{
		Version: kinesisvideoSnapshotVersion,
		Tables:  tables,
	}

	return persistence.MarshalSnapshot(ctx, "kinesisvideo", &snap)
}
...
func (b *InMemoryBackend) Restore(ctx context.Context, data []byte) error {
	...
	if err := persistence.UnmarshalSnapshot(ctx, "kinesisvideo", data, &snap); err != nil {
		return err
	}
	...
	if snap.Version != kinesisvideoSnapshotVersion {
		logger.Load(ctx).WarnContext(ctx,
			"kinesisvideo: discarding incompatible snapshot version, starting empty",
			"gotVersion", snap.Version, "wantVersion", kinesisvideoSnapshotVersion)
	...
	if err := b.registry.RestoreAll(snap.Tables); err != nil {
		return fmt.Errorf("kinesisvideo: restore snapshot tables: %w", err)
	}
