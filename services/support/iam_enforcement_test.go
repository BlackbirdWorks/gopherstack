...
type mockSUPPORTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockSUPPORTIAMBackend() *mockSUPPORTIAMBackend {
	return &mockSUPPORTIAMBackend{
...
func (m *mockSUPPORTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockSUPPORTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockSUPPORTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockSUPPORTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupSUPPORTEnforcementServer(t *testing.T, iamBackend *mockSUPPORTIAMBackend) *httptest.Server {
	t.Helper()

	backend := support.NewInMemoryBackend()
	handler := support.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockSUPPORTIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIASUPPORTUSER2",
			target:      "AWSSupport_20130415.CreateCase",
			body:        `{"subject":"test case","communicationBody":"help"}`,
			setupBackend: func(b *mockSUPPORTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIASUPPORTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockSUPPORTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupSUPPORTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/support/aws4_request, SignedHeaders=host, Signature=mock",
			)
