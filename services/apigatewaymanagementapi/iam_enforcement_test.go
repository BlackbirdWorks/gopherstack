...
type mockAPIGATEWAYMANAGEMENTAPIIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockAPIGATEWAYMANAGEMENTAPIIAMBackend() *mockAPIGATEWAYMANAGEMENTAPIIAMBackend {
	return &mockAPIGATEWAYMANAGEMENTAPIIAMBackend{
...
func (m *mockAPIGATEWAYMANAGEMENTAPIIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockAPIGATEWAYMANAGEMENTAPIIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockAPIGATEWAYMANAGEMENTAPIIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockAPIGATEWAYMANAGEMENTAPIIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}
...
func setupAPIGATEWAYMANAGEMENTAPIEnforcementServer(
	...
	_, _ = backend.CreateConnection("conn-1", "10.0.0.1", "agent", nil)
	handler := apigatewaymanagementapi.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	scopedPolicy := `{
		...
		"Statement":[
			{
				"Effect":"Allow",
				"Action":["apigatewaymanagementapi:GetConnection"],
				"Resource":["*"]
			}
		]
	}`
	...
	tests := []struct {
		setupBackend  func(b *mockAPIGATEWAYMANAGEMENTAPIIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAAPIGATEWAYMANAGEMENTAPIUSER1",
			method:      "GET",
			path:        "/@connections/conn-1",
			body:        ``,
			setupBackend: func(b *mockAPIGATEWAYMANAGEMENTAPIIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAAPIGATEWAYMANAGEMENTAPIUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAAPIGATEWAYMANAGEMENTAPIUSER2",
			method:      "POST",
			path:        "/@connections/conn-1",
			body:        `hello`,
			setupBackend: func(b *mockAPIGATEWAYMANAGEMENTAPIIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAAPIGATEWAYMANAGEMENTAPIUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockAPIGATEWAYMANAGEMENTAPIIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupAPIGATEWAYMANAGEMENTAPIEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/execute-api/aws4_request, SignedHeaders=host, Signature=mock",
			)
