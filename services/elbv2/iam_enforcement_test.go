...
type mockELBV2IAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockELBV2IAMBackend() *mockELBV2IAMBackend {
	return &mockELBV2IAMBackend{
...
func (m *mockELBV2IAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockELBV2IAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockELBV2IAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockELBV2IAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupELBV2EnforcementServer(t *testing.T, iamBackend *mockELBV2IAMBackend) *httptest.Server {
	t.Helper()
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockELBV2IAMBackend)
		name          string
		accessKeyID   string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAELBV2USER2",
			body:        "Action=CreateLoadBalancer&Name=test-lb&Version=2015-12-01",
			setupBackend: func(b *mockELBV2IAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAELBV2USER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockELBV2IAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupELBV2EnforcementServer(t, iamBackend)
			ctx := t.Context()
