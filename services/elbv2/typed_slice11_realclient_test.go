...
// TestTypedSlice11RealClient drives elbv2's typed-coverage-blind ops
// (gopherstack-n3zi slice 11) through the real aws-sdk-go-v2 client.
func TestTypedSlice11RealClient(t *testing.T) {
	t.Parallel()

	t.Run("account limits and ssl policies", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("target group attributes and modify", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("listener attributes", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("modify rule and set rule priorities", func(t *testing.T) {
		t.Parallel()
		...
		// The real response wire always nests path-pattern values under
		// PathPatternConfig, not the deprecated top-level Values field
		// (confirmed against the pinned SDK's deserializer), even though a
		// request may set the deprecated field.
	...
	t.Run("trust store lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("get resource policy", func(t *testing.T) {
		t.Parallel()
