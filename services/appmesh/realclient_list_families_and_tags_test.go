...
// TestRealClient_ListFamiliesAndTags drives every appmesh op that a real
// aws-sdk-go-v2 client had never exercised before: the six
...
// ListTagsForResource).
func TestRealClient_ListFamiliesAndTags(t *testing.T) {
	t.Parallel()
	cases := []struct {
		run  func(t *testing.T)
		name string
	}{
		{name: "list_families", run: func(t *testing.T) {
			t.Helper()
