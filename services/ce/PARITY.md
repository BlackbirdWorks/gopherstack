...
gaps:
  - "gopherstack-43o8 reqfieldscan validation pass (2026-08-30), AFTER fixing 5 tool blind spots that were suppressing 3 of these findings entirely: 68 request fields remain flagged unread across 23 ops, hand-sampled and dominated by ONE systemic pattern rather than isolated bugs -- pagination cursors (NextPageToken/MaxResults/PageSize/NextToken, ~30 of the 68) and result-shaping params (Filter/GroupBy/SortBy/Metric(s)/TimePeriod, ~24 of the 68) parsed off the wire on ops like , then never applied. A generic paginateList[T] helper already exists and is already used correctly by GetAnomalies/ListCostCategoryDefinitions/GetAnomalyMonitors/GetAnomalySubscriptions -- this is a real, mechanical, not-yet-done wiring gap on the remaining ops, not a fabricated one. Deliberately NOT fixed in this pass (out of scope: retrofitting ~15-20 ops individually, several of which -- GetCostAndUsageWithResources.ResultsByTime, ListCostCategoryResourceAssociations -- are already documented above as legitimately-always-empty for lack of per-resource ledger state, so pagination there may be moot until that state exists). Candidate for a dedicated follow-up pass; the remaining smaller findings (AccountScope on GetReservationPurchaseRecommendation, ResourceTagFilter/CostCategoryArn on ListCostCategoryResourceAssociations, RecommendationIDs on ListSavingsPlansPurchaseRecommendationGeneration, AnalysisStatus on ListCommitmentPurchaseAnalyses, EffectiveOn on Describe/ListCostCategoryDefinitions) were not individually hand-verified either. (bd: needs issue)"
