...
type mockCEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCEIAMBackend() *mockCEIAMBackend {
	return &mockCEIAMBackend{
...
func (m *mockCEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCEEnforcementServer(t *testing.T, iamBackend *mockCEIAMBackend) *httptest.Server {
	t.Helper()

	backend := ce.NewInMemoryBackend("000000000000", "us-east-1")
	handler := ce.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			accessKeyID: "AKIACEUSER2",
			target:      "AWSInsightsIndexService.CreateCostCategoryDefinition",
			body:        `{"Name":"test-cat","RuleVersion":"CostCategoryExpression.v1","Rules":[]}`,
			setupBackend: func(b *mockCEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIACEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/ce/aws4_request, SignedHeaders=host, Signature=mock",
			)
