...
type mockVPCLATTICEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockVPCLATTICEIAMBackend() *mockVPCLATTICEIAMBackend {
	return &mockVPCLATTICEIAMBackend{
...
func (m *mockVPCLATTICEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockVPCLATTICEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockVPCLATTICEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockVPCLATTICEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupVPCLATTICEEnforcementServer(t *testing.T, iamBackend *mockVPCLATTICEIAMBackend) *httptest.Server {
	t.Helper()

	backend := vpclattice.NewInMemoryBackend("000000000000", "us-east-1")
	handler := vpclattice.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	scopedPolicy := `{
		...
		"Statement":[
			{
				"Effect":"Allow",
				"Action":["vpclattice:ListServices"],
				"Resource":["*"]
			}
		]
	}`
	...
	tests := []struct {
		setupBackend  func(b *mockVPCLATTICEIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAVPCLATTICEUSER1",
			method:      "GET",
			path:        "/services",
			body:        ``,
			setupBackend: func(b *mockVPCLATTICEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAVPCLATTICEUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAVPCLATTICEUSER2",
			method:      "POST",
			path:        "/services",
			body:        `{"name":"test-svc"}`,
			setupBackend: func(b *mockVPCLATTICEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAVPCLATTICEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockVPCLATTICEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupVPCLATTICEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/vpc-lattice/aws4_request, SignedHeaders=host, Signature=mock",
			)
