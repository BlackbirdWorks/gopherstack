...
## 2026-09-18 (gopherstack-21my: per-item field sweep, forecast)

Per-item sweep of `summaryOutput`/`resourceOutput` (field-level, not
wrapper-key). Fixed 3: `ListDatasetImportJobs` never emitted the real,
reachable `Message` field; every `Describe*`/Create-echo leaked a
