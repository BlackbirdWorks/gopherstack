...
type mockPOLLYIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockPOLLYIAMBackend() *mockPOLLYIAMBackend {
	return &mockPOLLYIAMBackend{
...
func (m *mockPOLLYIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockPOLLYIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockPOLLYIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockPOLLYIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupPOLLYEnforcementServer(t *testing.T, iamBackend *mockPOLLYIAMBackend) *httptest.Server {
	t.Helper()

	backend := polly.NewInMemoryBackend()
	handler := polly.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockPOLLYIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAPOLLYUSER1",
			method:      "GET",
			path:        "/v1/voices",
			body:        ``,
			setupBackend: func(b *mockPOLLYIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAPOLLYUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAPOLLYUSER2",
			method:      "POST",
			path:        "/v1/speech",
			body:        `{"Text":"hello","OutputFormat":"mp3","VoiceId":"Joanna"}`,
			setupBackend: func(b *mockPOLLYIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAPOLLYUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockPOLLYIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupPOLLYEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/polly/aws4_request, SignedHeaders=host, Signature=mock",
			)
