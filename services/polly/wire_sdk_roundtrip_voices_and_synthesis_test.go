...
// TestRealClient_VoicesAndSynthesis drives polly's typed-coverage-blind ops
// (gopherstack-n3zi) through the real aws-sdk-go-v2 client.
func TestRealClient_VoicesAndSynthesis(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "speech synthesis task lifecycle",
			run: func(t *testing.T) {
				t.Helper()
