...
// TestRealClient_UsersConfigBroker drives mq's remaining typed-coverage-
...
func TestRealClient_UsersConfigBroker(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				out, err := client.CreateBroker(ctx, &mqsdk.CreateBrokerInput{
					BrokerName:         aws.String("s23-tags-broker"),
					EngineType:         "ACTIVEMQ",
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				brokerOut, err := client.CreateBroker(ctx, &mqsdk.CreateBrokerInput{
					BrokerName:         aws.String("s23-users-broker"),
					EngineType:         "ACTIVEMQ",
					EngineVersion:      aws.String("5.15.14"),
					HostInstanceType:   aws.String("mq.t3.micro"),
					DeploymentMode:     "SINGLE_INSTANCE",
		...
		{
			name: "configuration revision and delete",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "broker engine types and instance options and shared resources",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "promote a replica broker to primary",
			run: func(t *testing.T) {
				t.Helper()
				...
				descBefore, err := client.DescribeBroker(
					ctx,
					&mqsdk.DescribeBrokerInput{BrokerId: aws.String(replicaID)},
				)
				require.NoError(t, err)
				...
				descAfter, err := client.DescribeBroker(
					ctx,
					&mqsdk.DescribeBrokerInput{BrokerId: aws.String(replicaID)},
				)
				require.NoError(t, err)
		...
		{
			name: "promote a non-replica broker is rejected",
			run: func(t *testing.T) {
				t.Helper()
				...
				brokerOut, err := client.CreateBroker(ctx, &mqsdk.CreateBrokerInput{
					BrokerName:         aws.String("s23-not-replica-broker"),
					EngineType:         "ACTIVEMQ",
					EngineVersion:      aws.String("5.15.14"),
					HostInstanceType:   aws.String("mq.t3.micro"),
					DeploymentMode:     "SINGLE_INSTANCE",
