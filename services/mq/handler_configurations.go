...
// createConfigurationInput has no Description member: real CreateConfigurationInput
// (mq@v1.39.4 api_op_CreateConfiguration.go) declares none -- Description is only
// settable via UpdateConfiguration, which creates a new revision.
...
func (h *Handler) handleCreateConfiguration(c *echo.Context, body []byte) error {
	...
	cfg, err := h.Backend.CreateConfiguration(
		in.Name,
		fmt.Sprintf("Auto-generated by Amazon MQ on %s", time.Now().UTC().Format(time.RFC3339)),
		in.EngineType,
		in.EngineVersion,
