...
overall: A            # 2026-07-31 (gopherstack-eje5): found and fixed a severe CreateBucket
                       ...
                       # below). One related sub-bug (c.String(204,"") returning ErrBodyNotAllowed)
                       # was fixed in handler_bucket.go but left open in 4 other handler files -- see
                       # gaps.
                       #
...
ops:
  ...
  DeleteBucket: {wire: ok, errors: ok, state: ok, persist: ok, note: "THIS PASS: ghost-map-row leak fix -- delete only removed the bucket row itself, leaving lifecycle/policy/tagging/versioning/replication/generic-resource-tags behind forever. Now cascade-cleans all five. ALSO THIS PASS (gopherstack-eje5): the handler used c.String(http.StatusNoContent, \"\"), which -- like handleDeleteBucketLifecycleConfiguration/handleDeleteBucketPolicy/handleDeleteBucketTagging -- returned http.ErrBodyNotAllowed on every real call (Write after a 204 WriteHeader is rejected by both httptest.ResponseRecorder and a real net/http server). Fixed to c.NoContent(http.StatusNoContent), matching DeleteBucketReplication's already-correct handler."}
...
gaps:
  ...
  - (NEW 2026-07-31, gopherstack-eje5) c.String(http.StatusNoContent, "") -- which returns http.ErrBodyNotAllowed on every real call, since Write after a 204 WriteHeader is rejected by both httptest.ResponseRecorder and a real net/http server -- remains UNFIXED in 8 more handlers outside handler_bucket.go (where the same pattern WAS fixed this pass, see DeleteBucket ops row): handler_access_grants.go (4 call sites), handler_object_lambda.go (2), handler_jobs.go (1), handler_access_points.go (1). Not fixed this pass -- out of scope for the account-mismatch bug this pass was tracking down -- but it is the identical real defect and should be swept in one pass across the file (grep `c.String(http.StatusNoContent` to find them).
