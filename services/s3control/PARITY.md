...
ops:
  ...
  PutMultiRegionAccessPointPolicy: {wire: ok, errors: ok, state: ok, persist: ok, note: "route used '/put_policy' (underscore); real SDK URI is '/put-policy' (hyphen) -- UNREACHABLE via real SDK. Fixed. 2026-09-07 (gopherstack-0c0d): FIXED the wrong-sentinel bug found-not-fixed during the gopherstack-l498 investigation -- on a missing MRAP this op returned the package-level ErrNotFound (\"NoSuchPublicAccessBlockConfiguration\", s3control's public-access-block sentinel) instead of errMRAPNotFound (\"NoSuchMultiRegionAccessPoint\"). Doc comment (api_op_PutMultiRegionAccessPointPolicy.go, s3control@v1.73.4) says nothing about async completion or a request token affecting error visibility -- the missing-MRAP check runs synchronously in the backend before any token is minted, so the bug was fully observable. Confirmed via `awk \"/deserializeOpErrorPutMultiRegionAccessPointPolicy\\(/,/^}/\" deserializers.go | grep -oE '\"[A-Za-z0-9]+\"'` that the op's deserializer declares no modeled error (bare `switch { default: ... }`, only a placeholder \"UnknownError\" literal that is overwritten by whatever <Code> the body actually carries) -- same as its sibling SubmitMultiRegionAccessPointRoutes. With no SDK-declared code to defer to, reused errMRAPNotFound, the sentinel every other MRAP op in this file already uses (Get/Delete/Describe/GetPolicy/GetPolicyStatus/GetRoutes/SubmitRoutes) -- consistent with the sibling fix. New test TestHandler_PutMultiRegionAccessPointPolicy_MRAPExistence pins both halves: missing MRAP gets 404 + NoSuchMultiRegionAccessPoint (and explicitly asserts the wrong code is absent), and an existing MRAP's policy is stored and reads back correctly via a real GetMultiRegionAccessPointPolicy request -- hand-verified to fail against the unfixed backend (3 failures: this test's missing-MRAP subtest, TestHandler_PutMultiRegionAccessPointPolicy's put_policy_missing case, and TestBackend_MRAP_Operations's put_policy_missing_mrap case), and separately hand-verified that gutting the policy assignment while leaving the error path fixed fails the existing-MRAP subtest, so the round-trip half is not vacuous. Corrected 2 pre-existing tests that were exercising this without catching it: TestBackend_MRAP_Operations's 'put_policy_missing_mrap' case (bare require.Error -- now require.ErrorContains(...,\"NoSuchMultiRegionAccessPoint\")) and TestHandler_PutMultiRegionAccessPointPolicy's 'put_policy_missing' case (asserted only the 404 status, no wantBody -- now asserts the wire error code)."}
...
Adjudicated all 9 items_still_open + 3 deferred entries against HEAD. Removed
6 as stale (already resolved, self-referential, or superseded by a later
entry in the same list -- the kx5v DeleteBucket "no defect" verdict, the
dead sync-DELETE-route note, the DeleteAccessGrantsInstance precondition
correction, the ObjectLambda Alias staleness note, the sampled-types-closed
note, and the c.String/c.NoContent correction). Fixed 1: removed the unused
`s3control.ErrAlreadyExists` sentinel (errors.go), confirmed dead via a
repo-wide grep; its real duplicate-CreateAccessPoint behavior can't be
verified (the SDK's error deserializer for this op has no modeled cases), so
wiring it up would be a guess. Kept 2, tightened to one paragraph each:
region-isolation (uniform single-region design, real but out of a
single-pass budget) and ListAccessPoints.DataSourceType (dead plumbing --
no non-bucket AP type modeled). Deferred entries kept as-is, tightened.

Gates: `gofmt -l` clean; `go build ./services/s3control/...` and `go vet
./services/s3control/...` clean; `go test -race -count=1
