...
func (h *Handler) handleListAccessPointsForObjectLambda(c *echo.Context) error {
	...
	// olAPItem mirrors types.ObjectLambdaAccessPoint. Its real Alias field
	// (auto-generated by AWS in a "<random>-ol-s3alias" format, distinct
	// from regular access points' "<name>-<accountid>-s3alias" scheme — see
	// CreateAccessPoint in access_points.go) is omitted (GAP, not
	// fabricated): ObjectLambdaAccessPoint (models.go) tracks no alias data,
	// and the real generation algorithm isn't verified closely enough to
	// synthesize safely.
...
// handleGetAccessPointConfigurationForObjectLambda. Real
...
// only recognizes "Configuration" at the top level; the real
// ObjectLambdaConfiguration type — SupportingAccessPoint,
// TransformationConfigurations, AllowedFeatures, CloudWatchMetricsEnabled —
// lives inside that element, not as its name). The payload is
// captured/replayed as raw inner XML (createJobXMLCapture, shared with
// CreateJob's Manifest/Operation/Report) so the nested structure round-trips
// intact.
