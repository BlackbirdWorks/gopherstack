...
type mockACCOUNTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockACCOUNTIAMBackend() *mockACCOUNTIAMBackend {
	return &mockACCOUNTIAMBackend{
...
func (m *mockACCOUNTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockACCOUNTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockACCOUNTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockACCOUNTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupACCOUNTEnforcementServer(t *testing.T, iamBackend *mockACCOUNTIAMBackend) *httptest.Server {
	t.Helper()

	backend := account.NewInMemoryBackend("000000000000", "us-east-1")
	handler := account.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAACCOUNTUSER1",
			method:      "POST",
			path:        "/listRegions",
			body:        `{}`,
			setupBackend: func(b *mockACCOUNTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAACCOUNTUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockACCOUNTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupACCOUNTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/account/aws4_request, SignedHeaders=host, Signature=mock",
			)
