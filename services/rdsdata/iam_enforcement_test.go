...
type mockRDSDATAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockRDSDATAIAMBackend() *mockRDSDATAIAMBackend {
	return &mockRDSDATAIAMBackend{
...
func (m *mockRDSDATAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockRDSDATAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockRDSDATAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockRDSDATAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupRDSDATAEnforcementServer(t *testing.T, iamBackend *mockRDSDATAIAMBackend) *httptest.Server {
	t.Helper()

	backend := rdsdata.NewInMemoryBackend("000000000000", "us-east-1")
	handler := rdsdata.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockRDSDATAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupRDSDATAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/rds-data/aws4_request, SignedHeaders=host, Signature=mock",
			)
