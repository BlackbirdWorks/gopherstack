...
type mockOPSWORKSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockOPSWORKSIAMBackend() *mockOPSWORKSIAMBackend {
	return &mockOPSWORKSIAMBackend{
...
func (m *mockOPSWORKSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockOPSWORKSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockOPSWORKSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockOPSWORKSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupOPSWORKSEnforcementServer(t *testing.T, iamBackend *mockOPSWORKSIAMBackend) *httptest.Server {
	t.Helper()

	backend := opsworks.NewInMemoryBackend("000000000000", "us-east-1")
	handler := opsworks.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAOPSWORKSUSER1",
			target:      "OpsWorks_20130218.DescribeStacks",
			body:        `{}`,
			setupBackend: func(b *mockOPSWORKSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAOPSWORKSUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockOPSWORKSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupOPSWORKSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/opsworks/aws4_request, SignedHeaders=host, Signature=mock",
			)
