...
gaps: []
items_still_open:
  - "CreateLayer's Attributes map (mega-batch-48, 2026-09-24) now round-trips generically -- fixed EcsClusterArn forcing aws_opsworks_ecs_cluster_layer to be destroyed and recreated on every apply. Still unmodeled: EnableAutoHealing/InstallUpdatesOnBoot-adjacent LifecycleEventConfiguration (drain-on-shutdown/shutdown-timeout), AutoAssignElasticIps/AutoAssignPublicIps, CustomInstanceProfileArn, CustomJson, UseEbsOptimizedInstances, VolumeConfigurations, CloudWatchLogsConfiguration, CustomSecurityGroupIds, Packages, CustomRecipes -- terraform's aws_opsworks_*_layer resources show a persistent (non-destructive) plan diff on these until a follow-up pass models them."
...
**2026-09-24** (mega-batch-48): fixed DescribeInstances omitting ReportedOs
(nil-pointer-crashed terraform-provider-aws's resourceInstanceRead) and
CreateLayer dropping the Attributes map (EcsClusterArn wasn't round-tripping,
forcing perpetual replace). Coverage for ecs_cluster/ganglia/haproxy/
java_app/memcached/mysql/nodejs_app/php_app/rails_app/static_web layers +
instance + rds_db_instance added via terraform apply/plan/destroy.
