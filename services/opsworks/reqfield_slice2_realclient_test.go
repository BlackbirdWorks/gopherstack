...
const (
	rfd2InstanceProfileArn = "arn:aws:iam::000000000000:instance-profile/opsworks"
	rfd2ServiceRoleArn     = "arn:aws:iam::000000000000:role/opsworks"
)

// TestReqFieldSlice2OpsworksStackAttributes drives CreateStack/UpdateStack
// through the real SDK client, asserting every tier-1 reqfielddiff finding
// on those two ops (gopherstack-xhu2t slice 2) is now stored and described
// back by DescribeStacks.
func TestReqFieldSlice2OpsworksStackAttributes(t *testing.T) {
	t.Parallel()

	t.Run("create_stack_describes_back_optional_attributes", func(t *testing.T) {
		t.Parallel()
		...
		created, err := client.CreateStack(t.Context(), &opsworkssdk.CreateStackInput{
			...
			DefaultInstanceProfileArn: aws.String(rfd2InstanceProfileArn),
			ServiceRoleArn:            aws.String(rfd2ServiceRoleArn),
	...
	t.Run("update_stack_applies_optional_attributes", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("update_stack_zero_values_leave_attributes_unchanged", func(t *testing.T) {
		t.Parallel()
...
// TestReqFieldSlice2OpsworksCloneStack drives CloneStack through the real
...
// comments).
func TestReqFieldSlice2OpsworksCloneStack(t *testing.T) {
	t.Parallel()

	t.Run("missing_required_service_role_arn_is_rejected", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("explicit_overrides_win_over_source", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, "arn:aws:iam::000000000000:instance-profile/clone", aws.ToString(got.DefaultInstanceProfileArn))
	...
	t.Run("omitted_attributes_inherit_from_source", func(t *testing.T) {
		t.Parallel()
		...
		got := described.Stacks[0]
		assert.Equal(t, rfd2InstanceProfileArn, aws.ToString(got.DefaultInstanceProfileArn))
...
// TestReqFieldSlice2OpsworksInstanceAttributes drives CreateInstance and
// UpdateInstance's tier-1 optional-surface findings through the real SDK
// client.
func TestReqFieldSlice2OpsworksInstanceAttributes(t *testing.T) {
	t.Parallel()

	t.Run("create_instance_describes_back_optional_attributes", func(t *testing.T) {
		t.Parallel()

		client := newTestClient(t)
		stackID, layerID := rfd2StackWithLayer(t, client)
	...
	t.Run("update_instance_applies_optional_attributes", func(t *testing.T) {
		t.Parallel()
...
// TestReqFieldSlice2OpsworksLayerAttributes drives CreateLayer and
// UpdateLayer's tier-1 InstallUpdatesOnBoot finding through the real SDK
// client.
func TestReqFieldSlice2OpsworksLayerAttributes(t *testing.T) {
	t.Parallel()

	t.Run("create_layer_describes_back_install_updates_on_boot", func(t *testing.T) {
		t.Parallel()

		client := newTestClient(t)
		stackID := rfd2Stack(t, client)
	...
	t.Run("update_layer_applies_install_updates_on_boot", func(t *testing.T) {
		t.Parallel()
...
// TestReqFieldSlice2OpsworksDeploymentCustomJSON drives CreateDeployment's
// tier-1 CustomJson finding through the real SDK client.
func TestReqFieldSlice2OpsworksDeploymentCustomJSON(t *testing.T) {
	t.Parallel()
...
func rfd2Stack(t *testing.T, client *opsworkssdk.Client) string {
	t.Helper()
...
func rfd2StackWithLayer(t *testing.T, client *opsworkssdk.Client) (string, string) {
	t.Helper()
