...
// TestRealClient_ResourceCoverage drives every op the census still
// listed as uncovered before this pass (gopherstack-n3zi typed-client
// coverage). Each case builds its own fresh client/backend and
// full precondition chain so cases can run in parallel with each other.
	...
	cases := []struct {
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				_, err = client.StartInstance(
					t.Context(),
					&opsworkssdk.StartInstanceInput{InstanceId: aws.String(instanceID)},
				)
				require.NoError(t, err)
				...
				_, err = client.StopInstance(
					t.Context(),
					&opsworkssdk.StopInstanceInput{InstanceId: aws.String(instanceID)},
				)
				require.NoError(t, err)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				described, err := client.DescribeApps(
					t.Context(),
					&opsworkssdk.DescribeAppsInput{AppIds: []string{appID}},
				)
				require.NoError(t, err)
				...
				described, err = client.DescribeApps(
					t.Context(),
					&opsworkssdk.DescribeAppsInput{StackId: stack.StackId},
				)
				require.NoError(t, err)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				described, err := client.DescribeTimeBasedAutoScaling(
					t.Context(),
					&opsworkssdk.DescribeTimeBasedAutoScalingInput{
						InstanceIds: []string{aws.ToString(inst.InstanceId)},
					},
				)
				require.NoError(t, err)
