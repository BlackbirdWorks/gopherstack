...
type mockROLESANYWHEREIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockROLESANYWHEREIAMBackend() *mockROLESANYWHEREIAMBackend {
	return &mockROLESANYWHEREIAMBackend{
...
func (m *mockROLESANYWHEREIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockROLESANYWHEREIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockROLESANYWHEREIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockROLESANYWHEREIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupROLESANYWHEREEnforcementServer(t *testing.T, iamBackend *mockROLESANYWHEREIAMBackend) *httptest.Server {
	t.Helper()

	backend := rolesanywhere.NewInMemoryBackend("000000000000", "us-east-1")
	handler := rolesanywhere.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockROLESANYWHEREIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAROLESANYWHEREUSER1",
			method:      "GET",
			path:        "/profiles",
			body:        ``,
			setupBackend: func(b *mockROLESANYWHEREIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAROLESANYWHEREUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAROLESANYWHEREUSER2",
			method:      "POST",
			path:        "/profiles",
			body:        `{"name":"test-p","roleArns":["arn:aws:iam::000000000000:role/r"]}`,
			setupBackend: func(b *mockROLESANYWHEREIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAROLESANYWHEREUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockROLESANYWHEREIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupROLESANYWHEREEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/rolesanywhere/aws4_request, SignedHeaders=host, Signature=mock",
			)
