...
type mockRESOURCEGROUPSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockRESOURCEGROUPSIAMBackend() *mockRESOURCEGROUPSIAMBackend {
	return &mockRESOURCEGROUPSIAMBackend{
...
func (m *mockRESOURCEGROUPSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockRESOURCEGROUPSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockRESOURCEGROUPSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockRESOURCEGROUPSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupRESOURCEGROUPSEnforcementServer(t *testing.T, iamBackend *mockRESOURCEGROUPSIAMBackend) *httptest.Server {
	t.Helper()

	backend := resourcegroups.NewInMemoryBackend("000000000000", "us-east-1")
	handler := resourcegroups.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockRESOURCEGROUPSIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIARESOURCEGROUPSUSER1",
			target:      "ResourceGroups.ListGroups",
			body:        `{}`,
			setupBackend: func(b *mockRESOURCEGROUPSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIARESOURCEGROUPSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIARESOURCEGROUPSUSER2",
			target:      "ResourceGroups.CreateGroup",
			body:        `{"Name":"test-group"}`,
			setupBackend: func(b *mockRESOURCEGROUPSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIARESOURCEGROUPSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockRESOURCEGROUPSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupRESOURCEGROUPSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/resource-groups/aws4_request, SignedHeaders=host, Signature=mock",
			)
