...
func newTestSlice33DocDBClient(t *testing.T) *docdbsdk.Client {
	t.Helper()
...
// TestSlice33_DocDB_RealClient drives every gopherstack-n3zi typed-slice-33
// uncovered docdb op through the real aws-sdk-go-v2 client
// (newTestDocDBClient, shared with handler_sdk_roundtrip_test.go).
func TestSlice33_DocDB_RealClient(t *testing.T) {
	t.Parallel()
...
func testClusterParameterGroupsRealClient(t *testing.T) {
	...
	client := newTestSlice33DocDBClient(t)
	ctx := t.Context()
