...
// This file backs the "DescribeEvents always returns an empty event list"
// gap identified in PARITY.md: there was no event log backing this backend
// at all, so DescribeEvents could never report anything regardless of what a
// caller had actually done -- mirroring the already-completed neptune
// service's identical fix (services/neptune/events.go). recordEvent is
// called from the key cluster/instance/snapshot lifecycle mutators
// (create/delete/start/stop/failover) while they already hold the backend
// write lock, appending a real entry DescribeEvents can filter and return.
...
// retains events for a limited window; this backend retains the most recent
// N entries instead of tracking wall-clock age).
const maxEventsLogPerRegion = 500

// DocDB event source types, matching types.SourceType.
const (
...
func (b *InMemoryBackend) recordEvent(region, sourceID, sourceType, sourceArn, message string, categories ...string) {
...
// (DescribeEventsInput: SourceIdentifier/SourceType/StartTime/EndTime/
// Duration/EventCategories -- Filters is documented "not currently
// supported" by AWS itself, so it is intentionally not modeled here).
