...
// MGN client's DescribeSourceServers with a NextToken large enough to push
...
// instead of the typed InternalServerException classifyMGNError's default
...
func TestHandler_OversizedBodySurfacesInternalServerException(t *testing.T) {
	...
	_, err := client.DescribeSourceServers(t.Context(), &mgnsdk.DescribeSourceServersInput{
		NextToken: huge,
	}, func(o *mgnsdk.Options) { o.RetryMaxAttempts = 1 })
	require.Error(t, err)
