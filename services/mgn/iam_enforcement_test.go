...
type mockMGNIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMGNIAMBackend() *mockMGNIAMBackend {
	return &mockMGNIAMBackend{
...
func (m *mockMGNIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMGNIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMGNIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMGNIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMGNEnforcementServer(t *testing.T, iamBackend *mockMGNIAMBackend) *httptest.Server {
	t.Helper()

	backend := mgn.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	backend.InitializeService()
	handler := mgn.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockMGNIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
		...
		},
		{
			...
			accessKeyID: "AKIAMGNUSER2",
			method:      "POST",
			path:        "/InitializeService",
			body:        `{}`,
			setupBackend: func(b *mockMGNIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMGNUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMGNIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMGNEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/mgn/aws4_request, SignedHeaders=host, Signature=mock",
			)
