...
type mockMACIE2IAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMACIE2IAMBackend() *mockMACIE2IAMBackend {
	return &mockMACIE2IAMBackend{
...
func (m *mockMACIE2IAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMACIE2IAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMACIE2IAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMACIE2IAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMACIE2EnforcementServer(t *testing.T, iamBackend *mockMACIE2IAMBackend) *httptest.Server {
	t.Helper()

	backend := macie2.NewInMemoryBackend("000000000000", "us-east-1")
	handler := macie2.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockMACIE2IAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
		...
		},
		{
			...
			accessKeyID: "AKIAMACIE2USER2",
			method:      "POST",
			path:        "/custom-data-identifiers",
			body:        `{"name":"test-cdi","regex":"[0-9]+"}`,
			setupBackend: func(b *mockMACIE2IAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMACIE2USER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMACIE2IAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMACIE2EnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/macie2/aws4_request, SignedHeaders=host, Signature=mock",
			)
