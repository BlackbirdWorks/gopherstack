...
ops:
  ...
  SearchResources: {wire: gap, errors: ok, state: ok, persist: n/a, note: "DISCLOSED, not fixed this pass (constraint sweep): BucketCriteria (Includes/Excludes of SimpleCriterion{Key: ACCOUNT_ID|AUTOMATED_DISCOVERY_MONITORING_STATUS|S3_BUCKET_EFFECTIVE_PERMISSION|S3_BUCKET_NAME|S3_BUCKET_SHARED_ACCESS, Comparator EQ|NE} or TagCriterion -- types.SearchResourcesSimpleCriterion doc comment) and SortCriteria/maxResults/nextToken are all parsed by the handler and passed to the backend, but the backend signature discards them (`_, _, _`) and always returns an empty list. The doc-comment justification this file uses elsewhere for a genuinely-empty family (e.g. mgn's mapper segments) does not apply here: SearchResources reads the same s3Buckets store DescribeBuckets already filters correctly, so the data to honor SimpleCriterion exists -- this is an unimplemented feature, not a structural gap. Correctly filtering it needs a second, differently-shaped criteria engine (And[]{SimpleCriterion|TagCriterion} vs DescribeBuckets' flat per-property map) than the one this pass built for DescribeBuckets; left for a follow-up rather than rushed alongside it."}
