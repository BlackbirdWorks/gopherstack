...
type mockLIGHTSAILIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockLIGHTSAILIAMBackend() *mockLIGHTSAILIAMBackend {
	return &mockLIGHTSAILIAMBackend{
...
func (m *mockLIGHTSAILIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockLIGHTSAILIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockLIGHTSAILIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockLIGHTSAILIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupLIGHTSAILEnforcementServer(t *testing.T, iamBackend *mockLIGHTSAILIAMBackend) *httptest.Server {
	t.Helper()

	backend := lightsail.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := lightsail.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIALIGHTSAILUSER1",
			target:      "Lightsail_20161128.GetInstances",
			body:        `{}`,
			setupBackend: func(b *mockLIGHTSAILIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIALIGHTSAILUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockLIGHTSAILIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupLIGHTSAILEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/lightsail/aws4_request, SignedHeaders=host, Signature=mock",
			)
