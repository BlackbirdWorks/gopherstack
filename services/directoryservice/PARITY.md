...
last_audit_commit: 0c1472972  # 2026-09-24 mega-batch-39 terraform coverage; prior: 44bff591b
last_audit_date: 2026-09-24  # prior: 2026-09-19 -- 2026-09-19 over-wide-response sweep (gopherstack, see Notes) re-verified ListADAssessments/ListCertificates/ListIpRoutes/ListSchemaExtensions member-by-member against directoryservice@v1.41.4
...
ops:
  ...
  ListIpRoutes: {wire: partial, errors: ok, state: ok, persist: ok, note: "AddedDateTime epoch fix. Re-verified 2026-09-19 (over-wide-response sweep, gopherstack) against types.IpRouteInfo: the 5 emitted keys (DirectoryId/CidrIp/Description/AddedDateTime/IpRouteStatusMsg) have no leaks. CidrIpv6 and IpRouteStatusReason are unsourced -- IpRoute (models.go) has no IPv6 field at all (AddIpRoutes only ever reads CidrIp) and no status-reason field (see items_still_open); handler_ip_routes.go:105-114."}
  ...
  ListSchemaExtensions: {wire: partial, errors: ok, state: ok, persist: ok, note: "StartDateTime/EndDateTime epoch fix. Re-verified 2026-09-19 (over-wide-response sweep, gopherstack) against types.SchemaExtensionInfo: the 6 emitted keys have no leaks. SchemaExtensionStatusReason is unsourced -- SchemaExtension (models.go) has no status-reason field, same class as DomainController.StatusReason (see items_still_open); handler_schema_extensions.go:112-122."}
...
gaps: []
items_still_open:
  - "2026-09-19 (over-wide-response sweep, gopherstack): IpRouteInfo.CidrIpv6/IpRouteStatusReason (ListIpRoutes) and SchemaExtensionInfo.SchemaExtensionStatusReason (ListSchemaExtensions) are unsourced -- IpRoute/SchemaExtension (models.go) have no such fields, and AddIpRoutes never accepts an IPv6 CIDR at all. Same class of gap as DomainController.StatusReason above (bd: file follow-up)"
  ...
  - "2026-09-24 (mega-batch-39): aws_directory_service_shared_directory_accepter's real terraform apply confirms the consumer-side gap already recorded above (no mirrored Directory entry) as an observable failure, not just a described limitation: after AcceptSharedDirectory succeeds, the provider's own create-waiter polls DescribeDirectories(sharedDirectoryId) and gets \"waiting for Directory Service Shared Directory (d-...) accept: couldn't find resource (21 retries)\" forever. Left out of the mega-batch-39 fixture (owner-side aws_directory_service_shared_directory alone is covered and applies cleanly). aws_directory_service_trust and aws_directory_service_region were also tried: CreateTrust/AddRegion both succeed instantly via a direct SDK call, but a real terraform apply of either resource did not reach a terminal state within this session's test budget (60s+, one hit a secondary \"listing tags for Directory Service Directory: ListTagsForResource ... EntityDoesNotExistException\") -- not root-caused further; left out, not fabricated as working. (bd: unfiled)"
...
leaks: {status: clean, note: "fixed 2026-09-24 (background-timer audit): transitionDirectoryToActive and RestoreFromSnapshot used to spawn a bare `go func(){ time.Sleep(...); ... }()` per call with no lifecycle tie -- untracked, could pile up under concurrent CreateDirectory/RestoreFromSnapshot, and was never stopped on service shutdown. Both now schedule on a *worker.Group (b.work, rooted at the service's JanitorCtx via NewInMemoryBackendWithContext) whose After() timers are tracked and stopped by Close(); Handler.Shutdown reaches Close via a `interface{ Close() }` assertion (autoscaling's precedent). lazy_lifecycle_test.go (testing/synctest) proves the transitional stage holds pre-deadline, the final stage appears post-deadline, and Close cancels a still-pending transition. No new persisted fields; leak_main_test.go's testleak.VerifyTestMain and -race stay clean."}
---
...
Real bug found and fixed: `newDirectoryID`/`newSnapshotID` and five sibling ID minters (`a-`/`dc-`/`c-`/`t-`/`e-` prefixes) took `uuid.NewString()[:10]`, which always includes that UUID's own `-` separator (e.g. `d-abadaa11-1`) -- terraform-provider-aws validates `directory_id` client-side against `^d-[0-9a-f]{10}$` and rejected every ID this backend ever minted with "Invalid Attribute Value Match". Fixed via a shared `newHexID` helper stripping hyphens before truncating. `aws_directory_service_trust`/`_region`/`_shared_directory_accepter` left out of coverage; see items_still_open for the exact errors.

### 2026-09-19 over-wide-response sweep (gopherstack)

All four flagged List ops (ListADAssessments/ListCertificates/ListIpRoutes/
ListSchemaExtensions) already emit exactly their real Summary/Info shape --
no leaks. IpRouteInfo.CidrIpv6/StatusReason and SchemaExtensionInfo.
StatusReason are unsourced gaps (no code change; recorded in items_still_open).
