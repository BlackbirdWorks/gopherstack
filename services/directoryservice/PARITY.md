...
overall: B            # gopherstack-10hx follow-up pass (2026-07-30): hybrid-AD's SEVERE structural gap is CLOSED -- CreateHybridAD/UpdateHybridAD/DescribeHybridADUpdate now parse/build the real types.CreateHybridADInput/Output, types.UpdateHybridADInput/Output, and types.HybridUpdateActivities shapes (field-diffed against aws-sdk-go-v2/service/directoryservice@v1.41.0's types.go/serializers.go/deserializers.go/validators.go), with no fabricated fields (the invented top-level RequestId on all three ops is gone) and real state mutation (HybridSettingsDescription on DirectoryDescription is now genuinely populated by UpdateHybridAD, closing that companion gap too). HELD AT B, not raised: the SAME downgrade note this pass followed up on also cited a second, separate, still-fully-open gap -- StartADAssessment does not accept real AssessmentConfiguration input, so Assessment/AssessmentSummary are still missing ~8 real fields (CustomerDnsIps, DnsName, LastUpdateDateTime, SecurityGroupIds, SelfManagedInstanceIds, StatusCode, StatusReason, SubnetIds, Version, VpcId) -- explicitly out of scope for gopherstack-10hx (hybrid-AD wire shapes only) and not touched this pass beyond a narrow, load-bearing fix (see families). This gap alone is large enough that raising the grade back to A now would be premature; see gaps/deferred.
...
gaps:                     # known divergences NOT fixed — link bd issue ids
  ...
  - "AD-assessments (StartADAssessment/DescribeADAssessment/ListADAssessments): StartADAssessment does not accept the real StartADAssessmentInput.AssessmentConfiguration member (CustomerDnsIps, DnsName, InstanceIds, VpcSettings, SecurityGroupIds) at all, so none of it can ever be stored or echoed back. As a direct result, Assessment/AssessmentSummary are still missing these real members entirely: CustomerDnsIps, DnsName, LastUpdateDateTime, SecurityGroupIds, SelfManagedInstanceIds, StatusCode, StatusReason, SubnetIds, Version, VpcId. The two fields that WERE wrong (fabricated 'Region'/'AssessmentType' wire members) were fixed the same pass -- see the ops table -- but the remaining gap is real input-capture work, not a small field patch, so it's deferred rather than attempted piecemeal. STILL OPEN as of gopherstack-10hx (2026-07-30): that pass fixed hybrid-AD's wire shapes (a separate, now-closed gap) but deliberately did not attempt this one -- explicitly out of scope. One narrow, load-bearing fix WAS made to this family as a hybrid-AD dependency: Assessment.Status's real value was the non-enum string 'Completed' (types.Assessment.Status documents SUCCESS/FAILED/PENDING/IN_PROGRESS as the only valid values) -- a genuine wire-VALUE bug, not just a missing field, found because CreateHybridAD needed a real success status to gate on. Fixed to 'SUCCESS'. This is the reason directoryservice's overall grade remains B rather than returning to A."
