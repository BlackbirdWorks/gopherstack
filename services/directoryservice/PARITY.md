...
overall: B            # DirectoryDescription/DomainController field-diff closed (ConnectSettings/DesiredNumberOfDomainControllers/DnsIpv6Addrs/NetworkType/RadiusSettings/RadiusStatus/RegionsInfo now real; DomainController DnsIpAddr/DnsIpv6Addr/StatusLastUpdatedDateTime/SubnetId/VpcId now real); ConditionalForwarder DnsIpv6Addrs gap closed; AD-assessment wire fabrication removed (invented "Region"/"AssessmentType":"Operational" fields deleted, real ReportType:"CUSTOMER" substituted) -- BUT re-diffing the remaining "ok" families surfaced a SEVERE, NOT-fixed gap in hybrid-AD (CreateHybridAD/UpdateHybridAD/DescribeHybridADUpdate wire shapes substantially diverge from the real API, not just missing fields) and a large residual gap in AD-assessment (StartADAssessment still doesn't accept real AssessmentConfiguration input; Assessment/AssessmentSummary still missing ~8 real fields). Downgraded from A to B to reflect these newly-surfaced, unfixed gaps -- see gaps/deferred.
...
ops:
  ...
  StartADAssessment: {wire: gap, errors: ok, state: FIXED, persist: ok, note: "synchronous Completed; AWS is async but no client-visible divergence for polling clients (prior-pass note, still true). SEVERE finding this pass: StartADAssessmentInput.AssessmentConfiguration (CustomerDnsIps, DnsName, InstanceIds, VpcSettings, SecurityGroupIds -- all required-shape members of a real, non-optional-in-practice input struct) is not accepted by this operation's signature at all, so none of that configuration can ever be echoed back by Describe/List. FIXED the specific fabrication this caused: AssessType was hardcoded to the string 'Operational', which is not a valid ReportType enum value (real values: CUSTOMER, SYSTEM) and was wired onto the wire under the WRONG field name 'AssessmentType' (the real DirectoryDescription/Assessment member is 'ReportType', not 'AssessmentType' -- 'AssessmentType' does not exist in the SDK at all). Now emits the real field name ReportType with the real value CUSTOMER (correct because every assessment this backend creates originates from an explicit customer-initiated StartADAssessment call, never from the SYSTEM-triggered path CreateHybridAD would use in real AWS). AssessmentConfiguration input capture is NOT implemented -- see gaps."}
  DeleteADAssessment: {wire: ok, errors: ok, state: ok, persist: ok}
  DescribeADAssessment: {wire: FIXED, errors: ok, state: ok, persist: ok, note: "StartTime epoch fix (prior pass). This pass: removed the fabricated 'Region' wire field (Assessment/AssessmentSummary have no such member in the real SDK) and fixed the AssessmentType->ReportType/Operational->CUSTOMER fabrication described under StartADAssessment. Still missing real members: CustomerDnsIps, DnsName, LastUpdateDateTime, SecurityGroupIds, SelfManagedInstanceIds, StatusCode, StatusReason, SubnetIds, Version, VpcId -- see gaps (all require AssessmentConfiguration input capture that does not exist)."}
  ListADAssessments: {wire: FIXED, errors: ok, state: ok, persist: ok, note: "StartTime epoch fix (prior pass); same Region/ReportType fabrication fix and same residual gaps as DescribeADAssessment (AssessmentSummary's real member set is a subset of Assessment's, all still missing here)."}
  CreateHybridAD: {wire: gap, errors: ok, state: partial, persist: ok, note: "SEVERE finding this pass: field-diffed against types.CreateHybridADInput/CreateHybridADOutput and the wire shape is substantially wrong, not just missing fields. Real CreateHybridADInput requires AssessmentId (an existing successful StartADAssessment result) and SecretArn (a Secrets Manager ARN with the self-managed AD admin credentials) -- Name/ShortName/Description/Password/Edition are NOT real input members at all (AWS derives the domain name etc. from the referenced assessment). This backend instead reuses the CreateDirectory/CreateMicrosoftAD-style signature (name/shortName/description/password/edition/tags) with a fabricated RequestId return value; the real CreateHybridADOutput is just DirectoryId. Not fixed this pass -- a correct fix requires linking to the real ad_assessments.go state and is a larger redesign than a field patch; see gaps/deferred and the overall grade note."}
  UpdateHybridAD: {wire: gap, errors: ok, state: partial, persist: ok, note: "SEVERE finding, same pass as CreateHybridAD: real UpdateHybridADInput takes optional HybridAdministratorAccountUpdate{SecretArn} and/or SelfManagedInstancesSettings{CustomerDnsIps,InstanceIds} (types.HybridCustomerInstancesSettings) -- this backend's UpdateHybridAD(ctx, directoryID) accepts neither. Real UpdateHybridADOutput is {AssessmentId, DirectoryId}; this backend fabricates and returns a uuid RequestId instead, which is not a real output member at all. Not fixed this pass; see gaps/deferred."}
  DescribeHybridADUpdate: {wire: gap, errors: ok, state: partial, persist: ok, note: "SEVERE finding, same pass: real DescribeHybridADUpdateOutput is UpdateActivities{HybridAdministratorAccount: []HybridUpdateInfoEntry, SelfManagedInstances: []HybridUpdateInfoEntry} (types.HybridUpdateActivities), each entry carrying AssessmentId/InitiatedBy/LastUpdatedDateTime/NewValue/PreviousValue/StartTime/Status/StatusReason where NewValue/PreviousValue are HybridUpdateValue{DnsIps,InstanceIds}. This backend instead returns a flat list of {RequestId, DirectoryId, Status} entries -- RequestId is not a real member of any of these types. Because CreateHybridAD/UpdateHybridAD never captured real self-managed-instance/DNS configuration in the first place (see above), HybridSettingsDescription on DirectoryDescription also has no real source data to populate from -- this is why Directory.HybridSettings is listed as an unpopulated gap rather than fixed. Not fixed this pass; see gaps/deferred."}
...
gaps:                     # known divergences NOT fixed — link bd issue ids
  ...
  - "hybrid-AD (CreateHybridAD/UpdateHybridAD/DescribeHybridADUpdate) wire shapes substantially diverge from types.go v1.41.0, not just missing fields: (1) real CreateHybridADInput is {AssessmentId, SecretArn, Tags} (derives the domain from an existing StartADAssessment result) -- this backend instead takes {name, shortName, description, password, edition, tags}, a CreateMicrosoftAD-shaped signature with no relationship to any real assessment; (2) real CreateHybridADOutput/UpdateHybridADOutput return {DirectoryId} / {AssessmentId, DirectoryId} -- this backend fabricates and returns a uuid 'RequestId' that is not a member of either real output type; (3) real UpdateHybridADInput takes optional HybridAdministratorAccountUpdate{SecretArn} and/or SelfManagedInstancesSettings{CustomerDnsIps, InstanceIds} -- this backend's UpdateHybridAD(ctx, directoryID) accepts neither, so there is no way for a caller to ever supply the DNS-IP/instance-ID data that HybridSettingsDescription would need; (4) real DescribeHybridADUpdateOutput is UpdateActivities{HybridAdministratorAccount: []HybridUpdateInfoEntry, SelfManagedInstances: []HybridUpdateInfoEntry}, each entry carrying AssessmentId/InitiatedBy/LastUpdatedDateTime/NewValue/PreviousValue/StartTime/Status/StatusReason (NewValue/PreviousValue are HybridUpdateValue{DnsIps,InstanceIds}) -- this backend instead returns a flat, UpdateType-agnostic list of {RequestId, DirectoryId, Status}. Fixing this properly is a redesign of all three operations' request/response plumbing (new stored-state shape, real UpdateType-keyed activity log, linking CreateHybridAD to an existing ad_assessments.go record instead of taking directory-creation parameters directly), not a field patch -- explicitly out of scope for this pass; flagged as the primary reason for this pass's overall grade downgrade (A->B)."
  - "AD-assessments (StartADAssessment/DescribeADAssessment/ListADAssessments): StartADAssessment does not accept the real StartADAssessmentInput.AssessmentConfiguration member (CustomerDnsIps, DnsName, InstanceIds, VpcSettings, SecurityGroupIds) at all, so none of it can ever be stored or echoed back. As a direct result, Assessment/AssessmentSummary are still missing these real members entirely: CustomerDnsIps, DnsName, LastUpdateDateTime, SecurityGroupIds, SelfManagedInstanceIds, StatusCode, StatusReason, SubnetIds, Version, VpcId. The two fields that WERE wrong (fabricated 'Region'/'AssessmentType' wire members) were fixed this pass -- see the ops table -- but the remaining gap is real input-capture work, not a small field patch, so it's deferred rather than attempted piecemeal."
...
deferred:                 # consciously not audited this pass (scope) — next pass targets
  - "hybrid-AD family redesign (see gaps): CreateHybridAD/UpdateHybridAD/DescribeHybridADUpdate need their request/response shapes rebuilt against the real SDK types, including linking CreateHybridAD to an existing ad_assessments.go StartADAssessment record instead of taking CreateDirectory-shaped parameters."
  - "AD-assessment AssessmentConfiguration input capture (see gaps): StartADAssessment needs to accept and store CustomerDnsIps/DnsName/InstanceIds/VpcSettings/SecurityGroupIds so Assessment/AssessmentSummary can report their real remaining ~8 fields instead of the current DirectoryId/Status/StartTime/ReportType-only subset."
