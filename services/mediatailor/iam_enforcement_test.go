...
type mockMEDIATAILORIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMEDIATAILORIAMBackend() *mockMEDIATAILORIAMBackend {
	return &mockMEDIATAILORIAMBackend{
...
func (m *mockMEDIATAILORIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMEDIATAILORIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMEDIATAILORIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMEDIATAILORIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMEDIATAILOREnforcementServer(t *testing.T, iamBackend *mockMEDIATAILORIAMBackend) *httptest.Server {
	t.Helper()

	backend := mediatailor.NewInMemoryBackend("000000000000", "us-east-1")
	handler := mediatailor.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockMEDIATAILORIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAMEDIATAILORUSER1",
			method:      "GET",
			path:        "/playbackConfigurations",
			body:        ``,
			setupBackend: func(b *mockMEDIATAILORIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAMEDIATAILORUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAMEDIATAILORUSER2",
			method:      "PUT",
			path:        "/playbackConfiguration",
			body:        `{"Name":"test-pc","VideoContentSourceUrl":"http://example.com"}`,
			setupBackend: func(b *mockMEDIATAILORIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMEDIATAILORUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMEDIATAILORIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMEDIATAILOREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/mediatailor/aws4_request, SignedHeaders=host, Signature=mock",
			)
