...
// TestRealClient_ChannelSourcePlayback drives every gopherstack-n3zi
// uncovered mediatailor op through the real aws-sdk-go-v2 client.
func TestRealClient_ChannelSourcePlayback(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				_, err = client.DescribeChannel(
					ctx,
					&mediatailorsdk.DescribeChannelInput{ChannelName: aws.String("chan1")},
				)
				require.Error(t, err, "channel was deleted")
			},
		},
		{
			name: "channel_policy_lifecycle",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "source_location_and_live_source_and_vod_source",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "playback_configuration_and_prefetch_and_alerts",
			run: func(t *testing.T) {
				t.Helper()
				...
				assert.Equal(
					t,
					aws.ToString(putOut.PlaybackConfigurationArn),
					aws.ToString(getOut.PlaybackConfigurationArn),
				)
