...
// mediatailorSnapshotVersion identifies the shape of [backendSnapshot]. It
...
// mediatailorSnapshotVersion and is discarded the same way any other
// incompatible snapshot is.
const mediatailorSnapshotVersion = 1

// backendSnapshot is the top-level on-disk shape for the MediaTailor backend.
//
...
// registry is needed here. Tags is the one grouping map left un-converted
// (its values are plain string maps, not *T); channelPolicies was never
// persisted before this conversion (absent from the pre-Phase-3.3 snapshot
// struct in backend.go) and stays that way here. Version guards against
...
func (b *InMemoryBackend) Snapshot(ctx context.Context) []byte {
	...
	if err != nil {
		...
		// the Manager).
		logger.Load(ctx).WarnContext(ctx, "mediatailor: snapshot table marshal failed", "error", err)

		return nil
	}

	snap := backendSnapshot{
		Version:   mediatailorSnapshotVersion,
		Tables:    tables,
		Tags:      b.tags,
	...
	return persistence.MarshalSnapshot(ctx, "mediatailor", &snap)
}
...
func (b *InMemoryBackend) Restore(ctx context.Context, data []byte) error {
	...
	if err := persistence.UnmarshalSnapshot(ctx, "mediatailor", data, &snap); err != nil {
		return err
	}
	...
	if snap.Version != mediatailorSnapshotVersion {
		...
		logger.Load(ctx).WarnContext(ctx,
			"mediatailor: discarding incompatible snapshot version, starting empty",
			"gotVersion", snap.Version, "wantVersion", mediatailorSnapshotVersion)
	...
	if err := b.registry.RestoreAll(snap.Tables); err != nil {
		return fmt.Errorf("mediatailor: restore snapshot tables: %w", err)
	}
...
// This delegation is what actually wires MediaTailor into the persistence
// Manager.
