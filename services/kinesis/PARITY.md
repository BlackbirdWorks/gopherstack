...
applies any non-nil value incl. 0). UpdateShardCount/UpdateMaxRecordSize/
UpdateStreamWarmThroughput: false positives, required + already reject 0.

### 2026-09-18 (ledger burn-down, gopherstack parity sweep)

Adjudicated every items_still_open/deferred entry: removed 10 (RESOLVED/
CORRECTED history, or already fixed at HEAD -- StreamDescriptionSummary's
MaxRecordSizeInKiB/WarmThroughput and per-record EncryptionType were already
wired). Fixed 2 for real: UpdateShardCountOutput.StreamARN (now populated)
and ListStreamsOutput.StreamSummaries (ListStreams reshaped to paginate over
`[]*Stream`). Server-side fix for i8q7's root cause: Connection: close on
SubscribeToShard's response. Kept 5 structural items, condensed. Ledger:
12+1 -> 5+0. staleclaims flagged 7 rows; 1 real (folded in above), 6 heuristic
false positives (dated history / substring collisions).
