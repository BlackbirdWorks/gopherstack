...
items_still_open:
  ...
  - "No stream-level transient-state model (CREATING/UPDATING/DELETING) exists anywhere in kinesis -- every stream is ACTIVE immediately and stays so. Two consequences, both honest: ResourceInUseException is declared but unreachable for UpdateMaxRecordSize/UpdateStreamWarmThroughput, and UpdateStreamWarmThroughput applies synchronously (Current/Target always match on read) where real AWS is asynchronous. (gopherstack-nbg8)"
deferred: []
