...
func TestPutRecord_ByARN(t *testing.T) {
	...
	var desc *kinesis.DescribeStreamOutput

	// Setup runs inside a synctest bubble so CreateStream's CREATING window
	// lazily elapses without a real time.Sleep; the subtests below only call
	// PutRecord, which needs ACTIVE only at call time (already satisfied by
	// then) and does not itself gate on stream status transitions.
