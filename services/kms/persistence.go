...
func (b *InMemoryBackend) snapshotKeyMaterials() map[string]map[string]serializedKeyMaterial {
	serialized := make(map[string]map[string]serializedKeyMaterial)
	for region, regionMaterials := range b.keyMaterials {
		for keyID, km := range regionMaterials {
			...
			if serialized[region] == nil {
				serialized[region] = make(map[string]serializedKeyMaterial)
			}
			serialized[region][keyID] = s
		}
	}
...
func (b *InMemoryBackend) snapshotKeyMaterialHistory() map[string]map[string][]serializedKeyMaterial {
	serializedHistory := make(map[string]map[string][]serializedKeyMaterial)
	for region, regionHistory := range b.keyMaterialHistory {
		for keyID, history := range regionHistory {
			...
			if len(entries) > 0 {
				if serializedHistory[region] == nil {
					serializedHistory[region] = make(map[string][]serializedKeyMaterial)
				}
				serializedHistory[region][keyID] = entries
			}
		}
	}
