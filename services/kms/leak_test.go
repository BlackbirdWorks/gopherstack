...
// (aliases, grants, lastUsage -- see the other tests in this file), nothing
// removes a tags entry unless the janitor is explicitly wired to do so via
// Janitor.OnKeyPurged (set in Handler.WithJanitor). Since KMS key IDs are
// UUIDs that are never reused, an unfixed regression here leaks one
// *tags.Tags (and the lockmetrics/Prometheus registration it owns) per
// tagged key for the remaining lifetime of the process.
func TestTagsLeak_PurgeKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mem, ok := h.Backend.(*kms.InMemoryBackend)
			require.True(t, ok)

			keyIDs := make([]string, len(tt.tagOnly))

			for i, tagged := range tt.tagOnly {
				out, err := mem.CreateKey(ctx, &kms.CreateKeyInput{Description: "tags-leak-test"})
				require.NoError(t, err)
				keyIDs[i] = out.KeyMetadata.KeyID
				...
				_, err = mem.ScheduleKeyDeletion(ctx, &kms.ScheduleKeyDeletionInput{
					KeyID:               keyIDs[i],
					PendingWindowInDays: 7,
				})
				require.NoError(t, err)
			...
			h.SweepJanitorForTest(ctx)

			assert.Equal(t, 0, kms.KeyCount(mem), "all keys must be purged")
			assert.Equal(t, 0, h.TagsMapLen(),
