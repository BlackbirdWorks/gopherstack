...
func TestQuickSight_IdentityPropagationConfig(t *testing.T) {
	...
	invalidRec := doRequest(t, h, http.MethodPut, accountPath("/identity-propagation-config/BOGUS"), map[string]any{
		"AuthorizedTargets": []string{"arn:aws:sso::000000000000:application/foo"},
	})
	assert.Equal(t, http.StatusBadRequest, invalidRec.Code)

	updateRec := doRequest(t, h, http.MethodPut, accountPath("/identity-propagation-config/REDSHIFT"), map[string]any{
		"AuthorizedTargets": []string{"arn:aws:sso::000000000000:application/foo"},
	})
	...
	updateRec2 := doRequest(t, h, http.MethodPut, accountPath("/identity-propagation-config/REDSHIFT"), map[string]any{
	...
	assert.Equal(t, "arn:aws:sso::000000000000:application/bar", entry2["AuthorizedTargets"].([]any)[0])

	deleteRec := doRequest(t, h, http.MethodDelete, accountPath("/identity-propagation-config/REDSHIFT"), nil)
	require.Equal(t, http.StatusOK, deleteRec.Code)

	deleteMissingRec := doRequest(t, h, http.MethodDelete, accountPath("/identity-propagation-config/REDSHIFT"), nil)
	assert.Equal(t, http.StatusNotFound, deleteMissingRec.Code)

	finalListRec := doRequest(t, h, http.MethodGet, accountPath("/identity-propagation-config"), nil)
