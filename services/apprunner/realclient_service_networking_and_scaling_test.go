...
// TestRealClient_ServiceNetworkingAndScaling drives apprunner's remaining typed-coverage-
...
func TestRealClient_ServiceNetworkingAndScaling(t *testing.T) {
	t.Parallel()
	cases := []struct {
		run  func(t *testing.T)
		name string
	}{
		{name: "service pause/resume, operations, tags, update", run: func(t *testing.T) {
			t.Helper()
			...
			opsOut, err := client.ListOperations(
				ctx,
				&apprunnersdk.ListOperationsInput{ServiceArn: svc.Service.ServiceArn},
			)
			require.NoError(t, err)
		...
		{name: "custom domains", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "VPC connector describe and list", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "VPC ingress connection describe, list, update", run: func(t *testing.T) {
			t.Helper()
			...
			assert.Equal(
				t,
				"vpc-67890",
				aws.ToString(descAfterUpdate.VpcIngressConnection.IngressVpcConfiguration.VpcId),
			)
		}},
		{name: "auto scaling configuration default, list, services-for", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "DeleteObservabilityConfiguration", run: func(t *testing.T) {
			t.Helper()
			...
			require.NoError(t, err)
			_, err = client.DescribeObservabilityConfiguration(
				ctx,
