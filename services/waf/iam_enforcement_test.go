...
type mockWAFIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockWAFIAMBackend() *mockWAFIAMBackend {
	return &mockWAFIAMBackend{
...
func (m *mockWAFIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockWAFIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockWAFIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockWAFIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupWAFEnforcementServer(t *testing.T, iamBackend *mockWAFIAMBackend) *httptest.Server {
	t.Helper()

	backend := waf.NewInMemoryBackend("000000000000", "us-east-1")
	handler := waf.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockWAFIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIAWAFUSER1",
			target:      "AWSWAF_20150824.ListWebACLs",
			body:        `{"Limit":10}`,
			setupBackend: func(b *mockWAFIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAWAFUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAWAFUSER2",
			target:      "AWSWAF_20150824.CreateWebACL",
			body:        `{"Name":"acl","MetricName":"m","DefaultAction":{"Type":"ALLOW"},"ChangeToken":"tok"}`,
			setupBackend: func(b *mockWAFIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAWAFUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockWAFIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupWAFEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/waf/aws4_request, SignedHeaders=host, Signature=mock",
			)
