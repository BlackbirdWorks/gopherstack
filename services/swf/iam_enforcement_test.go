...
type mockSWFIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockSWFIAMBackend() *mockSWFIAMBackend {
	return &mockSWFIAMBackend{
...
func (m *mockSWFIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockSWFIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockSWFIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockSWFIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupSWFEnforcementServer(t *testing.T, iamBackend *mockSWFIAMBackend) *httptest.Server {
	t.Helper()

	backend := swf.NewInMemoryBackend()
	handler := swf.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockSWFIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIASWFUSER2",
			target:      "SimpleWorkflowService.RegisterDomain",
			body:        `{"name":"test-domain","workflowExecutionRetentionPeriodInDays":"10"}`,
			setupBackend: func(b *mockSWFIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIASWFUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockSWFIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupSWFEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/swf/aws4_request, SignedHeaders=host, Signature=mock",
			)
