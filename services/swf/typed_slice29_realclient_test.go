...
func Test_Slice29_RealClient(t *testing.T) {
	...
	t.Run("domain_lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("activity_type_lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("workflow_type_lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("workflow_execution_reads_and_signal", func(t *testing.T) {
		t.Parallel()
		...
		for _, ev := range histOut.Events {
			if ev.EventType == swftypes.EventTypeWorkflowExecutionSignaled {
				...
				assert.Equal(t, "my-signal", aws.ToString(ev.WorkflowExecutionSignaledEventAttributes.SignalName))
			}
		}
		...
		countClosed, err := client.CountClosedWorkflowExecutions(ctx, &swfsdk.CountClosedWorkflowExecutionsInput{
			Domain: aws.String("dom-we"),
	...
	t.Run("activity_task_lifecycle", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("activity_task_cancel_and_fail", func(t *testing.T) {
		t.Parallel()
