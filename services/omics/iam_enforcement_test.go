...
type mockOMICSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockOMICSIAMBackend() *mockOMICSIAMBackend {
	return &mockOMICSIAMBackend{
...
func (m *mockOMICSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockOMICSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockOMICSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockOMICSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupOMICSEnforcementServer(t *testing.T, iamBackend *mockOMICSIAMBackend) *httptest.Server {
	t.Helper()

	backend := omics.NewInMemoryBackend("000000000000", "us-east-1")
	handler := omics.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockOMICSIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
		...
		},
		{
			...
			accessKeyID: "AKIAOMICSUSER2",
			method:      "POST",
			path:        "/referencestore",
			body:        `{"name":"test-rs"}`,
			setupBackend: func(b *mockOMICSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAOMICSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockOMICSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupOMICSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/omics/aws4_request, SignedHeaders=host, Signature=mock",
			)
