...
func newRealClient(t *testing.T) *omicssdk.Client {
	t.Helper()
...
// TestRealClient_StoresAndRuns drives every op the census still listed as
// uncovered before this pass (gopherstack-n3zi typed-client coverage).
func TestRealClient_StoresAndRuns(t *testing.T) {
	t.Parallel()
			...
			run: func(t *testing.T) {
				...
				created, err := client.CreateShare(t.Context(), &omicssdk.CreateShareInput{
					ResourceArn: aws.String(
						"arn:aws:omics:us-east-1:000000000000:annotationStore/share-family-test",
					),
