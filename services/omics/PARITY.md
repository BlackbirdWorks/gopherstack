...
families:
  ...
  Workflow: {status: ok, note: "FIXED: GetWorkflow advances CREATING->ACTIVE on first poll (waiter-hang fix, prior pass). This pass: (1) ListWorkflows now applies its name/type query filters (gap jxc5); (2) CreateWorkflow's response now includes the optional uuid field real CreateWorkflowOutput has (gap fedo). VERIFIED 2026-09-11 (gopherstack-fedo re-audit): field set re-confirmed against omics@v1.49.5 (arn/id/status/tags/uuid exactly, awsRestjson1_deserializeOpDocumentCreateWorkflowOutput:3448); GetWorkflowOutput's uuid case also confirmed present. FIXED: Workflow.UUID switched from the truncated newID() form to newUUID() (real RFC 4122 string), same fix as Run above."}
