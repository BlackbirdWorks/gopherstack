...
type mockIOTWIRELESSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockIOTWIRELESSIAMBackend() *mockIOTWIRELESSIAMBackend {
	return &mockIOTWIRELESSIAMBackend{
...
func (m *mockIOTWIRELESSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockIOTWIRELESSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockIOTWIRELESSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockIOTWIRELESSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupIOTWIRELESSEnforcementServer(t *testing.T, iamBackend *mockIOTWIRELESSIAMBackend) *httptest.Server {
	t.Helper()
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockIOTWIRELESSIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAIOTWIRELESSUSER1",
			method:      "GET",
			path:        "/wireless-gateways",
			body:        ``,
			setupBackend: func(b *mockIOTWIRELESSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAIOTWIRELESSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAIOTWIRELESSUSER2",
			method:      "POST",
			path:        "/wireless-gateways",
			body:        `{"LoRaWAN":{"GatewayEui":"0123456789abcdef","RfRegion":"US915"}}`,
			setupBackend: func(b *mockIOTWIRELESSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAIOTWIRELESSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockIOTWIRELESSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupIOTWIRELESSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/iotwireless/aws4_request, SignedHeaders=host, Signature=mock",
			)
