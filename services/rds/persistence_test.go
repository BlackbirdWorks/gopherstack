...
func TestInMemoryBackend_SnapshotRestore(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(b *rds.InMemoryBackend) string {
				cluster, err := b.CreateDBCluster("test-cluster", "aurora-postgresql", "admin", "mydb", "", 0, nil, rds.DBClusterOptions{})
				if err != nil {
					return ""
				}
		...
		},
		{
			...
			setup: func(b *rds.InMemoryBackend) string {
				_, err := b.CreateDBCluster("snap-cluster", "aurora-postgresql", "admin", "mydb", "", 0, nil, rds.DBClusterOptions{})
				if err != nil {
					return ""
				}
