...
type mockPIPESIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockPIPESIAMBackend() *mockPIPESIAMBackend {
	return &mockPIPESIAMBackend{
...
func (m *mockPIPESIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockPIPESIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockPIPESIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockPIPESIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupPIPESEnforcementServer(t *testing.T, iamBackend *mockPIPESIAMBackend) *httptest.Server {
	t.Helper()

	backend := pipes.NewInMemoryBackend("000000000000", "us-east-1")
	handler := pipes.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAPIPESUSER1",
			method:      "GET",
			path:        "/v1/pipes",
			body:        ``,
			setupBackend: func(b *mockPIPESIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAPIPESUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockPIPESIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupPIPESEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/pipes/aws4_request, SignedHeaders=host, Signature=mock",
			)
