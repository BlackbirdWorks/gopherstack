...
**Addendum (2026-09-06, gopherstack-gcjw): Bug 2 and Bug 3's fixes are no
longer in the tree.** `EcsContainerOverride`, `EcsResourceRequirement`,
...
with no corresponding edit to this prose. Confirmed directly: `git show
d4e234022 -- services/pipes/targets.go` deletes every type this section
describes adding, and `grep -rn "ResourceRequirement\|EphemeralStorage"
services/pipes/*.go` today returns nothing. Left as historical record rather
than rewritten, but do not read Bug 2/Bug 3 as describing current code --
check `targets.go` directly. This is why gopherstack-gcjw's ECS-Overrides and
Batch-ResourceRequirements follow-up items (CreatePipe's ops-table note
above) could not be validated: the fields to validate do not exist.
