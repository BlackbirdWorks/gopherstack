...
ops:
  ...
  DeleteStreamProcessor: {wire: ok, errors: ok, state: ok, persist: ok, note: "gopherstack-zhts (2026-09-07): ResourceInUseException is in the declared error set (deserializers.go awsAwsjson11_deserializeOpErrorDeleteStreamProcessor) but no guard exists -- confirmed negative, not just asserted. Searched: api_op_DeleteStreamProcessor.go doc comment (no ResourceInUseException mention at all); types.ResourceInUseException's own doc ('The specified resource is already being used.', no state qualifier); vendored botocore api-2.json/docs-2.json for rekognition@2016-06-27 (identical generic sentence, docs-2.json shape has no per-op refs); the live API reference Errors section (same generic text verbatim); the AWS CLI reference page (no Errors section at all); the Java SDK javadoc (ResourceInUseException isn't even in this op's @throws list there). None ties the code to stream-processor RUNNING vs STOPPED. Backend does track Status (stream_processors.go: processorRunning/processorStopped) so a guard would be mechanically trivial, but adding one would fabricate an undocumented precondition -- declined per the campaign's no-invention rule. (A web search's own prose claiming 'must be stopped first' surfaced no actual AWS doc citation backing it -- discounted as unsourced.)"}
  ...
  ListTagsForResource: {wire: ok, errors: ok, state: ok, persist: ok, note: "Fixed 2026-09-24 (s3tables-messaging-and-streaming terraform coverage): resourceExists() only recognized collections, stream processors, and ProjectVersion (Custom Labels model) ARNs -- a plain project ARN always 404'd, even though CreateProjectInput.Tags accepts tags at creation and a real client (Terraform's aws_rekognition_project) calls ListTagsForResource with the project ARN on every read. Now also checks b.projects.Has."}
  CreateProject: {wire: ok, errors: ok, state: ok, persist: ok, note: "FIXED this sweep: duplicate name now returns ResourceInUseException (was ResourceAlreadyExistsException) — see Notes #2. Fixed 2026-09-24 (s3tables-messaging-and-streaming terraform coverage): new projects were stored with Status=CREATING and nothing ever transitioned them -- CreateProject is synchronous on real AWS, so a project must already be CREATED by the time DescribeProjects is called. A caller that polls for CREATED (e.g. Terraform's aws_rekognition_project) hung forever. Now set to CREATED at creation."}
  ...
  StartProjectVersion: {wire: ok, errors: ok, state: ok, persist: ok, note: "FIXED this sweep (Notes #6): now accepts and stores the optional MaxInferenceUnits (StartProjectVersionInput member; was parsed nowhere, so MinInferenceUnits was the only value ever recorded). gopherstack-zhts (2026-09-07): ResourceInUseException is declared but unguarded here too -- same source list checked as DeleteStreamProcessor's note above (SDK doc comment, types.ResourceInUseException doc, botocore api-2.json/docs-2.json, live API reference, CLI reference, Java SDK javadoc), all silent on which ProjectVersion status (TRAINING_IN_PROGRESS/RUNNING/STOPPED/etc, project_versions.go) triggers it. Backend can observe 'already RUNNING' (v.Status), so a guard is mechanically trivial, but nothing documents that precondition -- declined per no-invention rule."}
