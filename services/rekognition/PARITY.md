...
ops:
  ...
  ListTagsForResource: {wire: ok, errors: ok, state: ok, persist: ok, note: "Fixed 2026-09-24 (mega-batch-52 terraform coverage): resourceExists() only recognized collections, stream processors, and ProjectVersion (Custom Labels model) ARNs -- a plain project ARN always 404'd, even though CreateProjectInput.Tags accepts tags at creation and a real client (Terraform's aws_rekognition_project) calls ListTagsForResource with the project ARN on every read. Now also checks b.projects.Has."}
  CreateProject: {wire: ok, errors: ok, state: ok, persist: ok, note: "FIXED this sweep: duplicate name now returns ResourceInUseException (was ResourceAlreadyExistsException) — see Notes #2. Fixed 2026-09-24 (mega-batch-52 terraform coverage): new projects were stored with Status=CREATING and nothing ever transitioned them -- CreateProject is synchronous on real AWS, so a project must already be CREATED by the time DescribeProjects is called. A caller that polls for CREATED (e.g. Terraform's aws_rekognition_project) hung forever. Now set to CREATED at creation."}
