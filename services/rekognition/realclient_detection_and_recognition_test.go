...
// TestRealClient_DetectionAndRecognition drives every op the census still
// listed as uncovered before this pass (gopherstack-n3zi typed-client
// coverage). Each case builds its own fresh client/backend.
func TestRealClient_DetectionAndRecognition(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				moderation, err := client.DetectModerationLabels(
					t.Context(),
					&rekognitionsdk.DetectModerationLabelsInput{
						Image:         &fakeImage,
				...
				ppe, err := client.DetectProtectiveEquipment(
					t.Context(),
					&rekognitionsdk.DetectProtectiveEquipmentInput{
						Image: &fakeImage,
					},
				)
				require.NoError(t, err)
		...
		{
			name: "video_jobs_labels_moderation",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "video_jobs_person_segment_text",
			run: func(t *testing.T) {
				t.Helper()
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				described, err := client.DescribeStreamProcessor(
					t.Context(),
					&rekognitionsdk.DescribeStreamProcessorInput{
						Name: aws.String(name),
					},
				)
				require.NoError(t, err)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				described, err := client.DescribeProjectVersions(
					t.Context(),
					&rekognitionsdk.DescribeProjectVersionsInput{
						ProjectArn: aws.String(projectARN),
					},
				)
				require.NoError(t, err)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				assert.InDelta(
					t,
					float32(50),
					aws.ToFloat32(got.OperationsConfig.DetectModerationLabels.MinConfidence),
					0.01,
				)
