...
type mockREKOGNITIONIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockREKOGNITIONIAMBackend() *mockREKOGNITIONIAMBackend {
	return &mockREKOGNITIONIAMBackend{
...
func (m *mockREKOGNITIONIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockREKOGNITIONIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockREKOGNITIONIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockREKOGNITIONIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupREKOGNITIONEnforcementServer(t *testing.T, iamBackend *mockREKOGNITIONIAMBackend) *httptest.Server {
	t.Helper()

	backend := rekognition.NewInMemoryBackend("000000000000", "us-east-1")
	handler := rekognition.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockREKOGNITIONIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAREKOGNITIONUSER2",
			target:      "RekognitionService.CreateCollection",
			body:        `{"CollectionId":"test-c"}`,
			setupBackend: func(b *mockREKOGNITIONIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAREKOGNITIONUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockREKOGNITIONIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupREKOGNITIONEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/rekognition/aws4_request, SignedHeaders=host, Signature=mock",
			)
