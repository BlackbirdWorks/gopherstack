...
type mockCOMPREHENDIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCOMPREHENDIAMBackend() *mockCOMPREHENDIAMBackend {
	return &mockCOMPREHENDIAMBackend{
...
func (m *mockCOMPREHENDIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCOMPREHENDIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCOMPREHENDIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCOMPREHENDIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCOMPREHENDEnforcementServer(t *testing.T, iamBackend *mockCOMPREHENDIAMBackend) *httptest.Server {
	t.Helper()

	backend := comprehend.NewInMemoryBackend("000000000000", "us-east-1")
	handler := comprehend.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIACOMPREHENDUSER1",
			target:      "Comprehend_20171127.ListDocumentClassifiers",
			body:        `{}`,
			setupBackend: func(b *mockCOMPREHENDIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACOMPREHENDUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCOMPREHENDIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCOMPREHENDEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/comprehend/aws4_request, SignedHeaders=host, Signature=mock",
			)
