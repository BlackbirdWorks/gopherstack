...
func TestErrorsNotFound(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Real AWS uses a distinct JobNotFoundException (not
			// ResourceNotFoundException) for every Describe*Job/Stop*Job
			// operation -- confirmed against every
			// awsAwsjson11_deserializeOpErrorDescribe*Job case in the SDK's
			// generated deserializers.go.
			name:   "describe_missing_job",
			action: "DescribeSentimentDetectionJob",
			body:   `{"JobId":"no-such-job-id"}`,
			want:   "JobNotFoundException",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m := decodeBody(t, rec)
			assert.Equal(t, tt.want, m["__type"])
