...
last_audit_date: 2026-09-19  # gopherstack-2wb parity sweep: fixed CreatePipeline/UpdatePipeline
...
cmd/overwidecandidates flagged all 5 List ops. ListChannels/ListDatasetContents
already matched their Summary type exactly; ListDatasets/ListDatastores were
already fixed by the 2026-08-23 pass (Actions/Triggers,
DatastorePartitions/FileFormatType). ListPipelines had a real leak:
pipelineReprocessingSummary carried startTime/endTime, which real
ReprocessingSummary doesn't have at all -- fixed, and the test that used to
assert startTime's presence (TestHandler_PipelineReprocessingSummary_StartTime)
now asserts its absence. See list_summary_shapes_test.go.
