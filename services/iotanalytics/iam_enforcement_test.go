...
type mockIOTANALYTICSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockIOTANALYTICSIAMBackend() *mockIOTANALYTICSIAMBackend {
	return &mockIOTANALYTICSIAMBackend{
...
func (m *mockIOTANALYTICSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockIOTANALYTICSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockIOTANALYTICSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockIOTANALYTICSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupIOTANALYTICSEnforcementServer(t *testing.T, iamBackend *mockIOTANALYTICSIAMBackend) *httptest.Server {
	t.Helper()

	backend := iotanalytics.NewInMemoryBackend()
	handler := iotanalytics.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockIOTANALYTICSIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAIOTANALYTICSUSER1",
			method:      "GET",
			path:        "/channels",
			body:        ``,
			setupBackend: func(b *mockIOTANALYTICSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAIOTANALYTICSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAIOTANALYTICSUSER2",
			method:      "POST",
			path:        "/channels",
			body:        `{"channelName":"test-ch"}`,
			setupBackend: func(b *mockIOTANALYTICSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAIOTANALYTICSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockIOTANALYTICSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupIOTANALYTICSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/iotanalytics/aws4_request, SignedHeaders=host, Signature=mock",
			)
