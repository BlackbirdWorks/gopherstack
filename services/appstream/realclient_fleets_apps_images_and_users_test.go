...
// for gopherstack-n3zi. AppStream is rpc-v2-cbor
...
func newRealClient(t *testing.T) *appstreamsdk.Client {
	t.Helper()
...
// TestRealClient_FleetsAppsImagesAndUsers drives every op the census still
// listed as uncovered before this pass (gopherstack-n3zi typed-client
// coverage).
func TestRealClient_FleetsAppsImagesAndUsers(t *testing.T) {
	t.Parallel()
	cases := []struct {
		run  func(t *testing.T)
		name string
	}{
		{name: "fleets_and_associations", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "tags", run: func(t *testing.T) {
			...
			created, err := client.CreateStack(
				ctx,
				&appstreamsdk.CreateStackInput{Name: aws.String("slice18-tag-stack")},
			)
			require.NoError(t, err)
		...
		{name: "app_blocks_and_builders", run: func(t *testing.T) {
			t.Helper()
			...
			url, err := client.CreateAppBlockBuilderStreamingURL(
				ctx,
				&appstreamsdk.CreateAppBlockBuilderStreamingURLInput{
			...
			_, err = client.DisassociateAppBlockBuilderAppBlock(
				ctx,
				&appstreamsdk.DisassociateAppBlockBuilderAppBlockInput{
					AppBlockBuilderName: aws.String("slice18-builder"),
					AppBlockArn:         aws.String(appBlockARN),
				},
			)
		...
		{name: "applications", run: func(t *testing.T) {
			...
			_, err = client.DeleteApplication(
				ctx,
				&appstreamsdk.DeleteApplicationInput{Name: aws.String("slice18-app")},
			)
			require.NoError(t, err)
		}},
		{name: "application_fleet_associations", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "entitlements", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "directory_configs", run: func(t *testing.T) {
			t.Helper()
			...
			assert.Equal(
				t,
				"svc-account-2",
				aws.ToString(updated.DirectoryConfig.ServiceAccountCredentials.AccountName),
			)
		...
		{name: "images", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "image_builders", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "users", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "user_stack_associations", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "usage_report_subscriptions", run: func(t *testing.T) {
			t.Helper()
		...
		{name: "themes", run: func(t *testing.T) {
			t.Helper()
