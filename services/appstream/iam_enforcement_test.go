...
type mockAPPSTREAMIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockAPPSTREAMIAMBackend() *mockAPPSTREAMIAMBackend {
	return &mockAPPSTREAMIAMBackend{
...
func (m *mockAPPSTREAMIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockAPPSTREAMIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockAPPSTREAMIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockAPPSTREAMIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupAPPSTREAMEnforcementServer(t *testing.T, iamBackend *mockAPPSTREAMIAMBackend) *httptest.Server {
	t.Helper()

	backend := appstream.NewInMemoryBackend("123456789012", "us-east-1")
	handler := appstream.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockAPPSTREAMIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAAPPSTREAMUSER2",
			target:      "PhotonAdminProxyService.CreateStack",
			body:        `{"Name":"test-stack"}`,
			setupBackend: func(b *mockAPPSTREAMIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAAPPSTREAMUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockAPPSTREAMIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupAPPSTREAMEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/appstream/aws4_request, SignedHeaders=host, Signature=mock",
			)
