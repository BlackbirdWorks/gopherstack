...
type mockEFSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockEFSIAMBackend() *mockEFSIAMBackend {
	return &mockEFSIAMBackend{
...
func (m *mockEFSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockEFSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockEFSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockEFSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupEFSEnforcementServer(t *testing.T, iamBackend *mockEFSIAMBackend) *httptest.Server {
	t.Helper()

	backend := efs.NewInMemoryBackend("000000000000", "us-east-1")
	handler := efs.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	scopedPolicy := `{
		...
		"Statement":[
			{
				"Effect":"Allow",
				"Action":["efs:DescribeFileSystems"],
				"Resource":["*"]
			}
		]
	}`
	...
	tests := []struct {
		setupBackend  func(b *mockEFSIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
		...
		},
		{
			...
			accessKeyID: "AKIAEFSUSER2",
			method:      "POST",
			path:        "/2015-02-01/file-systems",
			body:        `{"CreationToken":"tok"}`,
			setupBackend: func(b *mockEFSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAEFSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockEFSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupEFSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/elasticfilesystem/aws4_request, SignedHeaders=host, Signature=mock",
			)
