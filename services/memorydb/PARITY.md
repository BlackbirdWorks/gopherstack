...
ops:
  ...
  UpdateMultiRegionCluster: {wire: ok, errors: ok, state: ok, persist: ok, note: "fixed (gopherstack-yusn): same MultiRegionParameterGroupName gap as CreateMultiRegionCluster -- accepted and stored any name, including nonexistent ones, with no FK check. Now validated."}
