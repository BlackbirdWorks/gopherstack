...
type mockPINPOINTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockPINPOINTIAMBackend() *mockPINPOINTIAMBackend {
	return &mockPINPOINTIAMBackend{
...
func (m *mockPINPOINTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockPINPOINTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockPINPOINTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockPINPOINTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupPINPOINTEnforcementServer(t *testing.T, iamBackend *mockPINPOINTIAMBackend) *httptest.Server {
	t.Helper()

	backend := pinpoint.NewInMemoryBackend("us-east-1", "000000000000")
	handler := pinpoint.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockPINPOINTIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAPINPOINTUSER1",
			method:      "GET",
			path:        "/v1/apps",
			body:        ``,
			setupBackend: func(b *mockPINPOINTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAPINPOINTUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAPINPOINTUSER2",
			method:      "POST",
			path:        "/v1/apps",
			body:        `{"Name":"test-app"}`,
			setupBackend: func(b *mockPINPOINTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAPINPOINTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockPINPOINTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupPINPOINTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/mobiletargeting/aws4_request, SignedHeaders=host, Signature=mock",
			)
