...
// TestRealClient_CampaignsAndChannels drives pinpoint's typed-coverage-blind
// ops (gopherstack-n3zi) through the real aws-sdk-go-v2 pinpoint
// client, one case per named priority family, asserting decoded values.
func TestRealClient_CampaignsAndChannels(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				assert.Equal(
					t,
					"s7-activity",
					aws.ToString(actMetricsOut.JourneyRunExecutionActivityMetricsResponse.JourneyActivityId),
				)
		...
		{
			name: "templates_push_voice_inapp",
			run: func(t *testing.T) {
				t.Helper()
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				_, err = client.UpdateApnsVoipSandboxChannel(
					t.Context(),
					&pinpointsdk.UpdateApnsVoipSandboxChannelInput{
						ApplicationId: aws.String(appID),
