...
type mockELBIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockELBIAMBackend() *mockELBIAMBackend {
	return &mockELBIAMBackend{
...
func (m *mockELBIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockELBIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockELBIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockELBIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupELBEnforcementServer(t *testing.T, iamBackend *mockELBIAMBackend) *httptest.Server {
	t.Helper()

	backend := elb.NewInMemoryBackend("000000000000", "us-east-1")
	handler := elb.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockELBIAMBackend)
		name          string
		accessKeyID   string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAELBUSER2",
			body:        "Action=CreateLoadBalancer&LoadBalancerName=test-lb&Version=2012-06-01",
			setupBackend: func(b *mockELBIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAELBUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockELBIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupELBEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/elasticloadbalancing/aws4_request, SignedHeaders=host, Signature=mock",
			)
