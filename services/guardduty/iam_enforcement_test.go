...
type mockGUARDDUTYIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockGUARDDUTYIAMBackend() *mockGUARDDUTYIAMBackend {
	return &mockGUARDDUTYIAMBackend{
...
func (m *mockGUARDDUTYIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockGUARDDUTYIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockGUARDDUTYIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockGUARDDUTYIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupGUARDDUTYEnforcementServer(t *testing.T, iamBackend *mockGUARDDUTYIAMBackend) *httptest.Server {
	t.Helper()

	backend := guardduty.NewInMemoryBackend("000000000000", "us-east-1")
	handler := guardduty.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockGUARDDUTYIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAGUARDDUTYUSER1",
			method:      "GET",
			path:        "/detector",
			body:        ``,
			setupBackend: func(b *mockGUARDDUTYIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAGUARDDUTYUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAGUARDDUTYUSER2",
			method:      "POST",
			path:        "/detector",
			body:        `{"enable":true}`,
			setupBackend: func(b *mockGUARDDUTYIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAGUARDDUTYUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockGUARDDUTYIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupGUARDDUTYEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/guardduty/aws4_request, SignedHeaders=host, Signature=mock",
			)
