...
type mockRAMIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockRAMIAMBackend() *mockRAMIAMBackend {
	return &mockRAMIAMBackend{
...
func (m *mockRAMIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockRAMIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockRAMIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockRAMIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupRAMEnforcementServer(t *testing.T, iamBackend *mockRAMIAMBackend) *httptest.Server {
	t.Helper()

	backend := ram.NewInMemoryBackend("000000000000", "us-east-1")
	handler := ram.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockRAMIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIARAMUSER1",
			method:      "POST",
			path:        "/createresourceshare",
			body:        `{"name":"test-share","allowExternalPrincipals":true}`,
			setupBackend: func(b *mockRAMIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIARAMUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIARAMUSER2",
			method:      "POST",
			path:        "/deleteresourceshare",
			body:        `{"resourceShareArn":"arn:aws:ram:us-east-1:000000000000:resource-share/s"}`,
			setupBackend: func(b *mockRAMIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIARAMUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockRAMIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupRAMEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/ram/aws4_request, SignedHeaders=host, Signature=mock",
			)
