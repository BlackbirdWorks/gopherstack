...
Wired the mega-batch-49 items_still_open gap: EnableSharingWithAwsOrganization
...
2 tier-1 undeclared request fields, both fixed: `ListPermissionAssociations.DefaultVersion`
(never declared, silently returned every version) and `ListResourceTypes.ResourceRegionScope`
(handler ignored its request body entirely). 0 false positives, 0 recorded-as-gap. tier1 count
2 -> 0 (`go run ./cmd/reqfielddiff -dir ram`). See the two ops rows above for citations/tests.
