...
type mockFIREHOSEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockFIREHOSEIAMBackend() *mockFIREHOSEIAMBackend {
	return &mockFIREHOSEIAMBackend{
...
func (m *mockFIREHOSEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockFIREHOSEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockFIREHOSEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockFIREHOSEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupFIREHOSEEnforcementServer(t *testing.T, iamBackend *mockFIREHOSEIAMBackend) *httptest.Server {
	t.Helper()

	backend := firehose.NewInMemoryBackend("000000000000", "us-east-1")
	handler := firehose.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockFIREHOSEIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAFIREHOSEUSER2",
			target:      "Firehose_20150804.CreateDeliveryStream",
			body:        `{"DeliveryStreamName":"test-stream"}`,
			setupBackend: func(b *mockFIREHOSEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAFIREHOSEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockFIREHOSEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupFIREHOSEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/firehose/aws4_request, SignedHeaders=host, Signature=mock",
			)
