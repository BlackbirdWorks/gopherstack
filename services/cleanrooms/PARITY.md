...
# 41/100 -> 100/100 (100%), realclient_table_and_collaboration_lifecycle_test.go, 12 cases
...
families:
  Collaboration: {status: ok, note: "FIXED this pass -- see bugs 1-4: CollaborationIdentifier/memberAbilities were invented output fields (deleted from the wire), auto-membership creation added, DeleteMember/DeleteCollaboration state machines fixed. gopherstack ignored-parameter sweep (2026-08-29): ListCollaborationsInput.MemberStatus ('the caller's status in a collaboration') was parsed from the query string but then discarded (passed as `_`) before reaching InMemoryBackend.ListCollaborations -- every collaboration was always returned. Backend signature now takes memberStatus and filters against the (still hardcoded-ACTIVE, see gaps) CollaborationSummary.MemberStatus field. SCREENED 2026-09-07 (bd gopherstack-b76t, errtargetaudit class-A leads): GetCollaboration/UpdateCollaboration (collaborations.go:96,143) emit ResourceNotFoundException for an unknown CollaborationIdentifier, but cleanrooms@v1.49.4's awsRestjson1_deserializeOpErrorGetCollaboration/-UpdateCollaboration switches don't type that exception (only AccessDeniedException/InternalServerException/ThrottlingException/ValidationException) -- the same trio-wide SDK omission already found and accepted for DeleteCollaboration (see handler_delete_collaboration_idempotent_test.go). Verdict: confirmed mismatch, deliberately retained -- NOT a false positive (corrected 2026-09-08, bd gopherstack-a2bp: the original 'false positive' label here contradicted the rest of this note and the reasoning below; a code the op doesn't declare genuinely is a defect, this one is just judged not worth 'fixing' by swapping to a worse code). Sibling ops with the identical Get/Update-by-ID shape (GetMembership, UpdateMembership, UpdateConfiguredTable) DO declare ResourceNotFoundException, so this is a genuine, consistent AWS quirk specific to the Collaboration resource's own CRUD trio, not a random omission. restjson1 doesn't restrict a server to an operation's modeled error set -- the generated Go client falls back to smithy.GenericAPIError for an unmodeled code but still reads the literal code off the wire, so .ErrorCode() still reports \"ResourceNotFoundException\" correctly (proven by TestHandler_GetUpdateCollaboration_NonexistentIsResourceNotFound). handler.go's handleError already documents why this matters: the Terraform provider's delete-waiter polls GetCollaboration after DeleteCollaboration and must recognize ResourceNotFoundException by that string. Unlike Delete, Get/Update can't be made idempotent (no real resource to fabricate for a missing ID), so ResourceNotFoundException remains the only non-misleading choice among the declared codes -- AccessDeniedException would misreport an authz failure, ValidationException a malformed request, and real AWS's actual behavior for an unknown CollaborationIdentifier isn't recoverable from the SDK model or botocore's cleanrooms/2022-02-17/service-2.json (docs give no hint either), so this can't be fully resolved without a live-AWS probe -- recorded as a deliberate, evidenced choice rather than closed as correct. Regression test added (handler_collaboration_notfound_test.go); no production code changed. RE-AUDITED 2026-09-08 (bd gopherstack-a2bp): independently re-derived both oracles (pinned cleanrooms@v1.49.4 deserializers.go AND botocore's service-2.json errors lists agree exactly: GetCollaboration/UpdateCollaboration/DeleteCollaboration declare only AccessDeniedException/InternalServerException/ThrottlingException/ValidationException, while GetMembership/UpdateMembership/UpdateConfiguredTable all add ResourceNotFoundException); swept every other ErrNotFound/ErrConflict/ErrAlreadyExists emission site in the service (errors.go's three sentinels, ~90 call sites across every family file) against its own op's declared set built mechanically from deserializers.go -- no other instance found. The three list ops that also lack ResourceNotFoundException (ListCollaborations, ListConfiguredTables, ListMemberships) and CreateCollaboration (lacks both ResourceNotFoundException and ConflictException) were confirmed to never emit ErrNotFound/ErrConflict in their own backend functions, so the Collaboration trio remains the sole instance of this defect class in cleanrooms. golangci-lint (0 issues) and go test -race (ok) both re-confirmed clean on this branch."}
...
Verified all 22 `overwidecandidates`-flagged List ops against `cleanrooms@v1.49.4` types.go.
19 were false positives (already narrow, or a pre-existing documented gap). 3 real
member-omission bugs fixed: `description` was missing from `CollaborationAnalysisTemplateSummary`,
`CollaborationIDNamespaceAssociationSummary`, and `ConfiguredAudienceModelAssociationSummary`
(tracked, just never copied) -- proven by `TestListCollaborationScopedSummaries_MissingMembers`.
Also corrected an `items_still_open` mischaracterization of `ReceiverConfigurations` as optional.
...
## reqfielddiff (2026-09-12, bd gopherstack-xhu2t)

Worked all 30 tier-1 findings from `cmd/reqfielddiff -dir cleanrooms`.

**21 false positives** (query-protocol blind spot, gopherstack-99nj): every
`List*Op.MaxResults` finding. cleanrooms is restjson1 and `MaxResults` is an
`httpQuery` binding on every List op (confirmed against
`awsRestjson1_serializeOpHttpBindings<Op>Input` for `ListCollaborations` and
representative siblings), read via `qp(c, "maxResults")` (an `echo.Context`
`QueryParam` wrapper) in every `handle<Op>` and applied through the shared
`paginate()` helper. reqfielddiff cannot see a query-param read keyed by a
string literal, so it reports these as undeclared. Affected ops:
ListAnalysisTemplates, ListCollaborationAnalysisTemplates,
ListCollaborationConfiguredAudienceModelAssociations,
ListCollaborationIdNamespaceAssociations,
ListCollaborationPrivacyBudgetTemplates, ListCollaborationPrivacyBudgets,
ListCollaborations, ListConfiguredAudienceModelAssociations,
ListConfiguredTableAssociations, ListConfiguredTables, ListIdMappingTables,
ListIdNamespaceAssociations, ListIntermediateTableVersions,
ListIntermediateTables, ListMembers, ListMemberships,
ListPrivacyBudgetTemplates, ListPrivacyBudgets, ListProtectedJobs,
ListProtectedQueries, ListSchemas.

**8 dropped parameters fixed** (all restjson1 body fields, confirmed via
`awsRestjson1_serializeOpDocument<Op>Input`, none httpQuery/Label/Header):

- `CreateCollaboration.CreatorPaymentConfiguration` -- now read and passed as
  the creator's own auto-created membership's `PaymentConfiguration`
  (previously always defaulted via `defaultPaymentConfig(abilities, nil)`).
  Observable via `GetMembership` on the collaboration's own
  `membershipId`/`membershipArn`.
- `CreateCollaboration.IsMetricsEnabled` -- new `Collaboration.IsMetricsEnabled`
  field, echoed on `GetCollaboration`/`CreateCollaboration`
  (default `false`).
- `CreateCollaboration.JobLogStatus` -- new `Collaboration.JobLogStatus`
  field, defaults to `DISABLED` when omitted, echoed on
  `GetCollaboration`/`CreateCollaboration`.
- `CreateMembership.DefaultJobResultConfiguration` -- new
  `Membership.DefaultJobResultConfiguration` field (distinct wire field from
  the pre-existing `DefaultResultConfiguration`/`defaultResultConfiguration`,
  confirmed as two separate members on `CreateMembershipInput`), echoed on
  `GetMembership`/`CreateMembership`.
- `CreateMembership.IsMetricsEnabled` -- new `Membership.IsMetricsEnabled`
  field, echoed on `GetMembership`/`CreateMembership` (default `false`).
- `CreateMembership.JobLogStatus` -- new `Membership.JobLogStatus` field,
  defaults to `DISABLED` when omitted, echoed on
  `GetMembership`/`CreateMembership`.
- `UpdateMembership.DefaultJobResultConfiguration` -- now applied to the
  existing membership when non-nil, observable via a follow-up
  `GetMembership`.
- `UpdateMembership.JobLogStatus` -- now applied to the existing membership
  when non-empty, observable via a follow-up `GetMembership`.

`createMembershipLocked`'s and `CreateMembership`'s parameter lists were
already at the edge of readability; introduced an internal `membershipSpec`
struct so `createMembershipLocked` takes one struct instead of eight
positional args (kept `CreateMembership`/`StorageBackend.CreateMembership`
positional -- the interface is called from `cleanrooms_test` (external test
package), and this repo's convention bans adding new `export_test.go`
exports just to hand a test package an unexported type).

**1 recorded as a real gap** (see `items_still_open`):
`PopulateIdMappingTable.JobType` is a genuine body field but this backend
has no `IdMappingJob` entity/store and no `GetIdMappingJob`-equivalent op
exists on this service at all, so there is no wire-observable place a real
client could read the value back from. Not fabricating a job store for one
write-only field.

Proof: `realclient_collaboration_and_membership_fields_test.go`, driving the real
`aws-sdk-go-v2/service/cleanrooms` typed client end to end (CreateCollaboration
-> GetMembership for the creator-payment-config/isMetricsEnabled/jobLogStatus
threading, CreateMembership -> GetMembership for the membership-level fields,
UpdateMembership -> GetMembership for the update-path fields).

Gates: `go build ./...`, `go vet ./services/cleanrooms/...`, `go test -race
-count=1 ./services/cleanrooms/...`, `golangci-lint run
--new-from-rev=HEAD ./services/cleanrooms/...`.
