...
func TestHTTP_IDMappingTableEndpoints(t *testing.T) {
	t.Parallel()
	e := newTestServer(t)
	...
	// Get
	getRec := doRequest(t, e, http.MethodGet, "/memberships/"+mID+"/idmappingtables/"+id, nil)
	require.Equal(t, http.StatusOK, getRec.Code)

	// List
	listRec := doRequest(t, e, http.MethodGet, "/memberships/"+mID+"/idmappingtables", nil)
	require.Equal(t, http.StatusOK, listRec.Code)

	// Update
	upRec := doRequest(t, e, http.MethodPatch, "/memberships/"+mID+"/idmappingtables/"+id, map[string]any{
		"description": "updated desc",
	})
	require.Equal(t, http.StatusOK, upRec.Code)

	// Delete
	delRec := doRequest(t, e, http.MethodDelete, "/memberships/"+mID+"/idmappingtables/"+id, nil)
