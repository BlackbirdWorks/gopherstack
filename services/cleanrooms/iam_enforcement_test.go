...
type mockCLEANROOMSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCLEANROOMSIAMBackend() *mockCLEANROOMSIAMBackend {
	return &mockCLEANROOMSIAMBackend{
...
func (m *mockCLEANROOMSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCLEANROOMSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCLEANROOMSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCLEANROOMSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCLEANROOMSEnforcementServer(t *testing.T, iamBackend *mockCLEANROOMSIAMBackend) *httptest.Server {
	t.Helper()

	backend := cleanrooms.NewInMemoryBackend("000000000000", "us-east-1")
	handler := cleanrooms.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIACLEANROOMSUSER1",
			method:      "GET",
			path:        "/collaborations",
			body:        ``,
			setupBackend: func(b *mockCLEANROOMSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACLEANROOMSUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCLEANROOMSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCLEANROOMSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/cleanrooms/aws4_request, SignedHeaders=host, Signature=mock",
			)
