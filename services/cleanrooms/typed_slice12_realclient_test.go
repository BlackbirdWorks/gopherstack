...
func TestTypedSlice12_CleanRooms(t *testing.T) {
	t.Parallel()

	t.Run("schema_ops_unpopulated", func(t *testing.T) {
		t.Parallel()
		...
		// Schemas are derived from ConfiguredTable+ConfiguredTableAssociation
		// state this backend does not yet compute (disclosed gap, PARITY.md
		// deferred list) -- these ops are exercised through their documented
		// empty/not-found real-client behavior, which is itself the
		// contract this pass verifies (not previously driven by any typed
		// client at all).
	...
	t.Run("configured_table_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("configured_table_association_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("intermediate_table_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("analysis_template_update_delete", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("id_mapping_table_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("id_namespace_association_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("configured_audience_model_association_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("membership_and_member_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("privacy_budget_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
	...
	t.Run("protected_job_and_query_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
		...
		// Cancel immediately, before any Get call auto-advances the job to a
		// terminal SUCCESS state (advanceProtectedJobsLocked runs on every
		// read) -- UpdateProtectedJob rejects cancelling an already-terminal
		// job with ConflictException.
	...
	t.Run("collaboration_change_request_lifecycle", func(t *testing.T) {
		t.Parallel()
		client := newRoundTripTestClient(t)
		ctx := t.Context()
