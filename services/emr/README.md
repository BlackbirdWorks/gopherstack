...
**Parity grade: A** · SDK `aws-sdk-go-v2/service/emr@v1.64.4` · last audited 2026-09-19 (`d522d763f`)
...
- "CancelSteps.StepCancellationOption (reqfieldiff tier-1, 2026-09-18) is not read: it chooses SEND_INTERRUPT vs TERMINATE_PROCESS semantics for cancelling a RUNNING step, but this backend's Step state machine has only PENDING/COMPLETED/CANCELLED -- steps never reach RUNNING (models.go), so cancelStep only ever accepts a still-PENDING step. There is no RUNNING-step cancellation path for the option to distinguish. (bd: unfiled)"
