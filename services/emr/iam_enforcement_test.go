...
type mockEMRIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockEMRIAMBackend() *mockEMRIAMBackend {
	return &mockEMRIAMBackend{
...
func (m *mockEMRIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockEMRIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockEMRIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockEMRIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupEMREnforcementServer(t *testing.T, iamBackend *mockEMRIAMBackend) *httptest.Server {
	t.Helper()

	backend := emr.NewInMemoryBackend("000000000000", "us-east-1")
	handler := emr.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockEMRIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAEMRUSER2",
			target:      "ElasticMapReduce.RunJobFlow",
			body:        `{"Name":"test-cluster","Instances":{"MasterInstanceType":"m5.xlarge"}}`,
			setupBackend: func(b *mockEMRIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAEMRUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockEMRIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupEMREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/elasticmapreduce/aws4_request, SignedHeaders=host, Signature=mock",
			)
