...
type mockAMPLIFYIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockAMPLIFYIAMBackend() *mockAMPLIFYIAMBackend {
	return &mockAMPLIFYIAMBackend{
...
func (m *mockAMPLIFYIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockAMPLIFYIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockAMPLIFYIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockAMPLIFYIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupAMPLIFYEnforcementServer(t *testing.T, iamBackend *mockAMPLIFYIAMBackend) *httptest.Server {
	t.Helper()

	backend := amplify.NewInMemoryBackend("000000000000", "us-east-1")
	handler := amplify.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockAMPLIFYIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAAMPLIFYUSER1",
			method:      "GET",
			path:        "/apps",
			body:        ``,
			setupBackend: func(b *mockAMPLIFYIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAAMPLIFYUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAAMPLIFYUSER2",
			method:      "POST",
			path:        "/apps",
			body:        `{"name":"test-app"}`,
			setupBackend: func(b *mockAMPLIFYIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAAMPLIFYUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockAMPLIFYIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupAMPLIFYEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/amplify/aws4_request, SignedHeaders=host, Signature=mock",
			)
