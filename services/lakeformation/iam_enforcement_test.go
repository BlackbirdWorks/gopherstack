...
type mockLAKEFORMATIONIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockLAKEFORMATIONIAMBackend() *mockLAKEFORMATIONIAMBackend {
	return &mockLAKEFORMATIONIAMBackend{
...
func (m *mockLAKEFORMATIONIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockLAKEFORMATIONIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockLAKEFORMATIONIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockLAKEFORMATIONIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupLAKEFORMATIONEnforcementServer(t *testing.T, iamBackend *mockLAKEFORMATIONIAMBackend) *httptest.Server {
	t.Helper()

	backend := lakeformation.NewInMemoryBackend()
	handler := lakeformation.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockLAKEFORMATIONIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIALAKEFORMATIONUSER1",
			method:      "POST",
			path:        "/GetDataLakeSettings",
			body:        `{}`,
			setupBackend: func(b *mockLAKEFORMATIONIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIALAKEFORMATIONUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIALAKEFORMATIONUSER2",
			method:      "POST",
			path:        "/PutDataLakeSettings",
			body:        `{"DataLakeSettings":{"DataLakeAdmins":[]}}`,
			setupBackend: func(b *mockLAKEFORMATIONIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIALAKEFORMATIONUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockLAKEFORMATIONIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupLAKEFORMATIONEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/lakeformation/aws4_request, SignedHeaders=host, Signature=mock",
			)
