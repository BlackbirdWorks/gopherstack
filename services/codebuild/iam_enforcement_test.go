...
type mockCODEBUILDIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCODEBUILDIAMBackend() *mockCODEBUILDIAMBackend {
	return &mockCODEBUILDIAMBackend{
...
func (m *mockCODEBUILDIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCODEBUILDIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCODEBUILDIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCODEBUILDIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCODEBUILDEnforcementServer(t *testing.T, iamBackend *mockCODEBUILDIAMBackend) *httptest.Server {
	t.Helper()

	backend := codebuild.NewInMemoryBackend("000000000000", "us-east-1")
	handler := codebuild.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIACODEBUILDUSER1",
			target:      "CodeBuild_20161006.ListProjects",
			body:        `{}`,
			setupBackend: func(b *mockCODEBUILDIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACODEBUILDUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCODEBUILDIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCODEBUILDEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/codebuild/aws4_request, SignedHeaders=host, Signature=mock",
			)
