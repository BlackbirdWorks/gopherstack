...
type mockKAFKAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockKAFKAIAMBackend() *mockKAFKAIAMBackend {
	return &mockKAFKAIAMBackend{
...
func (m *mockKAFKAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockKAFKAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockKAFKAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockKAFKAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupKAFKAEnforcementServer(t *testing.T, iamBackend *mockKAFKAIAMBackend) *httptest.Server {
	t.Helper()

	backend := kafka.NewInMemoryBackend("000000000000", "us-east-1")
	handler := kafka.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAKAFKAUSER1",
			method:      "GET",
			path:        "/v1/clusters",
			body:        ``,
			setupBackend: func(b *mockKAFKAIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAKAFKAUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockKAFKAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupKAFKAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/kafka/aws4_request, SignedHeaders=host, Signature=mock",
			)
