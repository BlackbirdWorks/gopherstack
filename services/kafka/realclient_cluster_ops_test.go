...
// TestRealClient_ClusterOps drives kafka's cluster, configuration, and
// channel op families (gopherstack-n3zi) through the real aws-sdk-go-v2
// client.
func TestRealClient_ClusterOps(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				clusterConfigOut, err := client.UpdateClusterConfiguration(
					ctx,
					&kafkasdk.UpdateClusterConfigurationInput{
						ClusterArn:     clusterArn,
						CurrentVersion: currentVersion(t, client, clusterArn),
		...
		{
			name: "cluster listing v2 and operations",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "configuration update and revision",
			run: func(t *testing.T) {
				t.Helper()
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				disassocOut, err := client.BatchDisassociateScramSecret(
					ctx,
					&kafkasdk.BatchDisassociateScramSecretInput{
						ClusterArn:    clusterArn,
						SecretArnList: []string{secretArn},
					},
				)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				assert.Equal(
					t,
					aws.ToString(vpcConnArn),
					aws.ToString(clientListOut.ClientVpcConnections[0].VpcConnectionArn),
				)
