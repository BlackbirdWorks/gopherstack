...
- **Alarms is honestly empty, not fabricated (gopherstack-cdxe, this pass)**:
  ...
  cloudformation's `bp.GetCloudWatchHandler()`, but is set up at CLI backend-provider init
  time in cli.go, out of bounds for this pass), so a caller that took the ARN at face value
  and called cloudwatch:DescribeAlarms would find nothing. This is the same fabrication
