...
// TestTypedSlice15RealClient drives applicationautoscaling's
// typed-coverage-blind ops (gopherstack-n3zi slice 15) through the real
...
func TestTypedSlice15RealClient(t *testing.T) {
	...
	t.Run("tags", func(t *testing.T) {
		...
		client := newTestAASSDKClient(
			t,
			applicationautoscaling.NewHandler(applicationautoscaling.NewInMemoryBackend("123456789012", "us-east-1")),
		)
		ctx := t.Context()
	...
	t.Run("scheduled action lifecycle", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.PutScheduledAction(ctx, &aassdk.PutScheduledActionInput{
			...
			ScalableTargetAction: &aastypes.ScalableTargetAction{MinCapacity: aws.Int32(2), MaxCapacity: aws.Int32(20)},
		})
		require.NoError(t, err)
	...
	t.Run("scaling activities", func(t *testing.T) {
		t.Parallel()
