...
// newSlice27XRayClient stands up the real aws-sdk-go-v2 X-Ray client against
...
func newSlice27XRayClient(t *testing.T, h *xray.Handler) *xraysdk.Client {
	t.Helper()
...
// TestTypedSlice27RealClient drives xray's remaining typed-client-blind ops
// through the real aws-sdk-go-v2 client (gopherstack-n3zi typed slice 27).
func TestTypedSlice27RealClient(t *testing.T) {
	t.Parallel()

	t.Run("tag resource real wire shape", func(t *testing.T) {
		t.Parallel()

		client := newTestXRayClient(t)
		ctx := t.Context()

		grpOut, err := client.CreateGroup(ctx, &xraysdk.CreateGroupInput{GroupName: aws.String("s27-tag-group")})
		require.NoError(t, err)
	...
	t.Run("resource policies", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("indexing rules", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("insight family", func(t *testing.T) {
		t.Parallel()
		...
		client := newSlice27XRayClient(t, xray.NewHandler(backend))
		ctx := t.Context()
	...
	t.Run("trace retrieval and telemetry", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.GetRetrievedTracesGraph(ctx, &xraysdk.GetRetrievedTracesGraphInput{RetrievalToken: token})
	...
	t.Run("sampling statistics and targets", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("trace segment destination", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("batch get traces and trace graph", func(t *testing.T) {
		t.Parallel()
