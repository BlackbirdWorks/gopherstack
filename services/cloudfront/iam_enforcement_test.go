...
type mockCLOUDFRONTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCLOUDFRONTIAMBackend() *mockCLOUDFRONTIAMBackend {
	return &mockCLOUDFRONTIAMBackend{
...
func (m *mockCLOUDFRONTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCLOUDFRONTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCLOUDFRONTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCLOUDFRONTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCLOUDFRONTEnforcementServer(t *testing.T, iamBackend *mockCLOUDFRONTIAMBackend) *httptest.Server {
	t.Helper()
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockCLOUDFRONTIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIACLOUDFRONTUSER1",
			method:      "GET",
			path:        "/2020-05-31/distribution",
			body:        ``,
			setupBackend: func(b *mockCLOUDFRONTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACLOUDFRONTUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIACLOUDFRONTUSER2",
			method:      "POST",
			path:        "/2020-05-31/distribution",
			body:        `<DistributionConfig xmlns="http://cloudfront.amazonaws.com/doc/2020-05-31/"></DistributionConfig>`,
			setupBackend: func(b *mockCLOUDFRONTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIACLOUDFRONTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCLOUDFRONTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCLOUDFRONTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/cloudfront/aws4_request, SignedHeaders=host, Signature=mock",
			)
