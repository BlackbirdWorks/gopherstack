...
// TestTypedSlice19_CloudFront drives the 9 ops that a real aws-sdk-go-v2
// cloudfront client had never exercised before this slice (gopherstack-n3zi):
...
// family.
func TestTypedSlice19_CloudFront(t *testing.T) {
	t.Parallel()

	t.Run("get_distribution_config", func(t *testing.T) {
		t.Parallel()
		testGetDistributionConfigRealClient(t)
	})

	t.Run("tenant_invalidation_and_customization", func(t *testing.T) {
		t.Parallel()
		testTenantInvalidationAndCustomizationRealClient(t)
	})

	t.Run("verify_dns_configuration", func(t *testing.T) {
		t.Parallel()
		testVerifyDNSConfigurationRealClient(t)
	})

	t.Run("connection_group_and_function", func(t *testing.T) {
		t.Parallel()
		testConnectionGroupAndFunctionRealClient(t)
	})
}

func newSlice19CFClient(t *testing.T) *cfsdk.Client {
	t.Helper()
