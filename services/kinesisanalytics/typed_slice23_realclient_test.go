...
// TestTypedSlice23RealClient drives kinesisanalytics's remaining typed-
...
func TestTypedSlice23RealClient(t *testing.T) {
	...
	t.Run("full application configuration lifecycle", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.AddApplicationCloudWatchLoggingOption(ctx, &kasdk.AddApplicationCloudWatchLoggingOptionInput{
			ApplicationName:             aws.String(appName),
		...
		assert.Nil(t, descAfterInput.ApplicationDetail.InputDescriptions[0].InputProcessingConfigurationDescription)
		...
		require.NoError(t, err)
		require.NotNil(t, descAfterProc.ApplicationDetail.InputDescriptions[0].InputProcessingConfigurationDescription)
		assert.Equal(
		...
		assert.Equal(t, "DEST_SQL_STREAM", aws.ToString(descAfterOutput.ApplicationDetail.OutputDescriptions[0].Name))
		...
		_, err = client.DeleteApplicationReferenceDataSource(ctx, &kasdk.DeleteApplicationReferenceDataSourceInput{
			ApplicationName:             aws.String(appName),
			CurrentApplicationVersionId: aws.Int64(currentVersion()),
			ReferenceId:                 refID,
		})
	...
	t.Run("DiscoverInputSchema samples a real Kinesis reader", func(t *testing.T) {
		t.Parallel()
