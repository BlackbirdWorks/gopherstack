...
type mockKINESISANALYTICSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockKINESISANALYTICSIAMBackend() *mockKINESISANALYTICSIAMBackend {
	return &mockKINESISANALYTICSIAMBackend{
...
func (m *mockKINESISANALYTICSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockKINESISANALYTICSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockKINESISANALYTICSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockKINESISANALYTICSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupKINESISANALYTICSEnforcementServer(t *testing.T, iamBackend *mockKINESISANALYTICSIAMBackend) *httptest.Server {
	t.Helper()

	backend := kinesisanalytics.NewInMemoryBackend("000000000000", "us-east-1")
	handler := kinesisanalytics.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockKINESISANALYTICSIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAKINESISANALYTICSUSER2",
			target:      "KinesisAnalytics_20150814.CreateApplication",
			body:        `{"ApplicationName":"test-app"}`,
			setupBackend: func(b *mockKINESISANALYTICSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAKINESISANALYTICSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockKINESISANALYTICSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupKINESISANALYTICSEnforcementServer(t, iamBackend)
			ctx := t.Context()
