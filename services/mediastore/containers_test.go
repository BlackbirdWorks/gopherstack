		...
		t.Run(tt.name, func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				if tt.delay == 0 {
					...
					// advanceContainerStates only runs lazily (no background
					// goroutine): sleeping past the delay then reading once is enough.
					time.Sleep(tt.delay + time.Millisecond)

					var got *mediastore.Container
					got, err = b.DescribeContainer(context.Background(), container)
					require.NoError(t, err)
					assert.Equal(t, "ACTIVE", got.Status, "container never transitioned to ACTIVE")
				}
				...
				_, err = b.DescribeContainer(context.Background(), container)
				require.Error(t, err, "container was never removed after its deletion delay elapsed")
			})
		})
	}
}
