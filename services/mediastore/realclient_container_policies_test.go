...
// containerPolicyDoc builds a minimal valid container access policy
// document granting public GetObject on containerName.
func containerPolicyDoc(containerName string) string {
...
// TestRealClient_ContainerPolicies drives mediastore's remaining typed-
...
// StopAccessLogging.
func TestRealClient_ContainerPolicies(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "container policy lifecycle",
			run: func(t *testing.T) {
				t.Helper()
				...
				_, err := client.CreateContainer(
					ctx,
					&mediastoresdk.CreateContainerInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)

				policyDoc := containerPolicyDoc(name)
				...
				_, err = client.GetContainerPolicy(
					ctx,
					&mediastoresdk.GetContainerPolicyInput{ContainerName: aws.String(name)},
				)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				getOut, err := client.GetCorsPolicy(
					ctx,
					&mediastoresdk.GetCorsPolicyInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)
				...
				_, err = client.DeleteCorsPolicy(
					ctx,
					&mediastoresdk.DeleteCorsPolicyInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)
		...
		{
			name: "lifecycle policy lifecycle",
			run: func(t *testing.T) {
				t.Helper()
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				getOut, err := client.GetMetricPolicy(
					ctx,
					&mediastoresdk.GetMetricPolicyInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)
				...
				_, err = client.DeleteMetricPolicy(
					ctx,
					&mediastoresdk.DeleteMetricPolicyInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)

				_, err = client.GetMetricPolicy(
					ctx,
					&mediastoresdk.GetMetricPolicyInput{ContainerName: aws.String(name)},
				)
				require.Error(t, err, "policy no longer exists after delete")
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				_, err = client.StartAccessLogging(
					ctx,
					&mediastoresdk.StartAccessLoggingInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)
				...
				_, err = client.StopAccessLogging(
					ctx,
					&mediastoresdk.StopAccessLoggingInput{ContainerName: aws.String(name)},
				)
				require.NoError(t, err)
