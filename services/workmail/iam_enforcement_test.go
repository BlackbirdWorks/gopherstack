...
type mockWORKMAILIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockWORKMAILIAMBackend() *mockWORKMAILIAMBackend {
	return &mockWORKMAILIAMBackend{
...
func (m *mockWORKMAILIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockWORKMAILIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockWORKMAILIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockWORKMAILIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupWORKMAILEnforcementServer(t *testing.T, iamBackend *mockWORKMAILIAMBackend) *httptest.Server {
	t.Helper()

	backend := workmail.NewInMemoryBackend("000000000000", "us-east-1")
	handler := workmail.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockWORKMAILIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIAWORKMAILUSER1",
			target:      "WorkMailService.ListOrganizations",
			body:        `{}`,
			setupBackend: func(b *mockWORKMAILIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAWORKMAILUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAWORKMAILUSER2",
			target:      "WorkMailService.CreateOrganization",
			body:        `{"Alias":"test-org"}`,
			setupBackend: func(b *mockWORKMAILIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAWORKMAILUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockWORKMAILIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupWORKMAILEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/workmail/aws4_request, SignedHeaders=host, Signature=mock",
			)
