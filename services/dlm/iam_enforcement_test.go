...
type mockDLMIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockDLMIAMBackend() *mockDLMIAMBackend {
	return &mockDLMIAMBackend{
...
func (m *mockDLMIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockDLMIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockDLMIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockDLMIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupDLMEnforcementServer(t *testing.T, iamBackend *mockDLMIAMBackend) *httptest.Server {
	t.Helper()

	backend := dlm.NewInMemoryBackend("000000000000", "us-east-1")
	handler := dlm.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIADLMUSER1",
			method:      "GET",
			path:        "/policies",
			body:        ``,
			setupBackend: func(b *mockDLMIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIADLMUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockDLMIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupDLMEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/dlm/aws4_request, SignedHeaders=host, Signature=mock",
			)
