...
`cmd/reqfielddiff -dir fsx` reported 6 tier-1 findings. Fixed 2 (both
wire-visible drops): `CreateFileSystem.NetworkType` (defaults to IPV4 per
its own doc comment, now echoed on `FileSystem.NetworkType`) and
`Create/UpdateDataRepositoryAssociation.ImportedFileChunkSize` (Create
defaults to 1024, Update applies PATCH-style; both now round-trip on
`DataRepositoryAssociation.ImportedFileChunkSize`) -- see the FileSystem and
DataRepositoryAssociation family notes above for citations. The other 4 were
already correctly disclosed by prior passes and re-verified unchanged at
HEAD, no code change: `CreateFileSystem`/`CreateWorkflowVersion`-style
`FileSystemTypeVersion` (own `items_still_open` entry, out-of-scope
sibling-op gap), `DescribeSnapshots.IncludeShared` (structural,
single-tenant backend), and `CreateDataRepositoryAssociation.BatchImportMetaDataOnCreate`
(feature addition, not a value-semantics fix, per its existing
`items_still_open` entry). Proven via two new cases in
`TestRealClient_FileSystemAndStorageConfiguration` (real SDK client,
asserting the returned value). Tier-1: 6 before -> 3 after (the 3 remaining
are all pre-adjudicated structural/scope gaps, not new findings).

Gates: `gofmt -l` (clean), `go build ./...`, `go vet ./services/fsx/...`,
`go test -race -count=1 ./services/fsx/...` (pass), `go test -count=1
./pkgs/persistence/...` (pass, after adding the two new `storedFileSystem`/
`storedDataRepositoryAssoc` field rows to `snapshot_inventory.json` --
...
## 2026-09-19 DataRepositoryTask completion sweep

Reversed the prior day's "fabricated SLA" call: added a modeled-duration
lazy sweep so EXECUTING/CANCELING now settle at SUCCEEDED/CANCELED (see the
DataRepositoryTask family note above). `items_still_open`: 13 -> 12.
...
Drove all 19 typed-client-uncovered ops through the real aws-sdk-go-v2 fsx
client for the first time (`realclient_file_system_and_storage_configuration_test.go`): file system
alias family (AssociateFileSystemAliases, DisassociateFileSystemAliases,
DescribeFileSystemAliases), ReleaseFileSystemNfsV3Locks,
StartMisconfiguredStateRecovery, UpdateFileSystem, UntagResource, the
data-repository-association/file-cache/storage-virtual-machine/volume/
snapshot Update+Delete pairs, DescribeFileCaches, and
DescribeSharedVpcConfiguration/UpdateSharedVpcConfiguration.

**Two real wire bugs found and fixed:**

1. `DescribeSharedVpcConfiguration`/`UpdateSharedVpcConfiguration` used the
   wire key `EnableSharedVpcOnFileSystemCreation` on both request and
   response -- the real field (fsx@v1.68.4
   api_op_DescribeSharedVpcConfiguration.go /
   api_op_UpdateSharedVpcConfiguration.go, confirmed serializers.go:6927-6929
   and deserializers.go:17062-17068/17663-17669) is
   `EnableFsxRouteTableUpdatesFromParticipantAccounts`. A real client's
   request never populated the field this handler read, and its response
   parser never saw the field this handler sent -- both directions were
   silently broken for every real caller. Fixed the Go field name and JSON
   tag throughout (interfaces.go, shared_vpc_configuration.go,
   handler_shared_vpc_configuration.go) and updated the two raw-body tests
   that had pinned the wrong key as correct
   (handler_shared_vpc_configuration_test.go, persistence_test.go) -- the
   same "raw-body test passes on a well-formed body it wrote itself" trap
   this file has hit before.
2. `UpdateStorageVirtualMachine`'s only client-settable field was `Subtype`,
   but real `UpdateStorageVirtualMachineInput` (api_op_UpdateStorageVirtualMachine.go)
   has NO Subtype member at all -- Subtype is server-derived, never a real
   update input on any op. The op's actual (and only) scalar updatable field
   is `SvmAdminPassword` (write-only, never echoed back). A real client could
   never update anything through this op; the field it could set had no
   effect. Fixed: `updateStorageVirtualMachineInput.Subtype` ->
   `SvmAdminPassword`, dropped the fabricated Subtype mutation, updated
   handler_storage_virtual_machines_test.go's raw-body test. See
   items_still_open for the sibling Create-side Subtype finding (same bug
   class, not fixed this pass -- out of scope, Create was already covered).

fsx: 48/48 typed-client covered (was 29/48).

## Notes

### 2026-09-19 (terraform-coverage sweep, datasync-and-sesv2)

Two real bugs found via terraform applies, not unit tests: (1) Volume.OpenZFSConfiguration
was never emitted, so the AWS provider's own root-volume-read step failed right after a
successful aws_fsx_openzfs_file_system create ("empty result"); (2) CreateStorageVirtualMachine
set Lifecycle="AVAILABLE", a file-system-only enum value SVMs never reach for real, so the
provider's own create waiter ("wanted target CREATED, MISCONFIGURED") never completed; also
svmARN omitted the parent file-system-ID path segment real AWS includes. All fixed.
...
## 2026-09-20 (fsx-file-systems terraform coverage)

New Terraform coverage (test/terraform/fixtures/fsx-file-systems.tf,
fsx_file_systems_test.go) for aws_fsx_backup, aws_fsx_data_repository_association,
...
firing before its first poll. Applied `fsx-file-systems.tf`'s `timeouts {
