...
type mockACMPCAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockACMPCAIAMBackend() *mockACMPCAIAMBackend {
	return &mockACMPCAIAMBackend{
...
func (m *mockACMPCAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockACMPCAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockACMPCAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockACMPCAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupACMPCAEnforcementServer(t *testing.T, iamBackend *mockACMPCAIAMBackend) *httptest.Server {
	t.Helper()

	backend := acmpca.NewInMemoryBackend("000000000000", "us-east-1")
	handler := acmpca.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIAACMPCAUSER1",
			target:      "ACMPrivateCA.ListCertificateAuthorities",
			body:        `{}`,
			setupBackend: func(b *mockACMPCAIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAACMPCAUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockACMPCAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupACMPCAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/acm-pca/aws4_request, SignedHeaders=host, Signature=mock",
			)
