...
// TestTypedSlice19_Batch drives the 10 ops that a real aws-sdk-go-v2 batch
// client had never exercised before this slice (gopherstack-n3zi): CancelJob,
...
func TestTypedSlice19_Batch(t *testing.T) {
	t.Parallel()

	t.Run("job_lifecycle_cancel_terminate_snapshot", func(t *testing.T) {
		t.Parallel()
		testJobLifecycleRealClient(t)
	})

	t.Run("consumable_resource", func(t *testing.T) {
		t.Parallel()
		testConsumableResourceRealClient(t)
	})

	t.Run("quota_share", func(t *testing.T) {
		t.Parallel()
		testQuotaShareRealClient(t)
	})

	t.Run("service_job", func(t *testing.T) {
		t.Parallel()
		testServiceJobRealClient(t)
	})
}

func newSlice19BatchClient(t *testing.T) *batchsdk.Client {
	t.Helper()
...
func testJobLifecycleRealClient(t *testing.T) {
	t.Helper()
	ctx := t.Context()
	client := newSlice19BatchClient(t)
