...
type mockBATCHIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockBATCHIAMBackend() *mockBATCHIAMBackend {
	return &mockBATCHIAMBackend{
...
func (m *mockBATCHIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockBATCHIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockBATCHIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockBATCHIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupBATCHEnforcementServer(t *testing.T, iamBackend *mockBATCHIAMBackend) *httptest.Server {
	t.Helper()

	backend := batch.NewInMemoryBackend("000000000000", "us-east-1")
	handler := batch.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockBATCHIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
		...
		},
		{
			...
			accessKeyID: "AKIABATCHUSER2",
			method:      "POST",
			path:        "/v1/createjobqueue",
			body:        `{"jobQueueName":"test-jq","priority":1,"computeEnvironmentOrder":[]}`,
			setupBackend: func(b *mockBATCHIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIABATCHUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockBATCHIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupBATCHEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/batch/aws4_request, SignedHeaders=host, Signature=mock",
			)
