...
// TestRealClient_ApplicationLifecycle drives every remaining uncovered op
...
func TestRealClient_ApplicationLifecycle(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "cloudwatch logging and vpc config, operations",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "output and reference data source",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "presigned url, maintenance config, discover schema, snapshot describe",
			run: func(t *testing.T) {
				t.Helper()
