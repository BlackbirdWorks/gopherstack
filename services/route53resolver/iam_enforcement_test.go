...
type mockROUTE53RESOLVERIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockROUTE53RESOLVERIAMBackend() *mockROUTE53RESOLVERIAMBackend {
	return &mockROUTE53RESOLVERIAMBackend{
...
func (m *mockROUTE53RESOLVERIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockROUTE53RESOLVERIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockROUTE53RESOLVERIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockROUTE53RESOLVERIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupROUTE53RESOLVEREnforcementServer(t *testing.T, iamBackend *mockROUTE53RESOLVERIAMBackend) *httptest.Server {
	t.Helper()

	backend := route53resolver.NewInMemoryBackend("000000000000", "us-east-1")
	handler := route53resolver.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockROUTE53RESOLVERIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupROUTE53RESOLVEREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/route53resolver/aws4_request, SignedHeaders=host, Signature=mock",
			)
