...
func newRealClient(t *testing.T) *datasyncsdk.Client {
	t.Helper()
...
// TestRealClient_LocationAgentAndTask drives every gopherstack-n3zi
// uncovered datasync op through the real aws-sdk-go-v2
// client (newTestDataSyncClient, shared with wire_field_fixes_test.go).
func TestRealClient_LocationAgentAndTask(t *testing.T) {
	t.Parallel()
