...
func (b *InMemoryBackend) CreateLocationFsxLustre(
	...
	locationURI := fmt.Sprintf("%s%s.%s/%s", fsxLustreURIScheme, b.region, fsxShortResourceID(fsxFilesystemArn), sub)
...
func (b *InMemoryBackend) UpdateLocationFsxLustre(locationArn, subdirectory string) error {
	...
	if subdirectory != "" {
		...
		sub := strings.TrimPrefix(subdirectory, "/")
		l.LocationURI = fmt.Sprintf("%s%s.%s/%s", fsxLustreURIScheme, b.region, fsxShortResourceID(fsArn), sub)
	}
