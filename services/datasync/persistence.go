...
// datasyncSnapshotVersion identifies the shape of [backendSnapshot]. It must
...
// datasyncSnapshotVersion and is discarded the same way any other
// incompatible snapshot is.
const datasyncSnapshotVersion = 1

// backendSnapshot is the top-level on-disk shape for the DataSync backend.
//
// Tables holds one JSON-encoded array per registered table name (agents,
// locations, tasks, executions), produced by b.registry.SnapshotAll() --
// every store.Table-backed resource field is a "clean" table (see
// store_setup.go's file doc comment), so no ephemeral DTO registry is needed
// here. Tags is the one map left un-converted (its values are plain string
// maps, not *T). Version guards against decoding a snapshot from an
...
func (b *InMemoryBackend) Snapshot(ctx context.Context) []byte {
	...
	if err != nil {
		...
		// the Manager).
		logger.Load(ctx).WarnContext(ctx, "datasync: snapshot table marshal failed", "error", err)

		return nil
	}

	snap := backendSnapshot{
		Version:   datasyncSnapshotVersion,
		Tables:    tables,
		Tags:      b.tags,
	...
	return persistence.MarshalSnapshot(ctx, "datasync", &snap)
}
...
func (b *InMemoryBackend) Restore(ctx context.Context, data []byte) error {
	...
	if snap.Version != datasyncSnapshotVersion {
		...
		logger.Load(ctx).WarnContext(ctx,
			"datasync: discarding incompatible snapshot version, starting empty",
			"gotVersion", snap.Version, "wantVersion", datasyncSnapshotVersion)
	...
	if err := b.registry.RestoreAll(snap.Tables); err != nil {
		return fmt.Errorf("datasync: restore snapshot tables: %w", err)
	}
...
// This delegation is what actually wires DataSync into the persistence
// Manager.
