...
ops:
  ...
  ListLogGroups: {wire: fixed, errors: ok, state: fixed, persist: ok, note: "FIXED (2026-09-12, gopherstack-n3zi, first typed-client coverage): two real wire bugs. (1) the handler read logGroupNamePrefix (DescribeLogGroups' field) instead of the real ListLogGroupsInput wire key logGroupNamePattern (cloudwatchlogs@v1.86.0 serializers.go: case \"logGroupNamePattern\":), so a real client's filter was silently ignored regardless of what it sent -- fixed by reading the correct field and matching it as a real regular expression (ListLogGroups' documented semantics: prefix via ^, substring, or up to five |-separated patterns; DescribeLogGroups' LogGroupNamePrefix, unaffected, keeps its own literal-prefix semantics). (2) the response reused DescribeLogGroups' full LogGroup shape (arn/logGroupClass/logGroupName/...) instead of the real, narrower ListLogGroupsOutput.LogGroups item type LogGroupSummary (logGroupArn/logGroupClass/logGroupName only -- confirmed against awsAwsjson11_deserializeDocumentLogGroupSummary), so a real client's LogGroupArn field always decoded nil (the wire carried \"arn\", not \"logGroupArn\"). Proven via TestRealClient_LogGroupsDeliveriesAndPolicies/log_groups_listing_and_retention (real aws-sdk-go-v2 client)."}
...
or pure audit history already captured in ops: entries). Fixed 7 real gaps:
UpdateAnomaly.SuppressionPeriod, UpdateScheduledQuery full-replace (was
state-only), GetScheduledQueryHistory time/status filters, ListIntegrations
filters, DescribeIndexPolicies account-wide fallback, PutIndexPolicy/
PutTransformer log-group-existence validation, DescribeQueries QueryLanguage/
QueryDuration/BytesScanned. 15 items remain, tightened. Ledger: 36 -> 15.
...
- `GetScheduledQueryHistory` — sorts `ScheduledQueryRunSummary.TriggeredTimestamp`
  (renamed from the fabricated `InvocationTime` by gopherstack-glxp1, 2026-09-11 --
  see the op entry above) descending, source is `history.Runs`, an
  append-only slice field (`history.Runs = append(history.Runs, &r)`, never
  rebuilt from a table). Same append-ordered-source reasoning as above; not
  fixed.
...
cloudwatchlogs: 57/118 (48.3%) -> 118/118 (100%) typed-covered (61 -> 0
uncovered), 61 ops newly covered, `realclient_log_groups_deliveries_and_policies_test.go` added
(one table-driven `t.Parallel()` test, 25 cases covering every named priority
family: log group KMS/streams/listing/retention/record, export/import
tasks, account policies, data protection policy delete, destinations,
delivery destinations/sources/deliveries, integrations, lookup tables,
metric filters, query definitions, index policies + the two disclosed
empty-list catalogs, syslog configurations, transformer, storage tier
policy, scheduled queries, tags (both the legacy per-log-group family and
the generic TagResource family), bearer token authentication, log anomaly
detector delete). **Two real wire bugs found and fixed**, both structural
and previously unreachable by any typed client at all (see ListLogGroups
and GetLogRecord op entries above): ListLogGroups read the wrong request
field (DescribeLogGroups' logGroupNamePrefix instead of its own
logGroupNamePattern) and emitted the wrong response shape (full LogGroup
instead of the real, narrower LogGroupSummary with a different arn key);
GetLogRecord's pointer format was ambiguous for any log group name
containing "/" (nearly all real ones), corrupting the index parse.
DescribeConfigurationTemplates/DescribeFieldIndexes remain honest
unconditional empty lists (already correctly disclosed above, not
fabricated -- reconfirmed, not fixed). Gates: `go build ./...` (whole
module, clean), `go vet`, `golangci-lint run --new-from-rev=HEAD` (0
issues), `go test -count=1` (all pass); `go test -race -count=1` on this
package alone hits a pre-existing, unrelated data race inside
`wire_ptr_leak_test.go`'s `ptrLeakCaptureTransport` (net/http/context
stdlib internals) that reproduces with this pass's new test file excluded
...
`realclient_log_group_and_query_field_persistence_test.go` against the real typed client. **Newly
...
## 2026-09-18 (reqfielddiff, gopherstack-xhu2t, tier-1 re-pass)

13 tier-1: 1 fixed, 1 recorded, 11 already covered. Fixed:
`FilterLogEvents.StartFromHead` (2026-09-12's "false positive" note was
STALE -- confused with GetLogEvents' own field; genuinely undeclared,
always oldest-first). Recorded: `ListAggregateLogGroupSummaries.
