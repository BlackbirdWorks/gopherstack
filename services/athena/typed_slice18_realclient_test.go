...
const (
	slice18AthenaRegion    = "us-east-1"
	slice18AthenaAccountID = "000000000000"
)

// newSlice18AthenaClient stands up a fresh backend/handler/client triple for
// gopherstack-n3zi typed slice 18.
func newSlice18AthenaClient(t *testing.T) *athenasdk.Client {
	t.Helper()

	backend := athena.NewInMemoryBackend(slice18AthenaRegion, slice18AthenaAccountID)
	h := athena.NewHandler(backend)

	return newTestAthenaClient(t, h)
}

// TestTypedSlice18Athena_RealClient drives every op the census still listed
// as uncovered before this pass (gopherstack-n3zi typed-client coverage
// slice 18).
func TestTypedSlice18Athena_RealClient(t *testing.T) {
	t.Parallel()

	t.Run("capacity_reservations", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newSlice18AthenaClient(t)
	...
	t.Run("sessions", func(t *testing.T) {
		...
		// NotebookId travels inside EngineConfiguration.AdditionalConfigs on
		// the real wire, not a top-level StartSessionInput field (confirmed
		// against athena@v1.60.4 serializers.go -- see handler_sessions.go's
		// startSessionInput.notebookID doc comment for the fix this proves).
		...
		sessionARN := "arn:aws:athena:" + slice18AthenaRegion + ":" + slice18AthenaAccountID + ":session/" + sessionID
	...
	t.Run("prepared_statements", func(t *testing.T) {
		...
		assert.Equal(t, "SELECT * FROM sample_table WHERE id = ?", aws.ToString(got.PreparedStatement.QueryStatement))
	...
	t.Run("named_queries", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newSlice18AthenaClient(t)
	...
	t.Run("data_catalogs", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newSlice18AthenaClient(t)
	...
	t.Run("databases_and_tables", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		client := newSlice18AthenaClient(t)
	...
	t.Run("tags", func(t *testing.T) {
		...
		wgARN := "arn:aws:athena:" + slice18AthenaRegion + ":" + slice18AthenaAccountID + ":workgroup/primary"
	...
	t.Run("query_executions", func(t *testing.T) {
		t.Parallel()
		ctx := t.Context()
		backend := athena.NewInMemoryBackend(slice18AthenaRegion, slice18AthenaAccountID)
		h := athena.NewHandler(backend)
		...
		// This backend completes StartQueryExecution synchronously (no
		// simulated RUNNING window), so StopQueryExecution on a fresh query
		// always sees a terminal state and errors -- correctly, matching
		// real AWS's own InvalidRequestException in that case. Force the
		// query back to RUNNING via the test-only seam to exercise the
		// actually-running path.
