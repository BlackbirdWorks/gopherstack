...
type mockATHENAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockATHENAIAMBackend() *mockATHENAIAMBackend {
	return &mockATHENAIAMBackend{
...
func (m *mockATHENAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockATHENAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockATHENAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockATHENAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupATHENAEnforcementServer(t *testing.T, iamBackend *mockATHENAIAMBackend) *httptest.Server {
	t.Helper()

	backend := athena.NewInMemoryBackend("000000000000", "us-east-1")
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockATHENAIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
		...
		},
		{
			...
			accessKeyID: "AKIAATHENAUSER2",
			target:      "AmazonAthena.CreateWorkGroup",
			body:        `{"Name":"test-wg"}`,
			setupBackend: func(b *mockATHENAIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAATHENAUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockATHENAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupATHENAEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/athena/aws4_request, SignedHeaders=host, Signature=mock",
			)
