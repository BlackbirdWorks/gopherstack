...
type mockMEDIASTOREDATAIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMEDIASTOREDATAIAMBackend() *mockMEDIASTOREDATAIAMBackend {
	return &mockMEDIASTOREDATAIAMBackend{
...
func (m *mockMEDIASTOREDATAIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMEDIASTOREDATAIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMEDIASTOREDATAIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMEDIASTOREDATAIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMEDIASTOREDATAEnforcementServer(t *testing.T, iamBackend *mockMEDIASTOREDATAIAMBackend) *httptest.Server {
	t.Helper()

	backend := mediastoredata.NewInMemoryBackend("us-east-1")
	handler := mediastoredata.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	scopedPolicy := `{
		...
		"Statement":[
			{
				"Effect":"Allow",
				"Action":["mediastoredata:ListItems"],
				"Resource":["*"]
			}
		]
	}`
	...
	tests := []struct {
		setupBackend  func(b *mockMEDIASTOREDATAIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAMEDIASTOREDATAUSER1",
			method:      "GET",
			path:        "/",
			body:        ``,
			setupBackend: func(b *mockMEDIASTOREDATAIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAMEDIASTOREDATAUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAMEDIASTOREDATAUSER2",
			method:      "PUT",
			path:        "/obj1",
			body:        `hello`,
			setupBackend: func(b *mockMEDIASTOREDATAIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMEDIASTOREDATAUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMEDIASTOREDATAIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMEDIASTOREDATAEnforcementServer(t, iamBackend)
			ctx := t.Context()

			req, err := http.NewRequestWithContext(ctx, tt.method, srv.URL+tt.path, strings.NewReader(tt.body))
			require.NoError(t, err)
			req.Header.Set("Content-Type", "application/json")
			req.Header.Set("User-Agent", "aws-sdk-go-v2/mediastoredata")
			req.Header.Set(
				"Authorization",
