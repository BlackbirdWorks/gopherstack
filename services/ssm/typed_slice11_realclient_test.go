...
// TestTypedSlice11RealClient drives ssm's remaining typed-client-blind ops
// through the real aws-sdk-go-v2 ssm client (gopherstack-n3zi typed slice
// 11).
func TestTypedSlice11RealClient(t *testing.T) {
	t.Parallel()

	t.Run("maintenance window targets and tasks", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.DeregisterTaskFromMaintenanceWindow(ctx, &ssmsdk.DeregisterTaskFromMaintenanceWindowInput{
			WindowId:     win.WindowId,
			WindowTaskId: task.WindowTaskId,
		})
		require.NoError(t, err)

		_, err = client.DeregisterTargetFromMaintenanceWindow(ctx, &ssmsdk.DeregisterTargetFromMaintenanceWindowInput{
			WindowId:       win.WindowId,
			WindowTargetId: target.WindowTargetId,
		})
		require.NoError(t, err)
	...
	t.Run("maintenance window lifecycle", func(t *testing.T) {
		t.Parallel()
		...
		getOut, err := client.GetMaintenanceWindow(ctx, &ssmsdk.GetMaintenanceWindowInput{WindowId: win.WindowId})
		require.NoError(t, err)
		...
		afterOut, err := client.GetMaintenanceWindow(ctx, &ssmsdk.GetMaintenanceWindowInput{WindowId: win.WindowId})
		require.NoError(t, err)
		assert.Equal(t, "s11-mw-lifecycle-renamed", aws.ToString(afterOut.Name))
	})

	t.Run("patch baselines", func(t *testing.T) {
		t.Parallel()
		...
		afterOut, err := client.GetPatchBaseline(ctx, &ssmsdk.GetPatchBaselineInput{BaselineId: createOut.BaselineId})
		require.NoError(t, err)
		...
		_, err = client.DeregisterPatchBaselineForPatchGroup(ctx, &ssmsdk.DeregisterPatchBaselineForPatchGroupInput{
			BaselineId: createOut.BaselineId,
			PatchGroup: aws.String("s11-patch-group"),
		})
		require.NoError(t, err)
	...
	t.Run("ops items related items", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("ops metadata", func(t *testing.T) {
		...
		_, err = client.GetOpsMetadata(ctx, &ssmsdk.GetOpsMetadataInput{OpsMetadataArn: createOut.OpsMetadataArn})
	...
	t.Run("managed instances", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("resource data sync", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("cloud connectors", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("compliance and inventory", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("automation step executions and signals", func(t *testing.T) {
		t.Parallel()
		...
		stepsOut, err := client.DescribeAutomationStepExecutions(ctx, &ssmsdk.DescribeAutomationStepExecutionsInput{
		...
		require.NoError(t, err)
		assert.Equal(t, aws.ToString(previewStart.ExecutionPreviewId), aws.ToString(previewGet.ExecutionPreviewId))
	})
	...
	t.Run("start change request execution", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("calendar state", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("ops summary", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("reset service setting", func(t *testing.T) {
		t.Parallel()
