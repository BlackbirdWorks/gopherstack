...
last_audit_commit: a72029cb1  # 2026-09-24 automation-execution/CancelMaintenanceWindowExecution sweep
...
ops:
  ...
  ListCommands: {wire: fixed, errors: fixed, state: fixed, persist: ok, note: "gopherstack-enpq (2026-08-21, structfielddiff pass 6): ListCommandsInput.InstanceId (real, optional) existed as a Go struct field but was never read in the handler body -- a real client's InstanceId filter silently returned every command regardless. Fixed: now filters via slices.Contains(cmd.InstanceIDs, input.InstanceID). TestListCommands_FilterByInstanceID, hand-verified failing against unfixed code. fixed (2026-09-24, filters-silently-ignored sweep): Filters ([]types.CommandFilter) had no ListCommandsInput field at all -- every Filters entry was silently dropped. Added Status/DocumentName/InvokedAfter/InvokedBefore (types.CommandFilter's doc comment enumerates these as the ListCommands-valid keys; ExecutionStage remains unmodeled -- see gaps). Wire keys confirmed lowercase \"key\"/\"value\" per serializers.go:9891-9897 (CommandFilter), not \"Key\"/\"Value\". TestListCommands_FiltersByDocumentName."}
  ListCommandInvocations: {wire: fixed, errors: ok, state: ok, persist: ok, note: "re-verified via structfielddiff -- CommandId/InstanceId filters and pagination already correct (see TestFull_Command_ListByInstanceId). CommandInvocation.DocumentVersion now round-trips (see SendCommand fix). CommandPlugins (per-document-step status/output breakdown), NotificationConfig, CloudWatchOutputConfig, InstanceName, TraceOutput, and Details on the input remain unmodeled -- this backend has no per-plugin execution model (a whole document runs as one synchronous unit, see command_exec.go), so a genuine per-plugin breakdown would require fabricating step names/outputs this backend has no real data for -- see gaps. fixed (2026-09-24, filters-silently-ignored sweep): Filters ([]types.CommandFilter) had no ListCommandInvocationsInput field at all -- same missing-field bug as ListCommands, same fix (Status/DocumentName/InvokedAfter/InvokedBefore; ExecutionStage is documented ListCommands-only, not applicable here). TestListCommandInvocations_FiltersByDocumentName."}
...
items_still_open:
  ...
  - "CreateResourceDataSync's S3Destination.DestinationDataSharing and
    ...
    no real completion event to source a success timestamp, status message, or state
    string from."
  ...
  - "ListOpsItemEvents' OpsItemEventSummary.DetailType and ListOpsItemRelatedItems'
    ...
    in this backend's two hardcoded create/update event records to source a value from
    without fabricating one, and CreatedBy/LastModifiedBy need the same caller-identity
    infra ServiceSetting.LastModifiedUser lacks; a related item also has no update path so
    LastModifiedTime would just duplicate CreatedTime. OpsItemRelatedItem.OpsItemId and
    CreatedTime themselves WERE fixed this pass (previously dropped despite being trivially
    sourced from the request/creation time)."
...
10 flagged ops, member-by-member verified against `ssm@v1.77.0`
(`structfielddiff`). 3 real fixes: ListOpsMetadata's OpsMetadataList leaked
the Get-only Metadata map onto every item (new OpsMetadataListItem type);
ListComplianceSummaries/ListResourceComplianceSummaries were missing the
real SeveritySummary breakdown (added, sourced from each ComplianceItem's
own Severity; ResourceComplianceSummaryItem.OverallSeverity also stopped
being hardcoded "INFORMATIONAL" and is now the resource's real highest
severity); ListOpsItemRelatedItems dropped OpsItemId/CreatedTime despite
both being trivially sourced (request field / creation time) — added, plus
Source/CreatedTime on OpsItemEventSummary (declared but never populated by
either event-recording site). ListAssociationVersions/ListCloudConnectors/
ListComplianceItems/ListDocumentVersions/ListResourceDataSync already exact
modulo pre-existing disclosed gaps. New unsourced gaps recorded in
items_still_open: ResourceDataSyncItem's LastSuccessfulSyncTime/
LastSyncStatusMessage/SyncLastModifiedTime/SyncSource.State (sync never
advances past "InProgress"); OpsItemEventSummary.DetailType and
OpsItemRelatedItem.CreatedBy/LastModifiedBy/LastModifiedTime. Proof:
list_summary_shapes_test.go, real aws-sdk-go-v2 client, hand-verified
failing pre-fix for the OpsMetadata leak (raw-body assertion).
...
### 2026-09-12 (gopherstack-xhu2t slice 1): reqfielddiff tier-1 sweep, 52 -> 52 (all 52 are tool blind spots or now-declared gaps)

Worked every tier-1 finding from `cmd/reqfielddiff` (`gopherstack-xhu2t`
campaign). **Major finding, filed as gopherstack-99nj-style tool blind
spot**: reqfielddiff cannot resolve the decode target through this
service's `jsonOp[I, O]` generic dispatch shape (`handler.go`: every op is
registered as `jsonOp(h.Backend.<Op>)`, so the wire struct IS the backend
method's own second parameter type, never a handler-local struct declared
near the registration site). Confirmed by hand against all 52 findings,
then confirmed a second way: re-ran `reqfielddiff -dir ssm` after fixing/
declaring every field below and the count did not move (still 52) --
proof the tool is structurally blind to this dispatch shape, not merely
slow to notice a declaration (unlike the query-protocol form-read blind
spot in gopherstack-99nj, this one is JSON-protocol but generic-function-
typed). Every one of the other 45 findings not listed below was
hand-verified already declared on the correct Input struct with the exact
SDK wire name AND genuinely read/applied by the backend method (not just
decoded and ignored) -- e.g. `CreateAssociation`'s five flagged fields
(`ApplyOnlyAtCronInterval`/`AssociationDispatchAssumeRole`/
`DocumentVersion`/`MaxConcurrency`/`SyncCompliance`) are all declared on
`CreateAssociationInput` (`models_associations.go`) and applied in
`InMemoryBackend.CreateAssociation` (`associations.go`); `GetServiceSetting`/
`ResetServiceSetting`/`UpdateServiceSetting`'s `SettingId`/`SettingValue`
and `RegisterDefaultPatchBaseline`'s `BaselineId` (all flagged `required in
the SDK`) are declared and read exactly as named (`models_service_
settings.go`, `models_patch_baselines.go`) -- these are the clearest proof
the flag isn't measuring backlog here: a required field the tool claims is
entirely missing is in fact present and enforced.

**Real dropped-parameter fixes (5, all newly declared and now applied)**:
- `DescribeAutomationStepExecutions.ReverseOrder` -- now reverses the
  materialized step list before pagination (`automations.go`).
- `DescribeParameters.Shared` -- this backend has no cross-account
  parameter sharing, so `Shared=true` now honestly returns an empty list
  (`parameters.go`) instead of behaving identically to `Shared` omitted.
- `GetDeployablePatchSnapshotForInstance.UseS3DualStackEndpoint` -- the
  fabricated `SnapshotDownloadUrl` now uses the `s3.dualstack.<region>`
  host form when set (`patch_baselines.go`).
- `RegisterTaskWithMaintenanceWindow`/`UpdateMaintenanceWindowTask`.
  `CutoffBehavior` -- declared, stored on `MaintenanceWindowTask`, and
  echoed back by `GetMaintenanceWindowTask`/`DescribeMaintenanceWindowTasks`/
  `UpdateMaintenanceWindowTask` (`models_maintenance_window.go`,
  `maintenance_window.go`). No cutoff *enforcement* engine was added (this
  backend has no real task-execution-duration simulation to cut off) --
  matching the existing round-trip-only convention this file already uses
  for e.g. `MaxConcurrency`/`MaxErrors` on the same struct.

**Recorded gaps (declared on the wire, left inert, `items_still_open`
updated in this commit)**: `ListCommandInvocations.Details` (no
`CommandPlugin` per-plugin execution model exists anywhere in this
backend); `StartExecutionPreview.DocumentVersion` (no observable echo
point anywhere in the real API's response chain for this op, and this
...
Gates: `go build ./services/ssm/...` clean, `go vet ./services/ssm/...`
clean, `go test -race -count=1 ./services/ssm/...` and
`./pkgs/persistence/...` pass, `golangci-lint run --new-from-rev=HEAD
./services/ssm/...` 0 issues, `go run ./cmd/paritylint` 0 FAIL. No
