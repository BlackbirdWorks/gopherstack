...
last_audit_commit: 22491c31e
last_audit_date: 2026-08-21
overall: A                 # gopherstack-enpq (2026-08-21, structfielddiff pass 7): two more families
...
ops:
  ...
  PutInventory: {wire: ok, errors: ok, state: ok, persist: ok, note: "re-verified -- Context/TypeName/SchemaVersion/CaptureTime/ContentHash/Content all match. PutInventoryOutput.Message (informational only, no behavioral content) not modeled -- low-value, disclosed"}
  ...
  DeleteDocument: {wire: gap, errors: ok, state: ok, persist: ok, note: "gopherstack-enpq pass 7 (structfielddiff): DocumentVersion/Force/VersionName have no Go struct members -- a version-scoped delete request silently deletes the ENTIRE document instead of just that version (more destructive than requested), and Force (bypass a share/association block) is moot since nothing currently blocks deletion. Disclosed, not fixed -- implementing real per-version delete semantics (including AWS's own restriction against deleting the version pinned as DefaultVersion) is new feature work, not a field-diff-sized fix. See gaps."}
  ListDocuments: {wire: fixed, errors: ok, state: ok, persist: ok, note: "gopherstack-enpq pass 7 (structfielddiff): DocumentIdentifier.DisplayName had no Go struct member; added."}
  ...
  DescribeDocumentPermission: {wire: gap, errors: ok, state: ok, persist: ok, note: "gopherstack-enpq pass 7 (structfielddiff): MaxResults/NextToken pagination missing entirely; AccountSharingInfoList is a permanently-empty []any literal, never populated even for accounts ModifyDocumentPermission has genuinely added (real types.AccountSharingInfo{AccountId,SharedDocumentVersion} would need Modify to track SharedDocumentVersion first -- see ModifyDocumentPermission). Disclosed, not fixed. See gaps."}
  ModifyDocumentPermission: {wire: gap, errors: ok, state: ok, persist: ok, note: "gopherstack-enpq pass 7 (structfielddiff): SharedDocumentVersion has no Go struct member at all -- a caller sharing a specific document version gets no error but the version pinning is silently dropped (shared accounts always see whatever DescribeDocumentPermission fabricates, which today is nothing -- see that op's note). Disclosed, not fixed. See gaps."}
...
families:
  ...
  documents: {status: partial, note: "gopherstack-enpq pass 7 (structfielddiff, 2026-08-21): PARTIAL, not fully settled -- per-service-completeness-beats-breadth means this is stated plainly rather than rounded up to ok. 12 ops: CreateDocument/UpdateDocument/DescribeDocument/GetDocument/ListDocuments/ListDocumentVersions were diffed and fixed (see each op's ops: note -- Attachments functional no-op + wrong wire key, DisplayName/Hash/HashType/Sha1/Update's TargetType all missing entirely). ListDocumentMetadataHistory/UpdateDocumentMetadata/UpdateDocumentDefaultVersion had missing required-field validation fixed (all three were on TestStubOps_SimpleCalls's lead list of ops silently accepting an empty body). DescribeDocumentPermission/ModifyDocumentPermission/DeleteDocument WERE structfielddiff'd but their real findings were disclosed rather than fixed this pass -- see gaps and each op's own note. VersionName (modeled on DocumentVersionInfo only, never populated, and entirely absent from CreateDocumentInput/UpdateDocumentInput/DocumentDescription/GetDocumentOutput) needs a full selector redesign mirroring resolveDocumentVersionSelector's $LATEST/$DEFAULT handling -- disclosed, not attempted. — Prior-pass notes preserved: CreateDocument/UpdateDocument/DescribeDocument content-leak and $DEFAULT/$LATEST conflation (bd gopherstack-1hg, closed) fixed; the version-cap eviction (maxDocumentVersionCap=1000) never evicts the DefaultVersion-pinned entry."}
