...
type mockOUTPOSTSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockOUTPOSTSIAMBackend() *mockOUTPOSTSIAMBackend {
	return &mockOUTPOSTSIAMBackend{
...
func (m *mockOUTPOSTSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockOUTPOSTSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockOUTPOSTSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockOUTPOSTSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupOUTPOSTSEnforcementServer(t *testing.T, iamBackend *mockOUTPOSTSIAMBackend) *httptest.Server {
	t.Helper()

	backend := outposts.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := outposts.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockOUTPOSTSIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAOUTPOSTSUSER1",
			method:      "GET",
			path:        "/outposts",
			body:        ``,
			setupBackend: func(b *mockOUTPOSTSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAOUTPOSTSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAOUTPOSTSUSER2",
			method:      "POST",
			path:        "/outposts",
			body:        `{"Name":"test-op","SiteId":"site-1"}`,
			setupBackend: func(b *mockOUTPOSTSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAOUTPOSTSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockOUTPOSTSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupOUTPOSTSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/outposts/aws4_request, SignedHeaders=host, Signature=mock",
			)
