...
ops:
  ...
  GetSiteAddress: {wire: fixed, errors: ok, state: ok, persist: ok, note: "GET /sites/{SiteId}/address; AddressType as query param, returns Shipping or Operating full Address. FIXED 2026-09-11 (gopherstack-mven required-output sweep): all seven required Address fields (AddressLine1/City/ContactName/ContactPhoneNumber/CountryCode/PostalCode/StateOrRegion, types/types.go:11-47) carried omitempty; each is *string on the real wire, so the client-side required check only rejects nil, not an empty string -- a legitimate empty value on any of them was silently dropped."}
