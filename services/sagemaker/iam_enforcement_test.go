...
type mockSAGEMAKERIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockSAGEMAKERIAMBackend() *mockSAGEMAKERIAMBackend {
	return &mockSAGEMAKERIAMBackend{
...
func (m *mockSAGEMAKERIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockSAGEMAKERIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockSAGEMAKERIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockSAGEMAKERIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupSAGEMAKEREnforcementServer(t *testing.T, iamBackend *mockSAGEMAKERIAMBackend) *httptest.Server {
	t.Helper()

	backend := sagemaker.NewInMemoryBackend("000000000000", "us-east-1")
	handler := sagemaker.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIASAGEMAKERUSER1",
			target:      "SageMaker.ListNotebookInstances",
			body:        `{}`,
			setupBackend: func(b *mockSAGEMAKERIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIASAGEMAKERUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockSAGEMAKERIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupSAGEMAKEREnforcementServer(t, iamBackend)
			ctx := t.Context()
