...
func TestHandler_InferenceComponentLifecycle(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		pollRec := doSageMakerRequest(t, h, "DescribeInferenceComponent", map[string]any{
			"InferenceComponentName": "my-component",
		})
		var pollResp map[string]any
		require.NoError(t, json.Unmarshal(pollRec.Body.Bytes(), &pollResp))

		if pollResp["InferenceComponentStatus"] != "InService" {
			return false
		}

		rc, _ := pollResp["RuntimeConfig"].(map[string]any)

		return rc["CurrentCopyCount"] == float64(2)
