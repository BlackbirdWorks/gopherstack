...
type mockVERIFIEDPERMISSIONSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockVERIFIEDPERMISSIONSIAMBackend() *mockVERIFIEDPERMISSIONSIAMBackend {
	return &mockVERIFIEDPERMISSIONSIAMBackend{
...
func (m *mockVERIFIEDPERMISSIONSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockVERIFIEDPERMISSIONSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockVERIFIEDPERMISSIONSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockVERIFIEDPERMISSIONSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}
...
func setupVERIFIEDPERMISSIONSEnforcementServer(t *testing.T, iamBackend *mockVERIFIEDPERMISSIONSIAMBackend) *httptest.Server {
	...
	handler := verifiedpermissions.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockVERIFIEDPERMISSIONSIAMBackend)
		name          string
		accessKeyID   string
		target        string
		body          string
	...
	}{
		{
			...
			accessKeyID: "AKIAVERIFIEDPERMISSIONSUSER1",
			target:      "VerifiedPermissions.ListPolicyStores",
			body:        `{}`,
			setupBackend: func(b *mockVERIFIEDPERMISSIONSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAVERIFIEDPERMISSIONSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAVERIFIEDPERMISSIONSUSER2",
			target:      "VerifiedPermissions.CreatePolicyStore",
			body:        `{"validationSettings":{"mode":"OFF"}}`,
			setupBackend: func(b *mockVERIFIEDPERMISSIONSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAVERIFIEDPERMISSIONSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockVERIFIEDPERMISSIONSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupVERIFIEDPERMISSIONSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/verifiedpermissions/aws4_request, SignedHeaders=host, Signature=mock",
			)
