...
// TestRealClient_PolicyStoreAndAuthorization drives every gopherstack-n3zi
// uncovered verifiedpermissions op through the real aws-sdk-go-v2 client.
func TestRealClient_PolicyStoreAndAuthorization(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				assert.Equal(
					t,
					aws.ToString(createOut.PolicyStoreId),
					aws.ToString(listOut.PolicyStores[0].PolicyStoreId),
				)
		...
		{
			name: "policy_store_alias_lifecycle",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "identity_source_lifecycle",
			run: func(t *testing.T) {
				t.Helper()
					...
					Requests: []types.BatchIsAuthorizedInputItem{
						{
							Principal: &types.EntityIdentifier{
								...
								EntityId:   aws.String("alice"),
							},
							Action: &types.ActionIdentifier{
								ActionType: aws.String("Action"),
								ActionId:   aws.String("view"),
							},
							Resource: &types.EntityIdentifier{
								EntityType: aws.String("Photo"),
								EntityId:   aws.String("photo1"),
							},
						},
					},
				})
		...
		{
			name: "token_based_authorization",
			run: func(t *testing.T) {
				t.Helper()
				...
				isOut, err := client.IsAuthorizedWithToken(ctx, &avpsdk.IsAuthorizedWithTokenInput{
					...
					Resource: &types.EntityIdentifier{
						EntityType: aws.String("Resource"),
						EntityId:   aws.String("res1"),
					},
				})
				require.NoError(t, err)
