...
type mockRESILIENCEHUBIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockRESILIENCEHUBIAMBackend() *mockRESILIENCEHUBIAMBackend {
	return &mockRESILIENCEHUBIAMBackend{
...
func (m *mockRESILIENCEHUBIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockRESILIENCEHUBIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockRESILIENCEHUBIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockRESILIENCEHUBIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupRESILIENCEHUBEnforcementServer(t *testing.T, iamBackend *mockRESILIENCEHUBIAMBackend) *httptest.Server {
	t.Helper()

	backend := resiliencehub.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := resiliencehub.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockRESILIENCEHUBIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIARESILIENCEHUBUSER1",
			method:      "GET",
			path:        "/list-apps",
			body:        ``,
			setupBackend: func(b *mockRESILIENCEHUBIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIARESILIENCEHUBUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIARESILIENCEHUBUSER2",
			method:      "POST",
			path:        "/create-app",
			body:        `{"name":"test-app"}`,
			setupBackend: func(b *mockRESILIENCEHUBIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIARESILIENCEHUBUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockRESILIENCEHUBIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupRESILIENCEHUBEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/resiliencehub/aws4_request, SignedHeaders=host, Signature=mock",
			)
