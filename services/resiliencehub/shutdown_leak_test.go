...
func TestHandler_ShutdownStopsScheduledTransitions_NoLeak(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		for range iterations {
			b := resiliencehub.NewInMemoryBackend(context.Background(), "123456789012", "us-east-1")
			h := resiliencehub.NewHandler(b)

			shutdowner, ok := any(h).(service.Shutdowner)
			require.True(t, ok,
				"resiliencehub.Handler must implement service.Shutdowner so cli.go's shutdownServices reaches it")
