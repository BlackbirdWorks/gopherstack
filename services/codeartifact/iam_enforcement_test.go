...
type mockCODEARTIFACTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockCODEARTIFACTIAMBackend() *mockCODEARTIFACTIAMBackend {
	return &mockCODEARTIFACTIAMBackend{
...
func (m *mockCODEARTIFACTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockCODEARTIFACTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockCODEARTIFACTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockCODEARTIFACTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupCODEARTIFACTEnforcementServer(t *testing.T, iamBackend *mockCODEARTIFACTIAMBackend) *httptest.Server {
	t.Helper()

	backend := codeartifact.NewInMemoryBackend("000000000000", "us-east-1")
	handler := codeartifact.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockCODEARTIFACTIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIACODEARTIFACTUSER1",
			method:      "POST",
			path:        "/v1/domains",
			body:        `{}`,
			setupBackend: func(b *mockCODEARTIFACTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIACODEARTIFACTUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIACODEARTIFACTUSER2",
			method:      "POST",
			path:        "/v1/domain?domain=test-domain",
			body:        ``,
			setupBackend: func(b *mockCODEARTIFACTIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIACODEARTIFACTUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockCODEARTIFACTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupCODEARTIFACTEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/codeartifact/aws4_request, SignedHeaders=host, Signature=mock",
			)
