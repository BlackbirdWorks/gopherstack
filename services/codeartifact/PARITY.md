...
ops:
  ...
  PutDomainPermissionsPolicy: {wire: fixed, errors: fixed, state: fixed, persist: ok, note: "FIXED 2026-09-12 (gopherstack-n3zi) -- unlike its Get/Delete siblings (domain/domain-owner as real httpQuery params) and unlike PutRepositoryPermissionsPolicy (domain/repository ARE httpQuery params on that op), PutDomainPermissionsPolicy's real Input has NO httpQuery bindings at all (serializers.go's awsRestjson1_serializeOpHttpBindingsPutDomainPermissionsPolicyInput returns nil unconditionally) -- domain is a plain JSON body member. The handler read domainName from the URL query string exclusively, so every real aws-sdk-go-v2 client's PutDomainPermissionsPolicy call failed with a fabricated 'domain is required' 400, regardless of what was actually sent. Fixed by adding domain to the body-decoded struct and dropping the query-param read entirely. FIXED (gopherstack-6flj) — PolicyDocument is 'This member is required.' on the real Input (confirmed via the real SDK's own generated client-side validator, validators.go) but was silently defaulted to an empty-statement policy instead of rejected with ValidationException; a raw caller (not a real SDK client, which can't send this request at all) could reach the old lenient behavior. ALSO FIXED (this pass) — PolicyRevision ('This revision is used for optimistic locking, which prevents others from overwriting your changes to the domain's resource policy.') was accepted and parsed nowhere: any caller could silently overwrite another caller's policy update. Now enforced against the stored policy's revision (ConflictException on mismatch); an omitted revision is still accepted unconditionally, matching the real optional field"}
...
- **2026-09-18 (gopherstack-xhu2t reqfielddiff tier-1 sweep)**: 2 tier-1 undeclared request
  fields. `PublishPackageVersion.unfinished` fixed -- was never read, every publish
  hardcoded Status=Published. `CopyPackageVersions.includeFromUpstream` recorded as a
  missing feature -- Repository.UpstreamRepositories is real stored config but inert
  everywhere, no upstream-resolution subsystem exists to toggle. tier1 count 2 -> 1.

- **2026-09-12 (typed coverage sweep, gopherstack-n3zi)**: added
  `realclient_domain_repository_and_package_management_test.go`, driving all 23 previously
  typed-client-uncovered ops (domain auth/policy, repository
  endpoint/policy/external-connection, package group CRUD, associated
  packages, package version asset/readme/dependencies, tags) through the
  real `aws-sdk-go-v2` client. **Two real wire bugs, both found only by
  decoding through a real typed client**:
  1. `PutDomainPermissionsPolicy` read `domain` from the URL query string,
     but the real op's Input has no httpQuery bindings at all -- `domain`
     travels in the JSON body (see ops table entry above). Every real
     client call failed with a fabricated "domain is required" 400. Fixed
     the handler to decode `domain` from the body; updated 8 raw-body test
     call sites (`handler_domains_test.go`, `wire_field_fixes_test.go`)
     that had pinned the query-string form as correct -- this repo's own
     documented "raw-body test passes on a well-formed body it wrote
     itself" trap.
  2. `GetPackageVersionAsset`'s `AssetName`/`PackageVersion`/
     `PackageVersionRevision` are real HTTP response headers
     (`X-AssetName`/`X-PackageVersion`/`X-PackageVersionRevision`), not
     body fields -- the handler wrote only `c.Blob` with no headers, so all
     three were always nil/empty for a real client regardless of what was
     published. Fixed by changing the backend method to also return the
     owning `PackageVersion` and setting the three headers in the handler.
  codeartifact: 25/48 -> 48/48 typed-client covered.
