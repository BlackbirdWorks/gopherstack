...
// TestRealClient_ConfigAndIdentity drives sesv2's remaining typed-client-
// blind ops through the real aws-sdk-go-v2 sesv2 client (gopherstack-n3zi).
func TestRealClient_ConfigAndIdentity(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name: "configuration set event destinations",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "configuration set attribute puts and list",
			run: func(t *testing.T) {
				t.Helper()
				...
				_, err = client.PutConfigurationSetDeliveryOptions(
					ctx,
					&sesv2sdk.PutConfigurationSetDeliveryOptionsInput{
						ConfigurationSetName: aws.String("s11-cfgattrs"),
				...
				_, err = client.PutConfigurationSetReputationOptions(
					ctx,
					&sesv2sdk.PutConfigurationSetReputationOptionsInput{
						ConfigurationSetName:     aws.String("s11-cfgattrs"),
		...
		{
			name: "custom verification email templates",
			run: func(t *testing.T) {
				t.Helper()
				...
				_, err = client.UpdateCustomVerificationEmailTemplate(
					ctx,
					&sesv2sdk.UpdateCustomVerificationEmailTemplateInput{
						TemplateName:          aws.String("s11-cvt"),
						FromEmailAddress:      aws.String("verify-from@example.com"),
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				createOut, err := client.CreateDeliverabilityTestReport(
					ctx,
					&sesv2sdk.CreateDeliverabilityTestReportInput{
				...
				listOut, err := client.ListDeliverabilityTestReports(
					ctx,
					&sesv2sdk.ListDeliverabilityTestReportsInput{},
				)
				require.NoError(t, err)
		...
		{
			name: "email identity attributes and delete",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "contacts and contact lists",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "dedicated ip pools and account warmup",
			run: func(t *testing.T) {
				t.Helper()
				...
				_, err = client.PutDedicatedIpPoolScalingAttributes(
					ctx,
					&sesv2sdk.PutDedicatedIpPoolScalingAttributesInput{
						PoolName:    aws.String("s11-pool"),
				...
				_, err = client.PutAccountDedicatedIpWarmupAttributes(
					ctx,
					&sesv2sdk.PutAccountDedicatedIpWarmupAttributesInput{
		...
		{
			name: "account sending, suppression and vdm attributes",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "multi region endpoint delete",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "tenants and tenant resource associations",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "email address insights, message insights and tags",
			run: func(t *testing.T) {
				t.Helper()
				...
				require.NotNil(
					t,
					insightsOut.MailboxValidation,
					"GetEmailAddressInsights must return a validation result",
				)
