...
type mockEMRSERVERLESSIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockEMRSERVERLESSIAMBackend() *mockEMRSERVERLESSIAMBackend {
	return &mockEMRSERVERLESSIAMBackend{
...
func (m *mockEMRSERVERLESSIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockEMRSERVERLESSIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockEMRSERVERLESSIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockEMRSERVERLESSIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupEMRSERVERLESSEnforcementServer(t *testing.T, iamBackend *mockEMRSERVERLESSIAMBackend) *httptest.Server {
	t.Helper()

	backend := emrserverless.NewInMemoryBackend("000000000000", "us-east-1")
	handler := emrserverless.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockEMRSERVERLESSIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAEMRSERVERLESSUSER1",
			method:      "GET",
			path:        "/applications",
			body:        ``,
			setupBackend: func(b *mockEMRSERVERLESSIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAEMRSERVERLESSUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAEMRSERVERLESSUSER2",
			method:      "POST",
			path:        "/applications",
			body:        `{"name":"test-app","releaseLabel":"emr-6.6.0","type":"Spark"}`,
			setupBackend: func(b *mockEMRSERVERLESSIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAEMRSERVERLESSUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockEMRSERVERLESSIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupEMRSERVERLESSEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/emr-serverless/aws4_request, SignedHeaders=host, Signature=mock",
			)
