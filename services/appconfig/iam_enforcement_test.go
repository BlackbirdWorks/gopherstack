...
type mockAPPCONFIGIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockAPPCONFIGIAMBackend() *mockAPPCONFIGIAMBackend {
	return &mockAPPCONFIGIAMBackend{
...
func (m *mockAPPCONFIGIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockAPPCONFIGIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockAPPCONFIGIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockAPPCONFIGIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupAPPCONFIGEnforcementServer(t *testing.T, iamBackend *mockAPPCONFIGIAMBackend) *httptest.Server {
	t.Helper()

	backend := appconfig.NewInMemoryBackend("000000000000", "us-east-1")
	handler := appconfig.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockAPPCONFIGIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAAPPCONFIGUSER1",
			method:      "GET",
			path:        "/applications",
			body:        ``,
			setupBackend: func(b *mockAPPCONFIGIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAAPPCONFIGUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAAPPCONFIGUSER2",
			method:      "POST",
			path:        "/applications",
			body:        `{"name":"test-app"}`,
			setupBackend: func(b *mockAPPCONFIGIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAAPPCONFIGUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockAPPCONFIGIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupAPPCONFIGEnforcementServer(t, iamBackend)
			ctx := t.Context()
