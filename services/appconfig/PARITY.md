...
last_audit_date: 2026-09-04   # bd gopherstack-5pl: full ranked bug-pattern sweep (delete preconditions,
                               ...
                               # undisclosed gap found and documented (not code-fixed, see gaps below):
                               # DeleteEnvironment/DeleteConfigurationProfile's DeletionProtectionCheck
                               # header is entirely unread. Not fixed because the real trigger condition
                               # (recent appconfigdata GetLatestConfiguration calls) has no honest data
                               # source in this backend -- see the new gap entry for why implementing it
                               # would either be a no-op or fabricate the wrong-direction behavior.
...
ops:
  ...
  DeleteEnvironment: {wire: gap, errors: ok, state: ok, persist: ok, note: "FIXED — same ExtensionAssociation + deployedConfigs cascade-cleanup as DeleteApplication. NOT FIXED, disclosed (2026-09-04, bd gopherstack-5pl): real DeleteEnvironmentInput.DeletionProtectionCheck (api_op_DeleteEnvironment.go, bound to the 'X-Amzn-Deletion-Protection-Check' request header per serializers.go:1267-1270) is never read by this handler at all -- see the deletion_protection_check gap entry below for why this is disclosed rather than fabricated."}
  ...
  DeleteConfigurationProfile: {wire: gap, errors: ok, state: ok, persist: ok, note: "FIXED — same ExtensionAssociation + deployedConfigs cascade-cleanup. NOT FIXED, disclosed (2026-09-04, bd gopherstack-5pl): real DeleteConfigurationProfileInput.DeletionProtectionCheck (api_op_DeleteConfigurationProfile.go, bound to the 'X-Amzn-Deletion-Protection-Check' request header per serializers.go:1120-1122) is never read by this handler at all -- see the deletion_protection_check gap entry below for why this is disclosed rather than fabricated."}
...
gaps:                     # known divergences NOT fixed — link bd issue ids
  ...
  - "NEW (2026-09-04, bd gopherstack-5pl): DeleteEnvironmentInput/DeleteConfigurationProfileInput.DeletionProtectionCheck (BYPASS|APPLY|ACCOUNT_DEFAULT, bound to the 'X-Amzn-Deletion-Protection-Check' request header, serializers.go:1120-1122 and :1267-1270) is a real request member that this handler never reads at all -- confirmed by grep, zero references anywhere in services/appconfig. NOT fixed, deliberately: DeletionProtectionSettings' own doc comment (types.go:222-247) states the check fires when 'AppConfig has called [GetLatestConfiguration] for the configuration profile or from the environment during the specified interval' -- i.e. the trigger condition is recent appconfigdata GetLatestConfiguration call history, not resource age or account settings alone. This backend has zero appconfigdata<->appconfig wiring (confirmed: no cross-package references either direction; appconfigdata's own StartSession checks only its own internal b.profiles map, sessions.go:26-29, never touching the real appconfig Environment/ConfigurationProfile state -- same pre-existing gap already flagged for bd gopherstack-uiyi), so the one signal the real check depends on cannot be produced honestly. AccountSettings.DeletionProtection.Enabled defaults to nil/unset (account_settings.go, store.go:125's zero-value AccountSettings) and CreatedAt IS tracked on both Environment and ConfigurationProfile (models.go), so the 'skip resources created in the past hour' half of the doc'd behavior COULD be modeled honestly -- but wiring only that half while the actual trigger (access recency) is always false is observably identical to today's behavior (delete is never blocked either way), so there is nothing to fix that would change any test-observable output; implementing a blanket 'block whenever Enabled=true' policy instead would be fabrication in the wrong direction (real AWS does NOT block deletion of a never-accessed resource merely because deletion protection is enabled). Left unenforced rather than fabricated, same class as the pre-existing GetConfiguration client_id gap above. No bd issue filed separately -- tracked under this audit's own gopherstack-5pl."
