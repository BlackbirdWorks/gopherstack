...
// TestHandler_OversizedBodySurfacesInternalFailure drives a real Auto Scaling
// client's DescribeAutoScalingGroups with a NextToken large enough to push
...
// RouteMatcher now falls back to the api/autoscaling User-Agent marker (set
// by every aws-sdk-go-v2 autoscaling client) when the body can't be read,
// claims the request, and lets Handler() produce the typed error.
func TestHandler_OversizedBodySurfacesInternalFailure(t *testing.T) {
	...
	_, err := client.DescribeAutoScalingGroups(t.Context(), &assdk.DescribeAutoScalingGroupsInput{
		NextToken: huge,
	}, func(o *assdk.Options) { o.RetryMaxAttempts = 1 })
	require.Error(t, err)
