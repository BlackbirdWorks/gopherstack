...
func (b *InMemoryBackend) CreateAutoScalingGroup(input CreateAutoScalingGroupInput) (*AutoScalingGroup, error) {
	...
	group := &AutoScalingGroup{
		...
		AutoScalingGroupARN:              "arn:aws:autoscaling:" + config.DefaultRegion + ":" + config.DefaultAccountID + ":autoScalingGroup:" + uuid.NewString() + ":autoScalingGroupName/" + input.AutoScalingGroupName,
...
// validHealthCheckTypes are the AWS-supported values for HealthCheckType.
...
// validTerminationPolicies are the AWS-supported termination policy values.
...
// validScalingProcessTypes are the 9 AWS-supported ASG process names.
var validScalingProcessTypes = map[string]bool{
	"Launch":              true,
	"Terminate":           true,
	"HealthCheck":         true,
...
func validateHealthCheckType(hct string) error {
	if hct == "" || validHealthCheckTypes[hct] {
		return nil
	}
...
func (b *InMemoryBackend) CreateLaunchConfiguration(
	...
	lc := &LaunchConfiguration{
		LaunchConfigurationName:      input.LaunchConfigurationName,
		LaunchConfigurationARN:       "arn:aws:autoscaling:" + config.DefaultRegion + ":" + config.DefaultAccountID + ":launchConfiguration:" + uuid.NewString() + ":launchConfigurationName/" + input.LaunchConfigurationName,
...
func (b *InMemoryBackend) PutLifecycleHook(hook LifecycleHook) error {
	...
	if hook.DefaultResult != "" && hook.DefaultResult != "CONTINUE" && hook.DefaultResult != "ABANDON" {
		return fmt.Errorf(
...
func (b *InMemoryBackend) SuspendProcesses(groupName string, processes []string) error {
	...
	for _, p := range processes {
		if !validScalingProcessTypes[p] {
			return fmt.Errorf("%w: unknown scaling process %q", ErrInvalidParameter, p)
		}
	}
...
func (b *InMemoryBackend) SetInstanceHealth(instanceID string, healthStatus string, shouldRespectGracePeriod bool) error {
