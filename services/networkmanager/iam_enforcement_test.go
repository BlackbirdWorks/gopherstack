...
type mockNETWORKMANAGERIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockNETWORKMANAGERIAMBackend() *mockNETWORKMANAGERIAMBackend {
	return &mockNETWORKMANAGERIAMBackend{
...
func (m *mockNETWORKMANAGERIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockNETWORKMANAGERIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockNETWORKMANAGERIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockNETWORKMANAGERIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupNETWORKMANAGEREnforcementServer(t *testing.T, iamBackend *mockNETWORKMANAGERIAMBackend) *httptest.Server {
	t.Helper()

	backend := networkmanager.NewInMemoryBackend(t.Context(), "000000000000", "us-east-1")
	handler := networkmanager.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockNETWORKMANAGERIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIANETWORKMANAGERUSER1",
			method:      "GET",
			path:        "/global-networks",
			body:        ``,
			setupBackend: func(b *mockNETWORKMANAGERIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIANETWORKMANAGERUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIANETWORKMANAGERUSER2",
			method:      "POST",
			path:        "/global-networks",
			body:        `{"Description":"desc"}`,
			setupBackend: func(b *mockNETWORKMANAGERIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIANETWORKMANAGERUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockNETWORKMANAGERIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupNETWORKMANAGEREnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/networkmanager/aws4_request, SignedHeaders=host, Signature=mock",
			)
