...
// TestLifecyclePolicy_NoPolicy verifies that GetChannelLifecyclePolicy
// returns 404 when no policy has been set.
func TestLifecyclePolicy_NoPolicy(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			h := newTestHandler(t)
			createChannelWithTags(t, h, "ch-no-policy", nil)

			code, _ := doRequestJSON(t, h, http.MethodGet, "/channels/ch-no-policy/lifecycle_policy", nil)
			assert.Equal(t, tc.wantCode, code)
		})
	}
}

// TestLifecyclePolicy_RoundTrip verifies the full put-then-get cycle.
func TestLifecyclePolicy_RoundTrip(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			h := newTestHandler(t)
			createChannelWithTags(t, h, "ch-lc-rt", nil)

			code, _ := doRequestJSON(
				t,
				h,
				http.MethodPut,
			...
			code, resp := doRequestJSON(t, h, http.MethodGet, "/channels/ch-lc-rt/lifecycle_policy", nil)
