...
type mockMEDIAPACKAGEIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockMEDIAPACKAGEIAMBackend() *mockMEDIAPACKAGEIAMBackend {
	return &mockMEDIAPACKAGEIAMBackend{
...
func (m *mockMEDIAPACKAGEIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockMEDIAPACKAGEIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockMEDIAPACKAGEIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockMEDIAPACKAGEIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupMEDIAPACKAGEEnforcementServer(t *testing.T, iamBackend *mockMEDIAPACKAGEIAMBackend) *httptest.Server {
	t.Helper()

	backend := mediapackage.NewInMemoryBackend("000000000000", "us-east-1")
	handler := mediapackage.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockMEDIAPACKAGEIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAMEDIAPACKAGEUSER1",
			method:      "GET",
			path:        "/channels",
			body:        ``,
			setupBackend: func(b *mockMEDIAPACKAGEIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAMEDIAPACKAGEUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAMEDIAPACKAGEUSER2",
			method:      "POST",
			path:        "/channels",
			body:        `{"id":"test-c"}`,
			setupBackend: func(b *mockMEDIAPACKAGEIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAMEDIAPACKAGEUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockMEDIAPACKAGEIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupMEDIAPACKAGEEnforcementServer(t, iamBackend)
			ctx := t.Context()
			...
			req.Header.Set(
				...
				"AWS4-HMAC-SHA256 Credential="+tt.accessKeyID+
					"/20260826/us-east-1/mediapackage/aws4_request, SignedHeaders=host, Signature=mock",
			)
