...
type mockBEDROCKAGENTIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockBEDROCKAGENTIAMBackend() *mockBEDROCKAGENTIAMBackend {
	return &mockBEDROCKAGENTIAMBackend{
...
func (m *mockBEDROCKAGENTIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockBEDROCKAGENTIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockBEDROCKAGENTIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockBEDROCKAGENTIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupBEDROCKAGENTEnforcementServer(t *testing.T, iamBackend *mockBEDROCKAGENTIAMBackend) *httptest.Server {
	t.Helper()

	backend := bedrockagent.NewInMemoryBackend("us-east-1", "000000000000")
	handler := bedrockagent.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	scopedPolicy := `{
		...
		"Statement":[
			{
				"Effect":"Allow",
				"Action":["bedrock-agent:ListKnowledgeBases"],
				"Resource":["*"]
			}
		]
	}`
	...
	tests := []struct {
	...
	}{
		{
			...
			accessKeyID: "AKIABEDROCKAGENTUSER1",
			method:      "GET",
			path:        "/knowledgebases",
			body:        ``,
			setupBackend: func(b *mockBEDROCKAGENTIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIABEDROCKAGENTUSER1"] = "user1"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockBEDROCKAGENTIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupBEDROCKAGENTEnforcementServer(t, iamBackend)
			ctx := t.Context()
