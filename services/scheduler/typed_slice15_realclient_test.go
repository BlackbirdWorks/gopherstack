...
// TestTypedSlice15RealClient drives scheduler's typed-coverage-blind ops
// (gopherstack-n3zi slice 15) through the real aws-sdk-go-v2 client.
func TestTypedSlice15RealClient(t *testing.T) {
	t.Parallel()

	t.Run("schedule group lifecycle and tags", func(t *testing.T) {
		t.Parallel()
		...
		_, err = client.GetScheduleGroup(ctx, &schedulersdk.GetScheduleGroupInput{Name: aws.String("s15-group-2")})
		require.Error(t, err)
	})

	t.Run("schedule update and delete", func(t *testing.T) {
		t.Parallel()
