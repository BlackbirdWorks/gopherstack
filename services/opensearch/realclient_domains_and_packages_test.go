...
func newRealClient(t *testing.T) (*opensearch.InMemoryBackend, *opensearchsdk.Client) {
	t.Helper()
...
// createRealClientDomain creates a domain via the real client and returns its
// CreateDomainOutput.
func createRealClientDomain(
	t *testing.T,
...
// TestRealClient_DomainsAndPackages drives every op the census still
// listed as uncovered before this pass (gopherstack-n3zi typed-client
// coverage).
func TestRealClient_DomainsAndPackages(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T) {
				...
				createRealClientDomain(t, client, "domains-fam-1")
				...
				_, err = client.CancelServiceSoftwareUpdate(
					t.Context(),
					&opensearchsdk.CancelServiceSoftwareUpdateInput{
						DomainName: aws.String("domains-fam-1"),
					},
				)
				require.NoError(t, err)
		...
		{
			name: "domain_config_maintenance_family",
			run: func(t *testing.T) {
				t.Helper()

				_, client := newRealClient(t)

				createRealClientDomain(t, client, "maint-fam-1")
		...
		{
			name: "vpc_endpoint_family",
			run: func(t *testing.T) {
				t.Helper()

				_, client := newRealClient(t)

				domOut := createRealClientDomain(t, client, "vpc-fam-1")
		...
		{
			name: "inbound_outbound_connection_family",
			run: func(t *testing.T) {
				t.Helper()
				...
				accepted, err := client.AcceptInboundConnection(
					t.Context(),
					&opensearchsdk.AcceptInboundConnectionInput{
				...
				rejected, err := client.RejectInboundConnection(
					t.Context(),
					&opensearchsdk.RejectInboundConnectionInput{
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				createRealClientDomain(t, client, "pkg-fam-domain")
				...
				listedForDomain, err := client.ListPackagesForDomain(
					t.Context(),
					&opensearchsdk.ListPackagesForDomainInput{
						DomainName: aws.String("pkg-fam-domain"),
					},
				)
				require.NoError(t, err)
				...
				listedForPackage, err := client.ListDomainsForPackage(
					t.Context(),
					&opensearchsdk.ListDomainsForPackageInput{
						PackageID: aws.String(packageID),
					},
				)
				require.NoError(t, err)
				...
				history, err := client.GetPackageVersionHistory(
					t.Context(),
					&opensearchsdk.GetPackageVersionHistoryInput{
						PackageID: aws.String(packageID),
					},
				)
				require.NoError(t, err)
		...
		{
			name: "data_source_family",
			run: func(t *testing.T) {
				t.Helper()

				_, client := newRealClient(t)

				createRealClientDomain(t, client, "ds-fam-domain")
		...
		{
			name: "direct_query_data_source_family",
			run: func(t *testing.T) {
				t.Helper()
				...
				_, err = client.UpdateDirectQueryDataSource(
					t.Context(),
					&opensearchsdk.UpdateDirectQueryDataSourceInput{
						DataSourceName: aws.String("dq-1"),
						DataSourceType: dqType,
					},
				)
				...
				_, err = client.DeleteDirectQueryDataSource(
					t.Context(),
					&opensearchsdk.DeleteDirectQueryDataSourceInput{
						DataSourceName: aws.String("dq-1"),
					},
				)
				require.NoError(t, err)
		...
		{
			name: "data_source_attachment_family",
			run: func(t *testing.T) {
				t.Helper()

				_, client := newRealClient(t)

				domOut := createRealClientDomain(t, client, "dsattach-fam-domain")
		...
		{
			name: "application_capability_family",
			run: func(t *testing.T) {
				t.Helper()
		...
		{
			name: "default_application_setting_family",
			run: func(t *testing.T) {
				t.Helper()
				...
				_, err = client.PutDefaultApplicationSetting(
					t.Context(),
					&opensearchsdk.PutDefaultApplicationSettingInput{
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				createRealClientDomain(t, client, "insight-fam-domain")
		...
		{
			name: "migration_family",
			run: func(t *testing.T) {
				t.Helper()

				_, client := newRealClient(t)

				domOut := createRealClientDomain(t, client, "migration-fam-domain")
				domainARN := aws.ToString(domOut.DomainStatus.ARN)
		...
		},
		{
			...
			run: func(t *testing.T) {
				...
				described, err := client.DescribeReservedInstances(
					t.Context(),
					&opensearchsdk.DescribeReservedInstancesInput{
		...
		{
			name: "scheduled_action_family",
			run: func(t *testing.T) {
				t.Helper()

				backend, client := newRealClient(t)

				createRealClientDomain(t, client, "sched-fam-domain")
