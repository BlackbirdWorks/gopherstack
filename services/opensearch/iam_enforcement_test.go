...
type mockOPENSEARCHIAMBackend struct {
	users    map[string]*iam.User
	keyMap   map[string]string
	policies map[string][]string
}

func newMockOPENSEARCHIAMBackend() *mockOPENSEARCHIAMBackend {
	return &mockOPENSEARCHIAMBackend{
...
func (m *mockOPENSEARCHIAMBackend) GetUserByAccessKeyID(accessKeyID string) (*iam.User, error) {
...
func (m *mockOPENSEARCHIAMBackend) GetPoliciesForUser(userName string) ([]string, error) {
	return m.policies[userName], nil
}

func (m *mockOPENSEARCHIAMBackend) GetPoliciesForRole(roleName string) ([]string, error) {
	return m.policies[roleName], nil
}

func (m *mockOPENSEARCHIAMBackend) GetGroupPoliciesForUser(_ string) ([]string, error) {
	return nil, nil
}

func setupOPENSEARCHEnforcementServer(t *testing.T, iamBackend *mockOPENSEARCHIAMBackend) *httptest.Server {
	t.Helper()

	backend := opensearch.NewInMemoryBackend("000000000000", "us-east-1")
	handler := opensearch.NewHandler(backend)
...
func TestIAMEnforcement(t *testing.T) {
	...
	tests := []struct {
		setupBackend  func(b *mockOPENSEARCHIAMBackend)
		name          string
		accessKeyID   string
		method        string
		path          string
	...
	}{
		{
			...
			accessKeyID: "AKIAOPENSEARCHUSER1",
			method:      "GET",
			path:        "/2021-01-01/domain",
			body:        ``,
			setupBackend: func(b *mockOPENSEARCHIAMBackend) {
				b.users["user1"] = &iam.User{UserName: "user1"}
				b.keyMap["AKIAOPENSEARCHUSER1"] = "user1"
		...
		},
		{
			...
			accessKeyID: "AKIAOPENSEARCHUSER2",
			method:      "POST",
			path:        "/2021-01-01/opensearch/domain",
			body:        `{"DomainName":"test-d"}`,
			setupBackend: func(b *mockOPENSEARCHIAMBackend) {
				b.users["user2"] = &iam.User{UserName: "user2"}
				b.keyMap["AKIAOPENSEARCHUSER2"] = "user2"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			iamBackend := newMockOPENSEARCHIAMBackend()
			tt.setupBackend(iamBackend)

			srv := setupOPENSEARCHEnforcementServer(t, iamBackend)
			ctx := t.Context()
