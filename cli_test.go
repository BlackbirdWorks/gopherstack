	...
	tests := []struct {
		...
		},
		{
			...
			wire: func(t *testing.T, bk resourcegroupstaggingapibackend.StorageBackend) string {
				...
				_, err := redshiftBk.CreateCluster("wiring-test-cluster", "dc2.large", "dev", "admin", nil, "", redshiftbackend.CreateClusterOptions{})
				require.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, bk resourcegroupstaggingapibackend.StorageBackend) (string, func() map[string]string) {
				...
				redshiftBk := redshiftbackend.NewInMemoryBackend(accountID, region)
				_, err := redshiftBk.CreateCluster("roundtrip-cluster", "dc2.large", "dev", "admin", nil, "", redshiftbackend.CreateClusterOptions{})
				require.NoError(t, err)
